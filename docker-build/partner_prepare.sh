...
HANZO_NODE_IMAGE=${HANZO_NODE_IMAGE:-dcspark/hanzo-node}
...
HANZO_NODE_ARCHIVE=dcspark_hanzo-node.tar
HANZO_SOURCE_PATH=../
...
post_prepare_env_info() {
  msg="Edit \"\${DOCKER_COMPOSE_ENV_FILE}\" if you want to start the node with preconfigured ai agents. You have the possibility to add ai agents also from Hanzo Visor."
  info \${msg}
}
...
post_prepare_visor_info() {
  msg="Once everything is up and running, install/start Hanzo Visor and use the default provided settings on the ui."
  info \${msg}
}
