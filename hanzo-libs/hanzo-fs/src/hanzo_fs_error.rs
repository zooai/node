...
impl From<SerdeError> for HanzoFsError {
    fn from(_error: SerdeError) -> Self {
        HanzoFsError::FailedJSONParsing
    }
}

impl From<reqwest::Error> for HanzoFsError {
    fn from(error: reqwest::Error) -> Self {
        HanzoFsError::RequestFailed(error.to_string())
    }
}

impl From<HanzoEmbeddingError> for HanzoFsError {
    fn from(error: HanzoEmbeddingError) -> Self {
        HanzoFsError::FailedEmbeddingGeneration(error.to_string())
    }
}
