...
use crate::simple_parser::file_parser_helper::HanzoFileParser;
...
impl TextGroup {
    ...
    pub fn format_text_for_embedding(&self, max_node_text_size: u64) -> String {
        ...
        let keywords: Vec<String> = HanzoFileParser::extract_keywords(&vec![self.clone()], 1);
    ...
    pub fn push_page_number(&mut self, page_number: u32) {
        ...
        if let Some(page_numbers_metadata) = self.metadata.get(&HanzoFileParser::page_numbers_metadata_key()) {
        ...
        self.metadata.insert(
            HanzoFileParser::page_numbers_metadata_key(),
            format!(
                "[{}]",
                unique_page_numbers
                    .iter()
