...
impl HanzoPath {
    ...
    /// Creates a new HanzoPath from a string slice, ensuring it's absolute relative to the base path.
    /// If `path` is not absolute, it is joined to the base path.
    pub fn from_str(path: &str) -> Self {
        Self::new(path)
    }

    /// Creates a new HanzoPath from a String, ensuring it's absolute relative to the base path.
    /// If `path` is not absolute, it is joined to the base path.
    ...
    /// Converts the HanzoPath to a Path reference.
    pub fn as_path(&self) -> &Path {
        &self.path
    }

    /// Returns the relative path of this HanzoPath with respect to the base path.
    ...
    /// Creates a new HanzoPath representing the base path.
...
// Implement Display for HanzoPath to easily print it
impl fmt::Display for HanzoPath {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

impl<'de> Deserialize<'de> for HanzoPath {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct HanzoPathVisitor;

        impl<'de> Visitor<'de> for HanzoPathVisitor {
            type Value = HanzoPath;
        ...
        deserializer.deserialize_any(HanzoPathVisitor)
    }
}

impl Serialize for HanzoPath {
...
mod tests {
    ...
    fn test_from_string_with_base_path() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let path = HanzoPath::from_string("word_files/christmas.docx".to_string());
        assert_eq!(
            path.as_path(),
            Path::new(&format!(
    ...
    fn test_extension() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let path_with_extension = HanzoPath::from_string("word_files/christmas.docx".to_string());
        assert_eq!(path_with_extension.extension(), Some("docx"));

        let path_without_extension = HanzoPath::from_string("word_files/christmas".to_string());
    ...
    fn test_new_with_base_path() {
        ...
        let hanzo_path = HanzoPath::new(test_path.to_str().unwrap());
        eprintln!("hanzo_path: {:?}", hanzo_path.full_path());
        assert_eq!(hanzo_path.path, test_path);
    }
    ...
    fn test_new_without_base_path() {
        ...
        let hanzo_path = HanzoPath::new(relative_path);
        eprintln!("hanzo_path: {:?}", hanzo_path.full_path());
        eprintln!("expected_path: {:?}", expected_path);

        assert_eq!(hanzo_path.path, expected_path);
    }
    ...
    fn test_new_with_root_path() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let root_path = "/";
        let hanzo_path = HanzoPath::new(root_path);

        let expected_path = HanzoPath::base_path().join(root_path.trim_start_matches('/'));
        assert_eq!(hanzo_path.path, expected_path);
    }
    ...
    fn test_is_file() {
        ...
        let hanzo_path = HanzoPath::from_string(file_path.to_string());

        fs::create_dir_all(hanzo_path.as_path().parent().unwrap()).unwrap();
        fs::write(hanzo_path.as_path(), "test".as_bytes()).unwrap();

        assert!(hanzo_path.is_file());
    }
    ...
    fn test_filename() {
        ...
        let hanzo_path_with_extension = HanzoPath::from_string(path_with_extension.to_string());
        fs::create_dir_all(hanzo_path_with_extension.as_path().parent().unwrap()).unwrap();
        fs::write(hanzo_path_with_extension.as_path(), "test".as_bytes()).unwrap();
        assert_eq!(hanzo_path_with_extension.filename(), Some("christmas.docx"));
        ...
        let hanzo_path_with_no_filename = HanzoPath::from_string(path_with_no_filename.to_string());
        assert_eq!(hanzo_path_with_no_filename.filename(), None);
    }
    ...
    fn test_serialize_relative_path() {
        ...
        // Create a HanzoPath instance
        let path = HanzoPath::from_string("word_files/christmas.docx".to_string());

        // Serialize the HanzoPath
