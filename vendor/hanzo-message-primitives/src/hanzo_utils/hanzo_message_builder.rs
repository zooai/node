use crate::hanzo_message::hanzo_message::NodeApiData;
...
use super::{
    encryption::{clone_static_secret_key, encryption_secret_key_to_string}, hanzo_time::HanzoStringTime, signatures::{clone_signature_secret_key, signature_secret_key_to_string}
};

pub type HanzoNameString = String;
...
#[derive(Clone)]
pub struct HanzoMessageBuilder {
...
impl HanzoMessageBuilder {
    ...
    pub fn external_metadata(mut self, recipient: HanzoNameString, sender: HanzoNameString) -> Self {
        ...
        let scheduled_time = HanzoStringTime::generate_time_now();
    ...
    pub fn build(&self) -> Result<HanzoMessage, &'static str> {
        ...
        if let Some(internal_metadata) = &mut new_self.internal_metadata {
            ...
            let mut unsigned_msg = HanzoMessage {
            ...
            let signed_body = match unsigned_msg.body {
                MessageBody::Unencrypted(ref body) => HanzoBody {
                    message_data: new_message_data.clone(),
...
impl std::fmt::Debug for HanzoMessageBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("HanzoMessageBuilder")
