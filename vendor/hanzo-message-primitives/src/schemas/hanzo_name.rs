...
// Valid Examples - Hanzo Network (Chain ID: 36963 mainnet, 36962 testnet)
// @alice.hanzo (mainnet - default)
// @alice.hanzo-mainnet (mainnet - explicit)
// @alice.hanzo-testnet (testnet)
// @alice.hanzotest (testnet alias)
// @alice.hanzo/profile
// @alice.hanzo/profile/agent/myChatGPTAgent
// @alice.hanzo/profile/device/myPhone
//
// Valid Examples - Lux Network (Chain ID: 96369 mainnet, 96368 testnet)
// @bob.lux (mainnet)
// @bob.lux-testnet (testnet)
// @bob.luxtest (testnet alias)
// @bob.lux/profile
//
// Valid Examples - Zoo Network (Chain ID: 200200 mainnet, 200201 testnet)
// @carol.zoo (mainnet)
// @carol.zoo-testnet (testnet)
// @carol.zootest (testnet alias)
// @carol.zoo/profile
//
// Valid Examples - Ethereum Testnets
// @dave.sepolia (Sepolia testnet)
// @eve.arbitrum-sepolia (Arbitrum Sepolia testnet)
//
// Auto-correction
// @zeekay (auto-corrects to @zeekay.hanzo)
...
impl HanzoName {
    // Define a list of valid network endings
    // Hanzo (Chain ID: 36963 mainnet, 36962 testnet)
    // Lux (Chain ID: 96369 mainnet, 96368 testnet)
    // Zoo (Chain ID: 200200 mainnet, 200201 testnet)
    const VALID_ENDINGS: [&'static str; 18] = [
        // Hanzo networks
        ".hanzo",
        ".hanzo-mainnet",
        ".hanzo-testnet",
        ".hanzotest",
        ".ai",              // .ai alias for Hanzo mainnet
        // Lux networks
        ".lux",
        ".lux-mainnet",
        ".lux-testnet",
        ...
        ".sepolia",
        ".arbitrum-sepolia",
        // Legacy aliases (deprecated, use specific network names)
        ".sep-hanzo",
        ".arb-sep-hanzo",
        ".sepolia-hanzo",
    ];
    ...
    pub fn validate_name(raw_name: &str) -> Result<(), &'static str> {
        ...
        if !parts[0].starts_with("@") || !Self::VALID_ENDINGS.iter().any(|&ending| parts[0].ends_with(ending)) {
            ...
            return Err("Node part of the name should start with '@' and end with a valid network identifier (e.g., '.hanzo', '.lux', '.zoo', '.sepolia').");
        }

        let node_name_regex = r"^@[a-zA-Z0-9\_\.]+(\.hanzo|\.hanzo-mainnet|\.hanzo-testnet|\.hanzotest|\.ai|\.lux|\.lux-mainnet|\.lux-testnet|\.luxtest|\.zoo|\.zoo-mainnet|\.zoo-testnet|\.zootest|\.sepolia|\.arbitrum-sepolia|\.sep-hanzo|\.arb-sep-hanzo|\.sepolia-hanzo)$";
    ...
    pub fn default_testnet_localhost() -> Self {
        HanzoName::new("@localhost.sepolia/main".to_string())
