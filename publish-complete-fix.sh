...
# Complete fix for publishing remaining zoo crates
# This script replaces ALL workspace inheritance including dependencies

set -e
export CARGO_REGISTRY_TOKEN="cio2uOphUOkgWyPZZQaYFUvFKwpVUBGbyHY"

echo "🚀 Publishing remaining zoo crates with COMPLETE workspace fix..."
echo "=========================================================="
echo ""
...
publish_crate_complete() {
  ...
  # Step 2: Replace ALL dependency workspace inheritance
  # These are the common workspace dependencies from the root Cargo.toml
  ...
  sed -i '' 's/serde = { workspace = true }/serde = "1.0.219"/' Cargo.toml
...
for crate in "${CRATES[@]}"; do
  if [ -d "zoo-libs/$crate" ]; then
    publish_crate_complete "$crate"
  else
    echo "⚠️ Directory zoo-libs/$crate not found, skipping..."
  fi
done
