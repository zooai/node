use super::network_handlers::{handle_based_on_message_content_and_encryption, verify_message_signature};
use crate::managers::{identity_manager::IdentityManagerTrait, IdentityManager};
use crate::network::{
...
impl HanzoMessageHandler {
    ...
    pub async fn handle_message_internode(
        ...
        let maybe_db = self.db.upgrade().ok_or("Database reference upgrade failed")?;
        ...
        let (encryption_sender_identity, encryption_public_key, actual_sender_name) =
            if !message.external_metadata.intra_sender.is_empty() {
                // Message came through relay - use intra_sender for encryption/decryption
                println!(
                    "🔄 Message came through relay - original sender: {}, relay: {}",
                    message.external_metadata.intra_sender, message.external_metadata.sender
                );
                ...
                verify_message_signature(sender_identity.node_signature_public_key, message).map_err(|e| {
                    hanzo_log(
                        HanzoLogOption::Node,
                        HanzoLogLevel::Error,
                ...
                eprintln!(
                    "🔍 Debug: message.external_metadata.other = {}",
                    message.external_metadata.other
                );
                if !message.external_metadata.other.is_empty() {
                    match string_to_encryption_public_key(&message.external_metadata.other) {
                        Ok(encryption_pk) => {
                            println!(
                                "✅ Using encryption public key from 'other' field for original sender: {}",
                                original_sender_node
                            );
                        ...
                        Err(e) => {
                            println!(
                            "⚠️  Failed to parse encryption public key from 'other' field, using relay's identity: {}",
                            e
                        );
                            (
                        ...
                        .await
                    {
                        Ok(original_identity) => {
                            println!(
                                "✅ Found original sender identity for decryption: {}",
                                original_sender_node
                            );
                            (
                                original_identity.clone(),
                                original_identity.node_encryption_public_key,
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "⚠️  Could not find original sender identity {}, falling back to relay identity: {}",
                                original_sender_node, e
                            );
                            (
                                sender_identity.clone(),
                                sender_identity.node_encryption_public_key,
        ...
        hanzo_log(
            HanzoLogOption::Node,
            HanzoLogLevel::Debug,
            &format!("{} > Sender Profile Name: {:?}", self.local_addr, actual_sender_name),
        );
        hanzo_log(
            HanzoLogOption::Node,
            HanzoLogLevel::Debug,
            &format!(
                ...
                self.local_addr, encryption_sender_identity
            ),
        );
        hanzo_log(
            HanzoLogOption::Node,
        ...
        let proxy_connection_info = self
            .proxy_connection_info
            .upgrade()
