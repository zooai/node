...
impl Node {
    pub async fn v2_api_add_custom_prompt(
        ...
        // Save the new prompt to the LanceHanzoDb
    ...
    pub async fn v2_api_delete_custom_prompt(
        ...
        match prompt {
            Ok(prompts) if prompts.len() == 1 => {
                ...
                // Delete the prompt from the LanceHanzoDb
    ...
    pub async fn v2_api_get_all_custom_prompts(
        ...
        // Get all prompts from the LanceHanzoDb
    ...
    pub async fn v2_api_get_custom_prompt(
        ...
        // Get the prompt from the LanceHanzoDb with optional filters
    ...
    pub async fn v2_api_update_custom_prompt(
        ...
        // Update the prompt in the LanceHanzoDb
        match db.update_prompt(&prompt).await {
            ...
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                    error: "Internal Server Error".to_string(),
                    message: format!("Failed to update custom prompt in LanceHanzoDb: {}", err),
                };
                let _ = res.send(Err(api_error)).await;
                Ok(())
            }
