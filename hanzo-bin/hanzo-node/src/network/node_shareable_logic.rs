...
pub async fn validate_message_main_logic(
        ...
        if is_body_encrypted {
            ...
            */
            ...
            if sender_encryption_pk.is_some() {
                ...
                let sender_name = HanzoName::from_hanzo_message_using_sender_and_intra_sender(
    ...
    let sender_name = match HanzoName::from_hanzo_message_using_sender_subidentity(&msg.clone()) {
        Ok(name) => name,
        Err(e) => {
            return Err(APIError {
                code: StatusCode::BAD_REQUEST.as_u16(),
...
pub async fn download_zip_from_url(
    ...
    let request = client
        ...
        .header("X-Hanzo-Signature", signature_hex);
