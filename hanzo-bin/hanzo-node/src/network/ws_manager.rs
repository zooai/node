...
use hanzo_message_primitives::schemas::ws_types::MessageQueue;
use hanzo_message_primitives::schemas::ws_types::MessageType;
...
use hanzo_message_primitives::schemas::ws_types::WebSocketManagerError;
...
pub struct WebSocketManager {
    ...
    // TODO: maybe the first string should be a HanzoName? or at least a hanzo name string
    subscriptions: HashMap<String, HashMap<String, bool>>,
    shared_keys: HashMap<String, String>,
    hanzo_db: Weak<SqliteManager>,
    node_name: HanzoName,
...
impl WebSocketManager {
    ...
    pub async fn has_access(&self, hanzo_name: HanzoName, topic: WSTopic, subtopic: Option<String>) -> bool {
        match topic {
            WSTopic::Inbox => {
                let subtopic = subtopic.unwrap_or_default();
                let inbox_name = InboxName::new(subtopic.clone()).unwrap(); // TODO: handle error
                let sender_identity = match self.get_sender_identity(hanzo_name.clone()).await {
                    Ok(identity) => identity,
                    Err(_) => return false,
                };
                let db_arc = self.hanzo_db.upgrade().ok_or("Failed to upgrade hanzo_db").unwrap();
                        ...
                        if value {
                            hanzo_log(
                                ...
                                format!(
                                    ...
                                    inbox_name, hanzo_name.full_name
                                )
                                .as_str(),
                            );
                        } else {
    ...
    pub async fn manage_connections(
        ...
        let sender_hanzo_name = HanzoName::new(format!("{}/main", node_name)).unwrap();
        let hanzo_profile_name = sender_hanzo_name.full_name.clone();
        ...
        for subscription in ws_message.subscriptions.iter() {
            ...
            {
                eprintln!(
                    "Access denied for hanzo_name: {} on topic: {:?} and subtopic: {:?}",
                    sender_hanzo_name, subscription.topic, subscription.subtopic
                );
                // TODO: should we send a HanzoMessage with an error inside back?
        ...
        self.connections.insert(hanzo_profile_name.clone(), connection);

        if let Some(key) = shared_key {
            self.shared_keys.insert(hanzo_profile_name.clone(), key);
        }
        // Note: uncomment this enforce having a shared encryption key
        // else if !self.shared_keys.contains_key(&hanzo_profile_name) {
        //     return Err(WebSocketManagerError::MissingSharedKey(format!(
        //         "Missing shared key for hanzo_name: {}",
        //         hanzo_profile_name
        //     )));
        // }
        ...
        let subscriptions_to_remove: Vec<(WSTopic, Option<String>)> = ws_message
            ...
            .map(|s| (s.topic.clone(), s.subtopic.clone()))
            .collect();
        self.update_subscriptions(&hanzo_profile_name, subscriptions_to_add, subscriptions_to_remove)
            .await;

        hanzo_log(
            HanzoLogOption::WsAPI,
            HanzoLogLevel::Info,
            format!("Successfully added connection for hanzo_name: main").as_str(),
        );
    ...
    pub async fn update_subscriptions(
        ...
        let profile_subscriptions = self.subscriptions.entry(hanzo_name.to_string()).or_default();
    ...
    pub async fn handle_update(
        ...
        for (id, connection) in self.connections.iter() {
                ...
                || (is_subscribed_to_sheets && topic == WSTopic::Sheet)
            {
                ...
                if is_subscribed_to_smart_inboxes {
                    match HanzoName::new(id.clone()) {
                        Ok(hanzo_name) => {
                            let hanzo_name_clone = hanzo_name.clone();
                            if !self
                                .has_access(hanzo_name_clone, topic.clone(), Some(subtopic.clone()))
                                .await
                            {
                                continue;
                            }
                            eprintln!(
                                "Access granted for hanzo_name: {} on topic: {:?} and subtopic: {:?}",
                                hanzo_name, topic, subtopic
                            );
                        }
                        Err(e) => {
                            hanzo_log(
                                HanzoLogOption::WsAPI,
                                HanzoLogLevel::Error,
                                format!("Failed to create HanzoName for id {}: {}", id, e).as_str(),
                            );
                            continue;
                        }
                    }
    ...
    pub async fn get_sender_identity(&self, hanzo_name: HanzoName) -> Result<Identity, WebSocketManagerError> {
        let identity_manager_lock = self.identity_manager_trait.lock().await;
        match identity_manager_lock.find_by_identity_name(hanzo_name.clone()) {
            Some(identity) => Ok(identity.clone()),
            None => {
                hanzo_log(
                    HanzoLogOption::WsAPI,
                    HanzoLogLevel::Error,
                    format!("No identity found for hanzo_name: {}", hanzo_name).as_str(),
                );
                Err(WebSocketManagerError::UserValidationFailed(format!(
                    "No identity found for hanzo_name: {}",
                    hanzo_name
                )))
            }
        }
