...
impl JobCallbackManager {
    ...
    pub async fn handle_implementation_check_callback(
        ...
        // Create a HanzoMessage that looks it came from the user
        let hanzo_message = HanzoMessageBuilder::job_message_unencrypted(
            job_id.to_string(),
            error_message,
            vec![],
            "".to_string(),
        ...
        if let Some(job_manager) = job_manager {
            let _result = Node::internal_job_message(job_manager, hanzo_message.clone(), true).await;
        } else {
