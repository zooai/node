...
pub async fn generate_execution_environment(
    ...
    envs.insert("X_HANZO_TOOL_ID".to_string(), tool_id.clone());
    envs.insert("X_HANZO_APP_ID".to_string(), app_id.clone());
    if let Some(agent_id) = agent_id {
        envs.insert("X_HANZO_AGENT_ID".to_string(), agent_id.clone());
    }
    envs.insert("X_HANZO_INSTANCE_ID".to_string(), instance_id.clone());
    envs.insert("X_HANZO_LLM_PROVIDER".to_string(), llm_provider);

    check_oauth(oauth, &tool_router_key)?;
    let oauth = handle_oauth(oauth, &db, app_id.clone(), tool_id.clone(), tool_router_key.clone()).await?;

    envs.insert("HANZO_OAUTH".to_string(), oauth.to_string());
    ...
    if let Some(oauth_configs) = oauth {
        for oauth in oauth_configs {
            ...
            if !missing_fields.is_empty() {
                let fix_redirect_url = format!("hanzo://config?tool={}", urlencoding::encode(tool_router_key));
...
mod tests {
    ...
    fn test_check_tool_missing_required_config() {
        ...
        if let Err(ToolError::MissingConfigError(msg)) = result {
            ...
            assert!(msg.contains("hanzo://config?tool=test%2Ftool"));
        } else {
    ...
    fn test_check_oauth_missing_multiple_fields() {
        ...
        if let Err(ToolError::MissingConfigError(msg)) = result {
            ...
            assert!(msg.contains("hanzo://config?tool=test_oauth"));
        } else {
            panic!("Expected MissingConfigError");
        }
    }
