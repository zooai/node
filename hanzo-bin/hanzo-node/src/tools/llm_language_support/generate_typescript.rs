 ...
 * }
 */
export async function hanzoDownloadPages(input: {urls: any[]}): Promise<{
    markdowns: string[];
}> {
    ...
    const data = {
        ...
        tool_type: 'deno',
        llm_provider: `${Deno.env.get('X_HANZO_LLM_PROVIDER')}`,
        parameters: input,
    };
    try {
        const response = await axios.post(_url, data, {
            headers: {
                ...
                'x-hanzo-tool-id': `${Deno.env.get('X_HANZO_TOOL_ID')}`,
                'x-hanzo-app-id': `${Deno.env.get('X_HANZO_APP_ID')}`,
                'x-hanzo-llm-provider': `${Deno.env.get('X_HANZO_LLM_PROVIDER')}`,
                'x-hanzo-agent-id': `${Deno.env.get('X_HANZO_AGENT_ID')}`
            }
        });
...
async function query_{}(query: string, params?: any[]){}",
            ...
            if generate_dts {
                ...
                " {\n    return hanzoSqliteQueryExecutor({query, params})\n}"
            }
        ));
    }
