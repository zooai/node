...
import os
import requests
async def hanzo_download_pages(input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    data = {
        ...
        'tool_type': 'deno',
        'llm_provider': os.environ.get('X_HANZO_LLM_PROVIDER', ''),
        'parameters': input
    }
    try:
        response = requests.post(
            ...
            headers={
                ...
                'x-hanzo-tool-id': os.environ.get('X_HANZO_TOOL_ID', ''),
                'x-hanzo-app-id': os.environ.get('X_HANZO_APP_ID', ''),
                'x-hanzo-llm-provider': os.environ.get('X_HANZO_LLM_PROVIDER', ''),
                'x-hanzo-agent-id': os.environ.get('X_HANZO_AGENT_ID', '')
            }
        )
...
pub fn create_function_name_set(tool: &HanzoToolHeader) -> String {
...
fn generate_parameters(tool: &HanzoToolHeader) -> String {
...
fn generate_docstring(tool: &HanzoToolHeader, tool_result: ToolResult, indent: &str) -> String {
