...
fn get_model_context_size(
    ...
    let hanzo_name = HanzoName::from_node_and_profile_names(node_name.get_node_name_string(), "main".to_string())
        .map_err(|_| ToolError::ExecutionError("Failed to create hanzo name".to_string()))?;
    let llm_provider = db
        .get_llm_provider(&llm_provider, &hanzo_name)
