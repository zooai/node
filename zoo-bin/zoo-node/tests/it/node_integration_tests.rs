...
fn subidentity_registration() {
    ...
    let e = rt.block_on(async {
        ...
        let relay_identity_name = Some("@@libp2p_relayer.sep-zoo".to_string());
        ...
        let interactions_handler = tokio::spawn(async move {
            ...
            // Send message from Node 2 subidentity to Node 1
            {
                ...
                let unchanged_message = ZooMessageBuilder::new(
                ...
                assert_eq!(
                    ZooMessage::is_body_currently_encrypted(&message_to_check.clone()),
                    false,
                ...
                assert_eq!(
                    ZooMessage::is_content_currently_encrypted(&message_to_check.clone()),
                    true,
            ...
            // Send message from Node 1 subidentity to Node 2 subidentity
            {
...

                {
                    ...
                    for _ in 0..30 {
                        ...
                        if ZooMessage::is_body_currently_encrypted(&message_to_check.clone()) {
                        ...
                        if !ZooMessage::is_content_currently_encrypted(&message_to_check.clone()) {
                            eprintln!(
...
fn test_relay_server_communication() {
    ...
    let e: Result<(), tokio::task::JoinError> = rt.block_on(async {
        ...
        let messaging_test = tokio::spawn(async move {
            ...
            let message_1to2 = ZooMessageBuilder::new(
                profile1_encryption_sk.clone(),
                clone_signature_secret_key(&profile1_sk),
            ...
            let message_2to1 = ZooMessageBuilder::new(
                profile2_encryption_sk.clone(),
                clone_signature_secret_key(&profile2_sk),
            ...
            if !node1_messages.is_empty() {
                ...
                for (i, message) in node1_messages.iter().enumerate() {
                    ...
                    if ZooMessage::is_content_currently_encrypted(message) {
...
fn test_localhost_relay_server_communication() {
    ...
    let e: Result<(), tokio::task::JoinError> = rt.block_on(async {
        let node1_identity_name = "@@node1_with_libp2p_relayer.sep-zoo";
        let node2_identity_name = "@@localhost.sep-zoo";
        let relay_identity_name = Some("@@libp2p_relayer.sep-zoo".to_string());
...
fn test_send_message_to_localhost_node() {
    ...
    let e: Result<(), tokio::task::JoinError> = rt.block_on(async {
        ...
        let node2_peer_id = zoo_node::network::libp2p_manager::verifying_key_to_peer_id(node2_identity_pk).unwrap();
