...
use zoo_message_primitives::zoo_message::zoo_message::{MessageBody, ZooMessage};
...
async fn test_insert_messages_with_simple_tree_structure_and_root() {
    ...
    let paginated_messages_inbox = zoo_db
    ...
    let root_message_parent_hash = zoo_db
        .get_parent_message_hash(
            &inbox_name_value,
...
async fn db_inbox() {
    ...
    let last_messages_all = zoo_db.get_last_messages_from_all(10).unwrap();
    ...
    let last_unread_messages_inbox = zoo_db
    ...
    let last_unread_messages_inbox_page2 = zoo_db
