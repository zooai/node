...
mod tests {
    use zoo_message_primitives::schemas::{
        llm_providers::serialized_llm_provider::{LLMProviderInterface, OpenAI, SerializedLLMProvider}, zoo_name::ZooName
    };
    ...
    async fn test_get_agent_profiles_and_toolkits() {
        ...
        let identity = ZooName::new("@@alice.zoo/profileName/agent/test_name".to_string()).unwrap();
        let profile = identity.extract_profile().unwrap();
