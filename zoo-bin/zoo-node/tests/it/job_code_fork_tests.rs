...

            {
                ...
                let tool_playground = ToolPlayground {
                    metadata: ToolPlaygroundMetadata {
                        ...
                        description: "A Zoo Node running in the directory of its home path.".to_string(),
                    ...
                    job_id: job_id.clone(),
                    job_id_history: vec![],
                    code: "import { getHomePath } from './zoo-local-support.ts';\n\ntype CONFIG = {};\ntype INPUTS = {};\ntype OUTPUT = {};\n\nexport async function run(config: CONFIG, inputs: INPUTS): Promise<OUTPUT> {\n  const homeDir = await getHomePath();\n  console.log(`The Zoo Node is running in the directory: ${homeDir}`);\n  return {};\n}".to_string(),
                    language: CodeLanguage::Typescript,
                    assets: None,
                };
...

            {
                ...
                node1_commands_sender
                    .send(NodeCommand::V2ApiDuplicateTool {
                        bearer: node1_api_key.clone(),
                        tool_key_path: format!("local:::__localhost_sep_zoo:::hello_world"),
                        res: res_sender,
                    })
                    .await
                    .unwrap();
