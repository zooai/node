...
use zoo_message_primitives::schemas::invoices::{Invoice, InvoiceStatusEnum};
use zoo_message_primitives::schemas::zoo_name::ZooName;
use zoo_message_primitives::schemas::zoo_tool_offering::{
    ZooToolOffering, ToolPrice, UsageType, UsageTypeInquiry
};
use zoo_message_primitives::schemas::wallet_complementary::{WalletRole, WalletSource};
use zoo_message_primitives::schemas::x402_types::{self, Network, PaymentRequirements};
...
use zoo_message_primitives::zoo_utils::utils::hash_string;
use zoo_node::network::Node;
...
use zoo_tools_primitives::tools::zoo_tool::{ZooTool, ZooToolHeader, ZooToolWithAssets};
        ...
        "content": [
            {
                ...
                "sql_queries": [],
                "sql_tables": [],
                "tool_router_key": "local:::__localhost_sep_zoo:::echo_function",
                "tool_set": "",
                "tools": [],
                "version": "1.0.0"
            },
...
fn micropayment_localhost_flow_test() {
    ...
    let e = rt.block_on(async {
        let node1_identity_name = "@@node1_with_libp2p_relayer.sep-zoo"; // The one that is running the tool
        let node2_identity_name = "@@localhost.sep-zoo";  // The one that is requesting the invoice / tool
        let node1_profile_name = "main";
        let node1_device_name = "node1_device";
        let node2_profile_name = "main";
        let relay_identity_name = "@@libp2p_relayer.sep-zoo";
        ...
        let interactions_handler = tokio::spawn(async move {
            ...
            let test_network_tool_name = "__node1_with_libp2p_relayer_sep_zoo:::__localhost_sep_zoo:::echo_function";
            let test_local_tool_key_name = "local:::__localhost_sep_zoo:::echo_function";

            let zoo_tool_offering = ZooToolOffering {
            ...
            let zoo_tool_header = ZooToolHeader {
                ...
                author: "@@localhost.sep-zoo".to_string(),
                version: "1.0.0".to_string(),
                enabled: true,
                config: Some(vec![]),
                usage_type: None,
                tool_offering: Some(zoo_tool_offering.clone()),
            };

            {
                ...
                let zoo_tool = ZooTool::Deno(deno_tool, is_active);

                let (sender, receiver) = async_channel::bounded(1);
                node1_commands_sender
                    .send(NodeCommand::V2ApiAddZooTool {
                        bearer: api_v2_key.to_string(),
                        zoo_tool: ZooToolWithAssets {
                            tool: zoo_tool,
                            assets: None,
                        },
                        res: sender,
                ...
                eprintln!("\nAll available Zoo tools (name, tool_router_key):");
                ...
                // // Retrieve the zoo_tool from node1
                // let (sender, receiver) = async_channel::bounded(1);
                // node1_commands_sender
                //     .send(NodeCommand::V2ApiGetZooTool {
                //         bearer: api_v2_key.to_string(),
                //         payload: "__node1_test_sep_zoo:::__official_zoo:::echo_function".to_string(),
                ...
                node1_commands_sender
                    .send(NodeCommand::V2ApiSetToolOffering {
                        bearer: api_v2_key.to_string(),
                        tool_offering: zoo_tool_offering.clone(),
                        res: sender,
                    })
                    .await
                    .unwrap();
                ...
                if let Err(api_error) = &resp {
                    // Assuming api_error is of a type that has a `message` field (e.g., APIError from zoo_http_api)
            ...
            }
            {
                ...
                let expected_response = vec![zoo_tool_header.clone()];
            ...
            }
            {
                ...
                // Convert ZooToolHeader to ZooTool
                // Manually create NetworkTool
                let provider = ZooName::new(node1_identity_name.to_string()).unwrap();
                let tool_router_key = ToolRouterKey::new(
                    provider.to_string(),
                    zoo_tool_header.author.clone(),
                    zoo_tool_header.name.clone(),
                    None,
                );
                let network_tool = NetworkTool {
                    name: zoo_tool_header.name.clone(),
                    author: zoo_tool_header.author.clone(),
                    description: zoo_tool_header.description.clone(),
                    version: zoo_tool_header.version.clone(),
                    mcp_enabled: zoo_tool_header.mcp_enabled.clone(),
                    provider,
                    tool_router_key: tool_router_key.to_string_without_version(),
                    usage_type: zoo_tool_offering.usage_type.clone(),
                    activated: zoo_tool_header.enabled,
                    config: zoo_tool_header.config.clone().unwrap_or_default(),
                ...
                let zoo_tool = ZooTool::Network(network_tool, true);

                let serialized_zoo_tool = serde_json::to_value(&zoo_tool).unwrap();
                eprintln!("serialized_zoo_tool: {:?}", serialized_zoo_tool);

                // Add the ZooTool to node2
                let (sender, receiver) = async_channel::bounded(1);
                node2_commands_sender
                    .send(NodeCommand::V2ApiAddZooTool {
                        bearer: api_v2_key.to_string(),
                    ...
                    .unwrap();
                let resp = receiver.recv().await.unwrap();
                eprintln!("resp add zoo tool to node2: {:?}", resp);
                ...
                node2_commands_sender
                    ...
                    .unwrap();
                let resp = receiver.recv().await.unwrap();
                eprintln!("\nAll available Zoo tools in node2 (name, tool_router_key):");
            ...
            }
            {
                ...
                // Search for the tool using the V2ApiSearchZooTool command
                let (sender, receiver) = async_channel::bounded(1);
                node2_commands_sender
                    ...
                    .unwrap();
                let resp = receiver.recv().await.unwrap();
                eprintln!("resp search zoo tool: {:?}", resp);
            ...
            }
            {
                ...
                node2_commands_sender
                    .send(NodeCommand::V2ApiPayInvoice {
                        ...
                        data_for_tool: serde_json::json!({ "message": "Hello, Zoo!" }),
                        res: sender,
                    })
                    .await
                    .unwrap();
