...
use zoo_message_primitives::zoo_utils::encryption::EncryptionMethod;
use zoo_message_primitives::zoo_utils::job_scope::MinimalJobScope;
use zoo_message_primitives::zoo_utils::zoo_message_builder::ZooMessageBuilder;
...
fn generate_message_with_text(
    ...
    ZooMessageBuilder::new(my_encryption_secret_key, my_signature_secret_key, receiver_public_key)
...
mod tests {
    ...
    use zoo_message_primitives::{
        schemas::{
            identity::{StandardIdentity, StandardIdentityType}, inbox_name::InboxName, inbox_permission::InboxPermission, job::ForkedJob, zoo_name::ZooName, subprompts::SubPrompt
        }, zoo_message::zoo_message_schemas::{IdentityPermissions, JobMessage}, zoo_utils::{
            encryption::unsafe_deterministic_encryption_keypair, job_scope::MinimalJobScope, zoo_message_builder::ZooMessageBuilder, signatures::{clone_signature_secret_key, unsafe_deterministic_signature_keypair}
        }
    };
    ...
    async fn test_create_new_job() {
        ...
        // Create a new job
        let _ = create_new_job(&zoo_db, job_id.clone(), agent_id.clone(), scope).await;

        // Retrieve all jobs
        let jobs = zoo_db.get_all_jobs().unwrap();
        ...
        let job = zoo_db.get_job(&job_id).unwrap();
    ...
    async fn test_get_agent_jobs() {
        ...
        for i in 1..=5 {
            ...
            let scope = MinimalJobScope::default();
            let _ = create_new_job(&zoo_db, job_id, agent_id.clone(), scope).await;
        }

        eprintln!("agent_id: {}", agent_id.clone());

        // Get all jobs for the agent
        let jobs = zoo_db.get_agent_jobs(agent_id.clone()).unwrap();
    ...
    async fn test_change_job_agent() {
        ...
        let _ = create_new_job(&zoo_db, job_id.clone(), initial_agent_id.clone(), scope).await;

        // Change the agent of the job
        zoo_db.change_job_llm_provider(&job_id, &new_agent_id).unwrap();
        ...
        let new_agent_jobs = zoo_db.get_agent_jobs(new_agent_id.clone()).unwrap();
        ...
        let initial_agent_jobs = zoo_db.get_agent_jobs(initial_agent_id.clone()).unwrap();
    ...
    async fn test_update_job_to_finished() {
        ...
        // let inbox_name =
        //     InboxName::new("inbox::@@node1.zoo/subidentity::@@node2.zoo/subidentity2::true".to_string())
        //         .unwrap();
        let scope = MinimalJobScope::default();
        let db = setup_test_db();
        let zoo_db = Arc::new(db);
        ...
        zoo_db.update_job_to_finished(&job_id.clone()).unwrap();

        // Retrieve the job and check that is_finished is set to true
        let job = zoo_db.get_job(&job_id.clone()).unwrap();
        assert!(job.is_finished);
    }
    ...
    async fn test_get_agent_jobs_none_exist() {
        ...
        let jobs_result = zoo_db.get_agent_jobs(agent_id.clone());
    ...
    async fn test_job_inbox_empty() {
        ...
        assert!(zoo_db.is_job_inbox_empty(&job_id).unwrap());

        let (placeholder_signature_sk, _) = unsafe_deterministic_signature_keypair(0);
        let zoo_message = ZooMessageBuilder::job_message_from_llm_provider(
            job_id.to_string(),
            "something".to_string(),
            None,
            vec![],
        ...
        let _ = zoo_db
            .add_message_to_job_inbox(&job_id.clone(), &zoo_message, None, None)
            .await;
    ...
    async fn test_job_inbox_tree_structure() {
        ...
        for i in 1..=4 {
            ...
            // Add a message to the job
            let _ = zoo_db
                .add_message_to_job_inbox(&job_id.clone(), &zoo_message, parent_hash.clone(), None)
                .await;

            // Update the parent message according to the tree structure
            if i == 1 {
                parent_message_hash = Some(zoo_message.calculate_message_hash_for_pagination());
            } else if i == 2 {
                parent_message_hash_2 = Some(zoo_message.calculate_message_hash_for_pagination());
            }
        ...
        let last_messages_inbox = zoo_db
    ...
    async fn test_insert_steps_with_simple_tree_structure() {
        let node1_identity_name = "@@node1.zoo";
        ...
        let _ = create_new_job(&zoo_db, job_id.to_string(), agent_id.clone(), scope).await;
        ...
        for i in 1..=4 {
            ...
            // Insert the ZooMessage into the database
            zoo_db
        ...
        let step_history = zoo_db.get_step_history(job_id, true).unwrap().unwrap();

        let step_history_content: Vec<String> = step_history
            .iter()
            .map(|zoo_message| {
                eprintln!("Zoo message: {:?}", zoo_message);
                let prompt = zoo_message.to_prompt();
    ...
    async fn test_add_forked_job() {
        ...
        // Create a new job
        let _ = create_new_job(&zoo_db, job_id.clone(), agent_id.clone(), scope.clone()).await;
        ...
        let forked_message2_id = last_messages_inbox
            ...
            .unwrap()
            .calculate_message_hash_for_pagination();
        let _ = create_new_job(&zoo_db, forked_job1_id.clone(), agent_id.clone(), scope.clone()).await;
        let _ = create_new_job(&zoo_db, forked_job2_id.clone(), agent_id.clone(), scope).await;
        ...
        match zoo_db.add_forked_job(&job_id, forked_job1) {
            Ok(_) => {}
            Err(e) => panic!("Error adding forked job: {:?}", e),
        }
        match zoo_db.add_forked_job(&job_id, forked_job2) {
            Ok(_) => {}
            Err(e) => panic!("Error adding forked job: {:?}", e),
        }
    ...
    async fn test_remove_job() {
        ...
        // Create new jobs
        let _ = create_new_job(&zoo_db, job1_id.clone(), agent_id.clone(), scope.clone()).await;
        let _ = create_new_job(&zoo_db, job2_id.clone(), agent_id.clone(), scope).await;
        ...
        let node1_profile_identity = StandardIdentity::new(
            ZooName::from_node_and_profile_names(
                node1_identity_name.to_string(),
                node1_subidentity_name.to_string(),
            )
            .unwrap(),
        ...
        let _ = zoo_db.insert_profile(node1_profile_identity.clone());
        ...
        zoo_db.remove_job(&job1_id).unwrap();

        // Check if the job is removed
        match zoo_db.get_job(&job1_id) {
