 ...
 * }
 */
export async function zooDownloadPages(input: {urls: any[]}): Promise<{
    markdowns: string[];
}> {

    const _url = `${Deno.env.get('ZOO_NODE_LOCATION')}/v1/node/tool_execution`;
    const data = {
        tool_router_key: 'local:::zoo_tool_download_pages:::zoo__download_pages',
        tool_type: 'deno',
        llm_provider: `${Deno.env.get('X_ZOO_LLM_PROVIDER')}`,
        parameters: input,
    };
    try {
        const response = await axios.post(_url, data, {
            headers: {
                ...
                'x-zoo-tool-id': `${Deno.env.get('X_ZOO_TOOL_ID')}`,
                'x-zoo-app-id': `${Deno.env.get('X_ZOO_APP_ID')}`,
                'x-zoo-llm-provider': `${Deno.env.get('X_ZOO_LLM_PROVIDER')}`,
                'x-zoo-agent-id': `${Deno.env.get('X_ZOO_AGENT_ID')}`
            }
        });
...
async function query_{}(query: string, params?: any[]){}",
            ...
            if generate_dts {
                ...
                " {\n    return zooSqliteQueryExecutor({query, params})\n}"
            }
        ));
    }
