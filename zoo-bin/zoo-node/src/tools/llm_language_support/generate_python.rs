...
import os
import requests
async def zoo_download_pages(input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    _url = os.environ.get('ZOO_NODE_LOCATION', '') + '/v1/node/tool_execution'
    data = {
        'tool_router_key': 'local:::zoo_tool_download_pages:::zoo__download_pages',
        'tool_type': 'deno',
        'llm_provider': os.environ.get('X_ZOO_LLM_PROVIDER', ''),
        'parameters': input
    }
    try:
        response = requests.post(
            ...
            headers={
                ...
                'x-zoo-tool-id': os.environ.get('X_ZOO_TOOL_ID', ''),
                'x-zoo-app-id': os.environ.get('X_ZOO_APP_ID', ''),
                'x-zoo-llm-provider': os.environ.get('X_ZOO_LLM_PROVIDER', ''),
                'x-zoo-agent-id': os.environ.get('X_ZOO_AGENT_ID', '')
            }
        )
...
pub fn create_function_name_set(tool: &ZooToolHeader) -> String {
...
fn generate_parameters(tool: &ZooToolHeader) -> String {
...
fn generate_docstring(tool: &ZooToolHeader, tool_result: ToolResult, indent: &str) -> String {
...
"#,
        ...
        if generate_pyi {
            ...
            python_output.push_str("    return zoo_sqlite_query_executor({'query': query, 'params': params})\n");
        }
    }
...
mod tests {
    use super::*;
    use serde_json::json;
    use zoo_tools_primitives::tools::parameters::{Parameters, Property};

    #[test]
    fn test_generate_python_definition() {
        // Create a test tool header
        let tool = ZooToolHeader {
            ...
            output_arg: zoo_tools_primitives::tools::tool_output_arg::ToolOutputArg { json: "{}".to_string() },
            config: None,
            usage_type: None,
            tool_offering: None,
        };
