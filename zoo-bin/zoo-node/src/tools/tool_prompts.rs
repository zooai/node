...
pub async fn generate_code_prompt(
    ...
    match language {
        CodeLanguage::Typescript => {
            let zoo_sqlite_query_executor = "zooSqliteQueryExecutor";
            let is_memory_required_message = if is_memory_required {
                format!("* If permanent memory is required, write to disk, store, sql always prioritize using {zoo_sqlite_query_executor}.")
            } else {
                "".to_string()
            };
...
<agent_deno_libraries>
  * Prefer libraries in the following order:
    1. A function provided by './zoo-local-tools.ts' that resolves correctly the requirement.
...
<agent_libraries>
  * You may use any of the following functions if they are relevant and a good match for the task.
  * Import them with the format: `from zoo_local_tools import xx`
...
* Prefer libraries in the following order:
  1. A function provided by './zoo_local_tools.py' that resolves correctly the requirement.
...
pub async fn tool_metadata_implementation_prompt(
    ...
    let oauth_example = if has_oauth {
        r#"[
      {
        ...
        "redirectUrl": "https://secrets.zoo.ngo/redirect",
          ...
          "items": {{
            ...
            "properties": {{
              ...
              "redirectUrl": {{
                ...
                "default": "https://secrets.zoo.ngo/redirect"
              }},
              "responseType": {{
                "type": "string",
...
<agent_metadata_examples>
  ...
  {{
    ...
    "tools": [
      "local:::__official_zoo:::zoo_sqlite_query_executor",
      "local:::zoo_tool_echo:::zoo_echo"
    ],
    "oauth": {oauth_example},
    "runner": "any",
    "operatingSystem": ["linux", "macos", "windows"],
...
<agent_metadata_rules>
  * If the code uses zooSqliteQueryExecutor then fill the sqlTables and sqlQueries sections, otherwise these sections are empty.
