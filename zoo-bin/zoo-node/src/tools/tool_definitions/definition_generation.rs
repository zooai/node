...
pub fn get_rust_tools() -> Vec<ZooToolHeader> {
...
pub async fn get_all_tools(sqlite_manager: Arc<SqliteManager>) -> Vec<ZooToolHeader> {
...
pub async fn generate_tool_definitions(
    ...
    match language {
        CodeLanguage::Typescript => {
            support_files.insert(
                "zoo-local-support".to_string(),
                generate_file_support_ts(only_headers),
            );
        }
        CodeLanguage::Python => {
            support_files.insert(
                "zoo_local_support".to_string(),
    ...
    let all_tools: Vec<ZooToolHeader> = get_all_tools(sqlite_manager.clone())
        .await
        .into_iter()
        .filter(|tool| {
    ...
    for tool_header in all_tools {
        ...
        let tool_result = match tool_data.clone() {
            ZooTool::Deno(deno_tool, _) => deno_tool.result,
            ZooTool::Python(python_tool, _) => python_tool.result,
            ZooTool::Rust(rust_tool, _) => {
            ...
            ZooTool::Network(network_tool, _) => {
...
",
    ...
    match language {
        CodeLanguage::Typescript => {
            support_files.insert("zoo-local-tools".to_string(), output);
        }
        CodeLanguage::Python => {
            support_files.insert("zoo_local_tools".to_string(), output);
        }
    };
