...
use zoo_message_primitives::schemas::zoo_tools::DynamicToolType;
use zoo_message_primitives::schemas::tool_router_key::ToolRouterKey;
use zoo_sqlite::oauth_manager::OAuthToken;
...
use zoo_tools_primitives::tools::tool_config::{BasicConfig, OAuth, ToolConfig};
use zoo_tools_primitives::tools::tool_types::{OperatingSystem, RunnerType};
...
pub async fn handle_oauth(
    ...
    if let Some(oauth_vec) = oauth {
        for o in oauth_vec {
            ...
            //redirect_uri=https://secrets.zoo.ngo/redirect
...
pub async fn execute_mcp_tool_cmd(
    ...
    let preferences_llm_provider_result = match db.get_preference::<String>("default_llm_provider") {
        Ok(Some(provider_id)) => {
            ...
            let profile_name =
                ZooName::new(format!("{}/main/agent/mcp_default", node_name.get_node_name_string())).unwrap();
...
```"#;
        ...
        let expected = vec![r#"import { getHomePath } from './zoo-local-support.ts';
...
"#
    ...
    fn test_override_tool_config_no_overrides() {
        ...
        let mut agent = Agent {
            ...
            full_identity_name: ZooName::new("test.agent".to_string()).unwrap(),
