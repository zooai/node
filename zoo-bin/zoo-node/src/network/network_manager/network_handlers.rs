...
use zoo_message_primitives::schemas::agent_network_offering::{
...
use zoo_message_primitives::{
    schemas::{
        invoices::{Invoice, InvoiceRequest, InvoiceRequestNetworkError}, zoo_name::ZooName
    }, zoo_message::{
        zoo_message::{MessageBody, MessageData, ZooMessage}, zoo_message_error::ZooMessageError, zoo_message_extension::EncryptionStatus, zoo_message_schemas::MessageSchemaType
    }, zoo_utils::{
        encryption::clone_static_secret_key, zoo_logging::{zoo_log, ZooLogLevel, ZooLogOption}, zoo_message_builder::{ZooMessageBuilder, ZooNameString}, signatures::{clone_signature_secret_key, signature_public_key_to_string}
    }
};
...
pub async fn handle_based_on_message_content_and_encryption(
    ...
    channel: Option<ResponseChannel<ZooMessage>>,
...
pub fn verify_message_signature(sender_signature_pk: VerifyingKey, message: &ZooMessage) -> io::Result<()> {
...
pub async fn handle_default_encryption(
    ...
    match decrypted_message_result {
        Ok(decrypted_message) => {
            ...
            let zoo_message = if decrypted_message.is_content_currently_encrypted() {
            ...
            let message = zoo_message.get_message_content();
            match message {
                Ok(message_content) => {
...
pub async fn handle_network_message_cases(
    ...
    if my_node_full_name.starts_with("@@localhost.") {
        ...
        if let Some(proxy_info) = &*proxy_connection {
            if message.external_metadata.sender == proxy_info.proxy_identity.get_node_name_string() {
                match ZooName::new(message.external_metadata.intra_sender.clone()) {
                    Ok(origin_identity) => {
                    ...
                    Err(e) => {
                        eprintln!("Error creating ZooName: {}", e);
                    }
                }
            }
        }
    ...
    let should_save = matches!(
        ...
        schema_result,
        Err(ZooMessageError::InvalidMessageSchemaType(err)) if err == "Message data is encrypted"
    );
        ...
        Ok(schema) => {
            match schema {
                MessageSchemaType::InvoiceRequest => {
                    let requester = ZooName::from_zoo_message_using_sender_subidentity(&message)?;
                    let receiver = ZooName::from_zoo_message_using_recipient_subidentity(&message)?;
                    zoo_log(
                        ZooLogOption::Network,
                        ZooLogLevel::Info,
                        &format!(
                    ...
                    match serde_json::from_str::<Invoice>(&content) {
                        Ok(invoice) => {
                            ...
                            let trace_info = json!({
                                ...
                                "tool_key": invoice.zoo_offering.tool_key,
...
pub async fn send_ack(
    ...
    receiver: ZooNameString,
    libp2p_event_sender: Option<tokio::sync::mpsc::UnboundedSender<NetworkEvent>>,
    channel: Option<ResponseChannel<ZooMessage>>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let msg = ZooMessageBuilder::ack_message(
...
pub async fn ping_pong(
    ...
    let msg = ZooMessageBuilder::ping_pong_message(
        message.to_owned(),
        clone_static_secret_key(&encryption_secret_key),
        signature_secret_key,
        receiver_public_key,
