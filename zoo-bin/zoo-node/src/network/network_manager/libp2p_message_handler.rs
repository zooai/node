...
use zoo_message_primitives::{
    schemas::{zoo_name::ZooName, ws_types::WSUpdateHandler},
...
/// Message handler that integrates libp2p messages with the existing Zoo network logic
pub struct ZooMessageHandler {
...
impl ZooMessageHandler {
    ...
    pub async fn handle_message_internode(
        ...
        let (encryption_sender_identity, encryption_public_key, actual_sender_name) = if !message.external_metadata.intra_sender.is_empty() {
            ...
            let original_sender_node = ZooName::new(original_sender_name.clone())
            ...
            let sender_profile_name_string = ZooName::from_zoo_message_only_using_sender_node_name(message)
