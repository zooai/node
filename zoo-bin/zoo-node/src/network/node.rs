...
use super::network_manager::libp2p_message_handler::ZooMessageHandler;
...
impl Node {
    ...
    pub async fn new(
        ...
        match ZooName::new(node_name.to_string().clone()) {
            Ok(_) => (),
        ...
        let node_name = ZooName::new(node_name).unwrap();
        {
        ...
        let proxy_connection_info: Option<ProxyConnectionInfo> = proxy_identity.and_then(|proxy_identity| {
            ZooName::new(proxy_identity.clone())
                .inspect_err(|_| {
                    zoo_log(
                        ZooLogOption::Node,
                        ZooLogLevel::Error,
    ...
    pub async fn start(&mut self) -> Result<(), NodeError> {
        ...
        // Initialize LibP2P networking
        {
            ...
            let message_handler = ZooMessageHandler::new(
            ...
            match LibP2PManager::new(self.node_name.to_string(), self.identity_secret_key.clone(), listen_port, message_handler, relay_address).await {
                ...
                Err(e) => {
                    zoo_log( // we'll show an error but not stop node initialization.
                        ZooLogOption::Network,
                        ZooLogLevel::Error,
    ...
    // Static function to get the address from a ZooName identity
    ...
    pub async fn save_to_db(
        ...
        if is_body_encrypted {
            ...
            if am_i_sender {
                counterpart_identity = ZooName::from_zoo_message_only_using_recipient_node_name(message)
                    .unwrap()
                    .to_string();
            } else {
                counterpart_identity = ZooName::from_zoo_message_only_using_sender_node_name(message)
                    .unwrap()
                    .to_string();
            }
    ...
    pub fn zoo_free_provider_id() -> String {
        "zoo_free_trial".to_string()
    }
}
