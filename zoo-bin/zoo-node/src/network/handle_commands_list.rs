...
impl Node {
    pub async fn handle_command(&self, command: NodeCommand) {
        match command {
            ...
            //     zoo_log(ZooLogOption::Node, ZooLogLevel::Info, "Shutdown command received. Stopping the
            // node.");     // self.db = Arc::new(Mutex::new(ZooDB::new("PLACEHOLDER").expect("Failed to
            ...
            NodeCommand::V2ApiPublishAgent { bearer, agent_id, res } => {
                ...
                let zoo_name = self.node_name.clone();
                let identity_manager_clone = self.identity_manager.clone();
            ...
            NodeCommand::V2ApiExportAgent { bearer, agent_id, res } => {
                ...
                let zoo_name = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_export_agent(db_clone, bearer, zoo_name, node_env, agent_id, res).await;
                });
            }
            ...
            NodeCommand::V2ApiZooBackendGetQuota {
                bearer,
                model_type,
                res,
            } => {
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_check_zoo_backend_quota(db_clone, model_type, bearer, res).await;
                });
            }
            ...
            NodeCommand::V2ApiListAllZooTools { bearer, category, res } => {
                let db_clone = Arc::clone(&self.db);
                let tool_router_clone = self.tool_router.clone();
                let node_name_clone = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_zoo_tools(
                        db_clone,
                        bearer,
                        node_name_clone,
                        category,
            ...
            NodeCommand::V2ApiListAllMcpZooTools { category, res } => {
                let db_clone = Arc::clone(&self.db);
                let tool_router_clone = self.tool_router.clone();
                let node_name_clone = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_mcp_zoo_tools(
                        db_clone,
                        node_name_clone,
                        category,
                        tool_router_clone,
            ...
            NodeCommand::V2ApiListAllNetworkZooTools { bearer, res } => {
                let db_clone = Arc::clone(&self.db);
                let tool_router_clone = self.tool_router.clone();
                let node_name_clone = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_network_zoo_tools(
                        db_clone,
                        bearer,
                        node_name_clone,
                        tool_router_clone,
            ...
            NodeCommand::V2ApiListAllZooToolsVersions { bearer, res } => {
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_list_all_zoo_tools_versions(db_clone, bearer, res).await;
                });
            }
            NodeCommand::V2ApiGetZooToolMetadata {
                bearer,
                tool_router_key,
                res,
            } => {
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_get_zoo_tool_metadata(db_clone, bearer, tool_router_key, res).await;
                });
            }
            ...
            NodeCommand::V2ApiSetZooTool {
                ...
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_set_zoo_tool(db_clone, bearer, tool_key, payload, res).await;
                });
            }
            NodeCommand::V2ApiAddZooTool {
                bearer,
                zoo_tool,
                res,
            } => {
                ...
                let node_env = fetch_node_environment();
                tokio::spawn(async move {
                    let _ = Node::v2_api_add_zoo_tool(db_clone, bearer, node_env, zoo_tool, res).await;
                });
            }
            ...
            NodeCommand::V2ApiGetZooTool {
                ...
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_get_zoo_tool(db_clone, bearer, payload, serialize_config, res).await;
                });
            }
            ...
            NodeCommand::V2ApiExportTool {
                ...
                tokio::spawn(async move {
                    let _ =
                        Node::v2_api_export_tool(db_clone, bearer, zoo_name, node_env, tool_key_path, res).await;
                });
            }
            ...
            NodeCommand::V2ApiResolveZooFileProtocol {
                bearer,
                zoo_file_protocol,
                res,
            } => {
                ...
                let node_name = self.node_name.clone();
                tokio::spawn(async move {
                    let _ = Node::v2_api_resolve_zoo_file_protocol(
                        bearer,
                        db_clone,
                        zoo_file_protocol,
                        node_storage_path,
            ...
            NodeCommand::V2ApiSearchZooTool {
                bearer,
                query,
                agent_or_llm,
                res,
                ...
                let db_clone = Arc::clone(&self.db);
                tokio::spawn(async move {
                    let _ = Node::v2_api_search_zoo_tool(db_clone, bearer, query, agent_or_llm, res).await;
                });
            }
            ...
            NodeCommand::V2ApiListMCPServers { bearer, res } => {
                let db_clone: Arc<zoo_sqlite::SqliteManager> = Arc::clone(&self.db);
                tokio::spawn(async move {
