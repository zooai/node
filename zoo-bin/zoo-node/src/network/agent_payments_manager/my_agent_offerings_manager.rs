...
use zoo_message_primitives::{
    schemas::{
        invoices::{InternalInvoiceRequest, Invoice, InvoiceStatusEnum, Payment}, zoo_name::ZooName, zoo_proxy_builder_info::ZooProxyBuilderInfo, zoo_tool_offering::{ZooToolOffering, ToolPrice, UsageTypeInquiry}, wallet_mixed::{AddressBalanceList, Asset}
    }, zoo_message::zoo_message_schemas::MessageSchemaType, zoo_utils::{
        encryption::clone_static_secret_key, zoo_message_builder::ZooMessageBuilder, signatures::clone_signature_secret_key
    }
};
use zoo_sqlite::SqliteManager;
use zoo_tools_primitives::tools::{
    network_tool::NetworkTool, parameters::Parameters, zoo_tool::ZooToolHeader, tool_output_arg::ToolOutputArg
};
...
impl MyAgentOfferingsManager {
    ...
    pub async fn pay_invoice(
        ...
        if let ToolPrice::Free = price {
            ...
            let payment = Payment::new(
                ...
                zoo_message_primitives::schemas::invoices::PaymentStatusEnum::Signed,
            );
    ...
    pub async fn pay_invoice_and_send_receipt(
        ...
        if let Some(db) = self.db.upgrade() {
            ...
            let mut trace_info = json!({
                ...
                "tool_key": updated_invoice.zoo_offering.tool_key,
                ...
                "tool_price": format!("{:?}", updated_invoice.zoo_offering.get_price_for_usage(&updated_invoice.usage_type_inquiry)),
    ...
    pub async fn get_balances(&self, node_name: ZooName) -> Result<AddressBalanceList, AgentOfferingManagerError> {
        let wallet_manager = self.wallet_manager.upgrade().ok_or_else(|| {
            AgentOfferingManagerError::OperationFailed("Failed to upgrade wallet_manager reference".to_string())
        })?;
...
mod tests {
    ...
    impl MockIdentityManager {
        pub fn new() -> Self {
            ...
            let dummy_standard_identity = Identity::Standard(StandardIdentity {
                full_identity_name: ZooName::new("@@localhost.sep-zoo/main".to_string()).unwrap(),
                addr: None,
    ...
    impl IdentityManagerTrait for MockIdentityManager {
        fn find_by_identity_name(&self, _full_profile_name: ZooName) -> Option<&Identity> {
            if _full_profile_name.to_string() == "@@localhost.sep-zoo/main" {
        ...
        async fn search_identity(&self, full_identity_name: &str) -> Option<Identity> {
            if full_identity_name == "@@localhost.sep-zoo/main" {
        ...
        async fn external_profile_to_global_identity(
            ...
            if full_profile_name == "@@localhost.sep-zoo" {
    ...
    //     let db = Arc::new(ZooDB::new("zoo_db_tests/zoodb").unwrap());
    ...
    //         LanceZooDb::new("lance_db_tests/lancedb", embedding_model.clone(), generator.clone()).await?,
    //     ));

    //     let tool_router = Arc::new(ToolRouter::new(lance_db.clone()));
    //     let node_name = ZooName::new("@@localhost.sep-zoo/main".to_string()).unwrap();
