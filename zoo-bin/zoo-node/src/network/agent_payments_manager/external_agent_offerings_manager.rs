...
use futures::Future;
use zoo_job_queue_manager::job_queue_manager::JobQueueManager;
use zoo_message_primitives::schemas::invoices::{
...
use zoo_message_primitives::zoo_message::zoo_message::ExternalMetadata;
...
use zoo_message_primitives::zoo_utils::signatures::clone_signature_secret_key;
use zoo_non_rust_code::functions::x402;
use zoo_non_rust_code::functions::x402::settle_payment::settle_payment;
use zoo_non_rust_code::functions::x402::settle_payment::Input as SettleInput;
use zoo_non_rust_code::functions::x402::verify_payment::verify_payment;
...
use zoo_message_primitives::schemas::x402_types::{
...
pub struct ExtAgentOfferingsManager {
    ...
    // pub shared_tools: Arc<DashMap<String, ZooToolOffering>>, // (streamer_profile:::path, shared_folder)
...
impl ExtAgentOfferingsManager {
    ...
    /// # Arguments
    ///
    /// * `db` - Weak reference to the ZooDB.
    ...
    pub async fn new(
        ...
        let db_prefix = "zoo__tool__offering_"; // dont change it
    ...
    /// * `_db` - Weak reference to the ZooDB.
    ...
    pub async fn update_shareable_tool_requirements(
        &self,
        updated_offering: ZooToolOffering,
    ...
    pub async fn invoice_requested(
        ...
        let usage_type = match invoice_request.usage_type_inquiry {
            UsageTypeInquiry::PerUse => match zoo_offering.usage_type {
    ...
    pub async fn network_invoice_requested(
        ...
        let invoice = match invoice {
            ...
            Err(e) => {
                ...
                if let Some(identity_manager_arc) = self.identity_manager.upgrade() {
                    ...
                    let error_message = ZooMessageBuilder::create_generic_invoice_message(
                        network_error.clone(),
        ...
        if let Some(identity_manager_arc) = self.identity_manager.upgrade() {
            ...
            let message = ZooMessageBuilder::create_generic_invoice_message(
                invoice.clone(),
    ...
    pub async fn confirm_invoice_payment_and_process(
        ...
        let is_free_tool = matches!(
            local_invoice.zoo_offering.usage_type,
        ...
        let output_opt = if !is_free_tool {
            ...
            let payment_requirements = match &local_invoice.zoo_offering.usage_type {
            ...
            .unwrap_or_else(|| serde_json::Map::new());
        {
            ...
            // js tool name
            let local_tool_key = local_invoice.zoo_offering.convert_tool_to_local().map_err(|e| {
...
mod tests {
    ...
    use zoo_message_primitives::{
        schemas::identity::{Identity, StandardIdentity, StandardIdentityType}, zoo_message::zoo_message_schemas::IdentityPermissions, zoo_utils::{
    ...
    impl IdentityManagerTrait for MockIdentityManager {
        ...
        async fn search_identity(&self, full_identity_name: &str) -> Option<Identity> {
            if full_identity_name == "@@node1.zoo/main" {
    ...
    fn setup() {
        ...
        let path = Path::new("zoo_db_tests/");
        let _ = fs::remove_dir_all(path);
    }

    fn default_test_profile() -> ZooName {
        ZooName::new("@@localhost.sep-zoo/main".to_string()).unwrap()
    }

    fn node_name() -> ZooName {
        ZooName::new("@@localhost.sep-zoo".to_string()).unwrap()
    }
}
