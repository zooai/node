...
use zoo_message_primitives::schemas::identity::{Identity, StandardIdentityType};
use std::io::Read;
use std::sync::Arc;
...
use zoo_message_primitives::{
    schemas::zoo_name::ZooName,
    zoo_message::{zoo_message::ZooMessage, zoo_message_schemas::MessageSchemaType},
    zoo_utils::encryption::string_to_encryption_public_key,
};
...
pub async fn validate_message_main_logic(
        ...
        if is_body_encrypted {
            ...
            */
            ...
            if sender_encryption_pk.is_some() {
                ...
                let sender_name = ZooName::from_zoo_message_using_sender_and_intra_sender(
    ...
    let sender_name = match ZooName::from_zoo_message_using_sender_subidentity(&msg.clone()) {
        Ok(name) => name,
        Err(e) => {
            return Err(APIError {
                code: StatusCode::BAD_REQUEST.as_u16(),
...
pub async fn download_zip_from_url(
    ...
    let request = client
        ...
        .header("X-Zoo-Signature", signature_hex);
