...
use zoo_message_primitives::schemas::{
    zoo_name::ZooName, zoo_tool_offering::ZooToolOffering, tool_router_key::ToolRouterKey
};
use zoo_sqlite::{errors::SqliteManagerError, SqliteManager};
use zoo_tools_primitives::tools::{
    network_tool::NetworkTool, zoo_tool::{ZooTool, ZooToolHeader}
};
...
impl Node {
    ...
    pub async fn v2_api_get_agent_network_offering(
        ...
        let node_name = match ZooName::new(node_name) {
            Ok(name) => name,
            Err(_) => {
                let api_error = APIError {
