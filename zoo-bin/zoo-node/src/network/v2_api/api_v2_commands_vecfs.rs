...
use zoo_message_primitives::{
    schemas::zoo_fs::ZooFileChunkCollection, zoo_message::zoo_message_schemas::{
...
impl Node {
    pub async fn v2_api_vec_fs_retrieve_path_simplified_json(
        ...
        let vr_path = ZooPath::from_string(input_payload.path);

        let depth = input_payload.depth.unwrap_or(1);

        // Use list_directory_contents_with_depth to get directory contents with depth 1
        let directory_contents = ZooFileManager::list_directory_contents_with_depth(vr_path, &db, depth);
    ...
    pub async fn v2_create_folder(
        ...
        let base_path = ZooPath::from_string(input_payload.path.clone());
        if !base_path.exists() {
            let api_error = APIError {
        ...
        let full_path = ZooPath::from_string(full_path_str);
        ...
        // Create the folder using ZooFileManager
        match ZooFileManager::create_folder(full_path) {
            Ok(_) => {
    ...
    pub async fn v2_move_item(
        ...
        let origin_path = ZooPath::from_string(input_payload.origin_path.clone());
        ...
        let destination_path = ZooPath::from_string(input_payload.destination_path.clone());
        ...
        // Move the file using ZooFileManager
        match ZooFileManager::move_file(origin_path, destination_path, &db) {
            Ok(_) => {
    ...
    pub async fn v2_copy_item(
        ...
        // Copy the file using ZooFileManager
        match ZooFileManager::copy_file(origin_path, destination_path) {
            Ok(_) => {
    ...
    pub async fn v2_move_folder(
        ...
        // Move the folder using ZooFileManager
        match ZooFileManager::move_folder(origin_path, destination_path, &db) {
            Ok(_) => {
    ...
    pub async fn v2_copy_folder(
        ...
        // let origin_path = match ZooPath::from_string(&input_payload.origin_path) {
        //     Ok(path) => path,
        //     Err(e) => {
        //         let api_error = APIError {
        //             code: StatusCode::BAD_REQUEST.as_u16(),
        ...
        // let destination_path = match ZooPath::from_string(&input_payload.destination_path) {
        //     Ok(path) => path,
        //     Err(e) => {
        //         let api_error = APIError {
        //             code: StatusCode::BAD_REQUEST.as_u16(),
    ...
    pub async fn v2_delete_folder(
        ...
        // Convert the path to ZooPath
        let folder_path = ZooPath::from_string(input_payload.path.clone());
        ...
        // Delete the folder using ZooFileManager
        match ZooFileManager::remove_folder(folder_path, &db) {
            Ok(_) => {
    ...
    pub async fn v2_delete_item(
        ...
        // Convert the path to ZooPath
        let item_path = ZooPath::from_string(input_payload.path.clone());
        if !item_path.exists() {
            let api_error = APIError {
                code: StatusCode::BAD_REQUEST.as_u16(),
                error: "Bad Request".to_string(),
        ...
        // Delete the file using ZooFileManager
        match ZooFileManager::remove_file(item_path, &db) {
            Ok(_) => {
    ...
    pub async fn v2_search_items(
        ...
        let search_path = ZooPath::from_string(search_path_str.clone());
            ...
            Ok(parsed_files) => {
                for parsed_file in parsed_files {
                    ...
                    paths_map.insert(
                        ...
                        ZooPath::from_string(parsed_file.relative_path.clone()),
                    );
                }
            }
            Err(e) => {
        ...
        let results = ZooFileChunkCollection {
    ...
    pub async fn v2_retrieve_vector_resource(
        ...
        // let vr_path = match ZooPath::from_string(&path) {
        //     Ok(path) => path,
        //     Err(e) => {
        //         let api_error = APIError {
        //             code: StatusCode::BAD_REQUEST.as_u16(),
    ...
    pub async fn v2_upload_file_to_folder(
        ...
        let full_path = ZooPath::from_string(full_path_str.clone());

        // Save and process the file
        match ZooFileManager::save_and_process_file(
            full_path.clone(),
            file,
            &db,
    ...
    pub async fn v2_retrieve_file(
        ...
        let vr_path = ZooPath::from_string(input_payload.path.clone());

        if use_processed {
            // Retrieve the parsed file from the database
            let parsed_file =
                match db.get_parsed_file_by_zoo_path(&ZooPath::from_string(input_payload.path.clone())) {
                    Ok(Some(pf)) => pf,
                    Ok(None) => {
                        let api_error = APIError {
    ...
    pub async fn v2_upload_file_to_job(
        ...
        match ZooFileManager::save_and_process_file_with_jobid(
            &job_id,
            filename.clone(),
            file,
            &db,
    ...
    pub async fn v2_api_vec_fs_retrieve_files_for_job(
        ...
        // Retrieve files for the given job_id using ZooFileManager
        let files_result = ZooFileManager::get_all_files_and_folders_for_job(&job_id, &db);

        let files = match files_result {
            Ok(files) => files,
            Err(ZooFsError::Io(io_error)) if io_error.kind() == std::io::ErrorKind::NotFound => {
    ...
    pub async fn v2_api_search_files_by_name(
        ...
        // Search for files using ZooFileManager::search_files_by_name_and_content
        match ZooFileManager::search_files_by_name_and_content(base_path, &name, &db) {
            Ok(files) => {
                let json_files = serde_json::to_value(files).map_err(|e| NodeError::from(e))?;
