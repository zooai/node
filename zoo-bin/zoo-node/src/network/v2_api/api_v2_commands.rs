use crate::llm_provider::providers::zoo_backend::check_quota;
...
use zoo_http_api::node_api_router::APIUseRegistrationCodeSuccessResponse;
use zoo_http_api::{
...
use zoo_message_primitives::{
    ...
    zoo_message::zoo_message_schemas::JobCreationInfo,
    zoo_message::{
        zoo_message::{MessageBody, MessageData, ZooMessage},
        zoo_message_schemas::{
    ...
    zoo_utils::{
        ...
        zoo_message_builder::ZooMessageBuilder,
        signatures::signature_public_key_to_string,
    },
    zoo_utils::{job_scope::MinimalJobScope, zoo_time::ZooStringTime},
};
use zoo_sqlite::regex_pattern_manager::RegexPattern;
...
impl Node {
    ...
    pub fn convert_zoo_message_to_v2_chat_message(
        zoo_message: ZooMessage,
    ) -> Result<V2ChatMessage, NodeError> {
        let internal_metadata = match &zoo_message.body {
        ...
        let message_data = match &zoo_message.body {
        ...
        let external_metadata = zoo_message.external_metadata;
    ...
    pub fn convert_zoo_messages_to_v2_chat_messages(
        zoo_messages: Vec<Vec<ZooMessage>>,
    ...
    pub fn api_v2_create_zoo_message(
        ...
        let timestamp = ZooStringTime::generate_time_now();
        ...
        ZooMessageBuilder::new(node_encryption_sk, node_signing_sk, receiver_public_key)
    ...
    pub async fn v2_api_check_zoo_backend_quota(
        db: Arc<SqliteManager>,
        model_type: String,
        bearer: String,
    ...
    pub async fn v2_api_change_nodes_name(
        ...
        let new_node_name = match ZooName::from_node_name(new_name) {
            Ok(name) => name,
            Err(_) => {
                let _ = res
                    .send(Err(APIError {
    ...
    pub async fn v2_api_add_agent(
        ...
        let expected_full_identity_name = ZooName::new(format!(
            "{}/main/agent/{}",
    ...
    pub async fn v2_api_test_llm_provider(
        ...
        let profile = ZooName::new(format!("{}/main", name)).unwrap();
        ...
        let provider = SerializedLLMProvider {
            ...
            name: None,
            description: None,
            full_identity_name: ZooName::new(llm_name).unwrap(),
    ...
    pub async fn v2_api_export_agent(
        ...
        let agent_zip: Result<Vec<u8>, APIError> = generate_agent_zip(db, zoo_name, node_env, agent_id, true).await;
    ...
    async fn publish_agent(
        ...
        let file_bytes: Vec<u8> =
            generate_agent_zip(db.clone(), zoo_name.clone(), node_env, agent_id.clone(), true).await?;
    ...
    pub async fn v2_api_check_default_tools_sync(
        ...
        let internal_comms_synced = match db.get_preference::<ZooInternalComms>("internal_comms") {
        ...
        .await
        {
            Ok(response) => {
                ...
                let galxe_quests_url = std::env::var("GALXE_QUESTS_URL")
                    .unwrap_or_else(|_| "https://api.zoo.ngo/galxe/user".to_string());
