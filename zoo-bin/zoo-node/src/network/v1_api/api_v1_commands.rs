...
use zoo_message_primitives::schemas::inbox_permission::InboxPermission;
use zoo_message_primitives::schemas::smart_inbox::SmartInbox;
use zoo_message_primitives::schemas::ws_types::WSUpdateHandler;
use zoo_message_primitives::zoo_utils::encryption::encryption_secret_key_to_string;
use zoo_message_primitives::{
    schemas::{
        inbox_name::InboxName, llm_providers::serialized_llm_provider::SerializedLLMProvider, zoo_name::{ZooName, ZooSubidentityType}
    }, zoo_message::{
        zoo_message::{MessageBody, MessageData, ZooMessage}, zoo_message_schemas::{
        ...
        }, zoo_logging::{zoo_log, ZooLogLevel, ZooLogOption}, signatures::{clone_signature_secret_key, signature_public_key_to_string, string_to_signature_public_key}
    }
};
...
impl Node {
    ...
    ) -> Result<(ZooMessage, Identity), APIError> {
    ...
    pub async fn has_inbox_access(
        ...
        let sender_zoo_name = ZooName::new(sender_subidentity.get_full_identity_name())
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;

        let has_creation_permission = inbox_name.has_creation_access(sender_zoo_name);
    ...
    pub async fn api_get_last_messages_from_inbox(
        ...
        node_name: ZooName,
        potentially_encrypted_msg: ZooMessage,
        res: Sender<Result<Vec<ZooMessage>, APIError>>,
    ) -> Result<(), NodeError> {
    ...
    pub async fn api_get_last_messages_from_inbox_with_branches(
        ...
        node_name: ZooName,
        potentially_encrypted_msg: ZooMessage,
        res: Sender<Result<Vec<Vec<ZooMessage>>, APIError>>,
    ) -> Result<(), NodeError> {
        Self::process_last_messages_from_inbox(
            encryption_secret_key,
            db,
    ...
    pub async fn handle_registration_code_usage(
        ...
        let _profile_list: Vec<ZooName> = match db.get_all_profiles(node_name.clone()) {
            ...
            Ok(success) => {
                match identity_type {
                    IdentityType::Profile | IdentityType::Global => {
                        ...
                        let full_identity_name_result = ZooName::from_node_and_profile_names(
                    ...
                    IdentityType::Device => {
                        ...
                        let code_info: zoo_message_primitives::schemas::identity_registration::RegistrationCodeInfo = db.get_registration_code_info(code.clone().as_str()).unwrap();
                        ...
                        // Check if the profile exists in the identity_manager
                        {
                            ...
                            let profile_identity_name = ZooName::from_node_and_profile_names(
                                node_name.get_node_name_string(),
                                profile_name.clone(),
                            )
                            .unwrap();
                        ...
                        // let full_identity_name = format!("{}/{}", self.node_profile_name.clone(),
                        // profile_name.clone());
                        let full_identity_name = ZooName::from_node_and_profile_names_and_type_and_name(
                            node_name.get_node_name_string(),
                            profile_name,
    ...
    pub async fn api_get_all_inboxes_for_profile(
        ...
        let profile_requested: ZooName = if ZooName::validate_name(&profile_requested_str).is_ok() {
            ZooName::new(profile_requested_str.clone()).map_err(|err| err.to_string())?
        } else {
            ZooName::from_node_and_profile_names(node_name.get_node_name_string(), profile_requested_str.clone())
    ...
    pub async fn api_job_message(
        ...
        let sender_zoo_name = sender_identity.get_full_identity_name();
        let sender_node_name = match ZooName::new(sender_zoo_name) {
    ...
    pub async fn api_available_llm_providers(
        ...
        let profile = ZooName::from_zoo_message_using_sender_subidentity(&msg.clone())?
    ...
    pub async fn api_add_ollama_models(
        ...
        // Convert ZooName to StandardIdentity if necessary and check if it's a Profile type with admin privileges
    ...
    pub async fn api_remove_agent(
        ...
        let profile = match ZooName::new(profile) {
            Ok(profile) => profile,
            Err(err) => {
                let api_error = APIError {
                    code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn api_search_zoo_tool(
        ...
        let (search_query, requester_name) = match Self::validate_and_extract_payload::<String>(
            ...
            encryption_secret_key,
            potentially_encrypted_msg,
            MessageSchemaType::SearchZooTool,
        )
        .await
        {
            Ok(data) => data,
    ...
    pub async fn api_list_all_zoo_tools(
        ...
        let requester_name = match Self::validate_and_extract_payload::<String>(
            ...
            encryption_secret_key,
            potentially_encrypted_msg,
            MessageSchemaType::ListAllZooTools,
        )
        .await
        {
    ...
    pub async fn api_get_zoo_tool(
        ...
        // Fetch the tool from the LanceZooDb
    ...
    pub async fn api_change_nodes_name(
        ...
        let new_node_name = match ZooName::from_node_name(new_node_name) {
            Ok(name) => name,
            Err(_) => {
                let _ = res
                    .send(Err(APIError {
    ...
    pub async fn api_handle_send_onionized_message(
        ...
        //
        let recipient_node_name = ZooName::from_zoo_message_only_using_recipient_node_name(&msg.clone())
            .unwrap()
            .get_node_name_string();

        let sender_node_name = ZooName::from_zoo_message_only_using_sender_node_name(&msg.clone())
            .unwrap()
            .get_node_name_string();
        ...
        let recipient_node_name_string = ZooName::from_zoo_message_only_using_recipient_node_name(&msg.clone())
            .unwrap()
            .to_string();
