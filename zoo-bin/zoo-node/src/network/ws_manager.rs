...
use zoo_message_primitives::schemas::ws_types::MessageQueue;
use zoo_message_primitives::schemas::ws_types::MessageType;
...
use zoo_message_primitives::schemas::ws_types::WebSocketManagerError;
...
pub struct WebSocketManager {
    ...
    // TODO: maybe the first string should be a ZooName? or at least a zoo name string
    subscriptions: HashMap<String, HashMap<String, bool>>,
    shared_keys: HashMap<String, String>,
    zoo_db: Weak<SqliteManager>,
    node_name: ZooName,
...
impl WebSocketManager {
    ...
    pub async fn has_access(&self, zoo_name: ZooName, topic: WSTopic, subtopic: Option<String>) -> bool {
        match topic {
            WSTopic::Inbox => {
                let subtopic = subtopic.unwrap_or_default();
                let inbox_name = InboxName::new(subtopic.clone()).unwrap(); // TODO: handle error
                let sender_identity = match self.get_sender_identity(zoo_name.clone()).await {
                    Ok(identity) => identity,
                    Err(_) => return false,
                };
                let db_arc = self.zoo_db.upgrade().ok_or("Failed to upgrade zoo_db").unwrap();
    ...
    pub async fn manage_connections(
        ...
        let sender_zoo_name = ZooName::new(format!("{}/main", node_name)).unwrap();
        let zoo_profile_name = sender_zoo_name.full_name.clone();
        ...
        for subscription in ws_message.subscriptions.iter() {
            ...
            {
                eprintln!(
                    "Access denied for zoo_name: {} on topic: {:?} and subtopic: {:?}",
                    sender_zoo_name, subscription.topic, subscription.subtopic
                );
                // TODO: should we send a ZooMessage with an error inside back?
        ...
        self.connections.insert(zoo_profile_name.clone(), connection);

        if let Some(key) = shared_key {
            self.shared_keys.insert(zoo_profile_name.clone(), key);
        }
        // Note: uncomment this enforce having a shared encryption key
        // else if !self.shared_keys.contains_key(&zoo_profile_name) {
        //     return Err(WebSocketManagerError::MissingSharedKey(format!(
        //         "Missing shared key for zoo_name: {}",
        //         zoo_profile_name
        //     )));
        // }
        ...
        let subscriptions_to_remove: Vec<(WSTopic, Option<String>)> = ws_message
            ...
            .map(|s| (s.topic.clone(), s.subtopic.clone()))
            .collect();
        self.update_subscriptions(&zoo_profile_name, subscriptions_to_add, subscriptions_to_remove)
            .await;

        zoo_log(
            ZooLogOption::WsAPI,
            ZooLogLevel::Info,
            format!("Successfully added connection for zoo_name: main").as_str(),
        );
    ...
    pub async fn update_subscriptions(
        ...
        let profile_subscriptions = self.subscriptions.entry(zoo_name.to_string()).or_default();
    ...
    pub async fn handle_update(
        ...
        for (id, connection) in self.connections.iter() {
                ...
                || (is_subscribed_to_sheets && topic == WSTopic::Sheet)
            {
                ...
                if is_subscribed_to_smart_inboxes {
                    match ZooName::new(id.clone()) {
                        Ok(zoo_name) => {
                            let zoo_name_clone = zoo_name.clone();
                            if !self
                                .has_access(zoo_name_clone, topic.clone(), Some(subtopic.clone()))
                                .await
                            {
                                continue;
                            }
                            eprintln!(
                                "Access granted for zoo_name: {} on topic: {:?} and subtopic: {:?}",
                                zoo_name, topic, subtopic
                            );
                        }
                        Err(e) => {
                            zoo_log(
                                ZooLogOption::WsAPI,
                                ZooLogLevel::Error,
                                format!("Failed to create ZooName for id {}: {}", id, e).as_str(),
                            );
                            continue;
                        }
                    }
    ...
    pub async fn get_sender_identity(&self, zoo_name: ZooName) -> Result<Identity, WebSocketManagerError> {
        let identity_manager_lock = self.identity_manager_trait.lock().await;
        match identity_manager_lock.find_by_identity_name(zoo_name.clone()) {
            Some(identity) => Ok(identity.clone()),
            None => {
                zoo_log(
                    ZooLogOption::WsAPI,
                    ZooLogLevel::Error,
                    format!("No identity found for zoo_name: {}", zoo_name).as_str(),
                );
                Err(WebSocketManagerError::UserValidationFailed(format!(
                    "No identity found for zoo_name: {}",
                    zoo_name
                )))
            }
        }
