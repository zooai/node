...
use zoo_message_primitives::{
    zoo_message::zoo_message::ZooMessage,
...
pub struct ZooNetworkBehaviour {
...
pub enum NetworkEvent {
    ...
    SendResponse {
        channel: ResponseChannel<ZooMessage>,
        message: ZooMessage,
    },
...
pub struct LibP2PManager {
    swarm: Swarm<ZooNetworkBehaviour>,
    event_sender: mpsc::UnboundedSender<NetworkEvent>,
    event_receiver: mpsc::UnboundedReceiver<NetworkEvent>,
    message_handler: Arc<ZooMessageHandler>,
...
use crate::network::network_manager::libp2p_message_handler::ZooMessageHandler;
...
impl LibP2PManager {
    ...
    pub async fn new(
        ...
        message_handler: ZooMessageHandler,
        ...
        libp2p::SwarmBuilder::with_existing_identity(local_key)
            ...
            .with_behaviour(|keypair, relay_behaviour| ZooNetworkBehaviour {
                ...
                identify: identify::Behaviour::new(identify::Config::new(
                    "/zoo/1.0.0".to_string(),
                    keypair.public(),
                ).with_agent_version(format!("zoo-node-{}", node_name))
    ...
    async fn handle_swarm_event(
        &mut self,
        event: SwarmEvent<ZooNetworkBehaviourEvent>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        match event {
            ...
            SwarmEvent::Behaviour(ZooNetworkBehaviourEvent::RelayClient(relay_event)) => {
            ...
            SwarmEvent::Behaviour(ZooNetworkBehaviourEvent::Dcutr(dcutr_event)) => {
            ...
            SwarmEvent::Behaviour(ZooNetworkBehaviourEvent::Ping(ping_event)) => {
            ...
            SwarmEvent::Behaviour(ZooNetworkBehaviourEvent::Identify(identify::Event::Received { peer_id, info, .. })) => {
                zoo_log(
                    ZooLogOption::Network,
                    ZooLogLevel::Debug,
                ...
                if supports_relay {
                    ...
                    // Check if this peer supports our Zoo message protocol
                    let supports_zoo = info.protocols.iter().any(|protocol| {
                        protocol.to_string().contains("/zoo/message/")
                    });
                    ...
                    if supports_zoo {
                        zoo_log(
                            ZooLogOption::Network,
                            ZooLogLevel::Info,
                            &format!("✅ Peer {} supports Zoo message protocol - ready for communication", peer_id),
                        );
                    } else {
                        zoo_log(
                            ZooLogOption::Network,
                            ZooLogLevel::Debug,
                            &format!("⚠️  Peer {} does not support Zoo message protocol", peer_id),
                        );
                    }
                }
            ...
            SwarmEvent::Behaviour(ZooNetworkBehaviourEvent::RequestResponse(req_resp_event)) => {
