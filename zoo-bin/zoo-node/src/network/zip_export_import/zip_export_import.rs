...
use zoo_tools_primitives::tools::agent_tool_wrapper::AgentToolWrapper;
...
async fn calculate_zip_dependencies(
    ...
    if let Some(tool) = tool_entry_point {
        ...
        match tool.clone() {
            ...
            ZooTool::Network(_, _) => (),
            ZooTool::MCPServer(mcp_server_tool, _) => {
    ...
    if let Some(agent) = agent_entry_point {
        ...
        let tool: ZooTool = db.get_tool_by_agent_id(&agent_id).map_err(|e| {
...
async fn get_dependencies_for_zip(
    ...
    tool_dependencies: &HashMap<String, ZooTool>,
...
fn add_knowledge_embeddings_to_zip(
    ...
    let text_groups: Vec<(String, Vec<f32>)> =
        ZooFileManager::get_text_groups_with_embeddings(item, &db).map_err(|e| APIError {
            code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
            error: "Internal Server Error".to_string(),
        ...
        if item.clone().exists() {
            ...
            for entry in files {
                    ...
                    .is_file()
                {
                    ...
                    zip_files.insert(
                        format!("__knowledge/{}", zoo_path.relative_path().to_string()),
                        file_bytes,
                    );
                    add_knowledge_embeddings_to_zip(db.clone(), zoo_path, zip_files)?;
                }
            }
        }
    }
...
async fn import_agent_knowledge(
    ...
    for file in files {
        if file.starts_with("__knowledge/") && !file.ends_with("/") {
            ...
            let dest_path = ZooPath::from_str(&relative_path);

            let file_content = ZooFileManager::get_file_content(dest_path.clone());
            ...
            if embedding_buffer.is_some() {
                ...
                ZooFileManager::save_and_process_file_with_embeddings(
                    dest_path,
                    buffer,
                    &db,
                ...
                ZooFileManager::save_and_process_file(
                    dest_path,
                    buffer,
                    &db,
...
pub async fn import_dependencies_tools(
    ...
    for file_name_str in file_list {
        if file_name_str.starts_with("__tools/") && file_name_str != "__tools/" {
            ...
            let tool: ZooTool = match serde_json::from_slice(&tool_zip.buffer) {
                Ok(tool) => tool,
                Err(err) => {
                    let api_error = APIError {
                        code: StatusCode::BAD_REQUEST.as_u16(),
...
pub async fn import_mcp_server(
    ...
    if let Ok(tools) = tools {
        for tool in tools {
            ...
            db.add_tool(zoo_tool).await.map_err(|e| APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                error: "Database Error".to_string(),
...
pub async fn import_agent(
    ...
    let preferences_llm_provider_result = match db.get_preference::<String>("default_llm_provider") {
        Ok(llm_provider) => match llm_provider {
            ...
            None => Node::zoo_free_provider_id(),
        },
        Err(_) => {
            return Err(APIError {
                code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
    ...
    let local_full_identity_name = ZooName::new(format!(
        "{}/main/agent/{}",
    ...
    if install {
        match db.add_agent(agent.clone(), &agent.full_identity_name) {
            Ok(_) => {
                ...
                let zoo_tool = ZooTool::Agent(agent_tool_wrapper, true);
                let install_tool = match db.get_tool_by_key(&zoo_tool.tool_router_key().to_string_without_version())
                {
                    Ok(_) => false,
                    Err(_) => true,
                };
...
pub fn get_tool_from_zip(mut archive: ZipArchive<std::io::Cursor<Vec<u8>>>) -> Result<ZooTool, APIError> {
    ...
    let tool: ZooTool = serde_json::from_slice(&buffer).map_err(|e| APIError {
        code: StatusCode::BAD_REQUEST.as_u16(),
        error: "Invalid Tool JSON".to_string(),
...
mod tests {
    ...
    async fn test_tool_dependency_cycles() {
        ...
        tool_a
            .tools
            .push(ZooTool::Deno(tool_b.clone(), true).tool_router_key());
        tool_b
            .tools
            .push(ZooTool::Deno(tool_c.clone(), true).tool_router_key());
        tool_c
            .tools
            .push(ZooTool::Deno(tool_a.clone(), true).tool_router_key());

        // Add tools to database
        db.add_tool_with_vector(
            ZooTool::Deno(tool_a.clone(), true),
            SqliteManager::generate_vector_for_testing(0.1),
        )
        .unwrap();
        db.add_tool_with_vector(
            ZooTool::Deno(tool_b.clone(), true),
            SqliteManager::generate_vector_for_testing(0.5),
        )
        .unwrap();
        db.add_tool_with_vector(
            ZooTool::Deno(tool_c.clone(), true),
        ...
        let result = calculate_zip_dependencies(
            db.clone(),
            profile.clone(),
            Some(ZooTool::Deno(tool_a.clone(), true)),
            None,
            None,
            &mut agent_dependencies,
            &mut tool_dependencies,
    ...
    async fn test_tool_dependency_cycles_agent() {
        let manager = setup_test_db().await;
        let db = Arc::new(manager);
        let profile: ZooName = ZooName::new("@@test_user.zoo/main".to_string()).unwrap();
        ...
        let agent_tool_wrapper = ZooTool::Agent(
    ...
    async fn test_agent_tool_dependencies() {
        ...
        // Add tools to database
        let tool_a_wrapper = ZooTool::Deno(tool_a.clone(), true);
        let tool_b_wrapper = ZooTool::Deno(tool_b.clone(), true);
