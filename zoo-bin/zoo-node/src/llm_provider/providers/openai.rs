...
use zoo_message_primitives::schemas::job_config::JobConfig;
use zoo_message_primitives::schemas::llm_providers::serialized_llm_provider::{LLMProviderInterface, OpenAI};
use zoo_message_primitives::schemas::prompts::Prompt;
use zoo_message_primitives::schemas::ws_types::{
    WSUpdateHandler,
};
...
pub async fn handle_streaming_response(
    ...
    let res = client
        ...
        .header(
            ...
            headers
                .as_ref()
                .and_then(|h| h.get("x-zoo-job-id").and_then(|v| v.as_str()))
                .unwrap_or(""),
        )
        .header(
            "X-Zoo-Version",
            headers
                .as_ref()
                .and_then(|h| h.get("x-zoo-version"))
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
        .header(
            ...
            headers
                .as_ref()
                .and_then(|h| h.get("x-zoo-identity"))
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
        .header(
            ...
            headers
                .as_ref()
                .and_then(|h| h.get("x-zoo-signature"))
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
        .header(
            ...
            headers
                .as_ref()
                .and_then(|h| h.get("x-zoo-metadata"))
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
        .header(
            ...
            headers
                .as_ref()
                .and_then(|h| h.get("x-zoo-session-id"))
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
        .json(&payload)
...
mod tests {
    ...
    async fn test_parse_openai_stream_chunk_smtp_function_call() {
        ...
        assert_eq!(fc.name, "zoo_tool_config_updater");
    ...
    async fn test_parse_openai_stream_chunk_invalid_function_parameters() {
        ...
        buffer.push_str(r#"{"error": {"message": "Invalid schema for function 'zoo_typescript_unsafe_processor': ['code', 'package', 'parameters', 'code'] has non-unique elements.", "type": "invalid_request_error", "param": "functions[2].parameters", "code": "invalid_function_parameters"}}"#);
        buffer.push('\n');
        ...
        if let Err(LLMProviderError::APIError(error_message)) = result {
            assert_eq!(
                error_message,
                "invalid_function_parameters: Invalid schema for function 'zoo_typescript_unsafe_processor': ['code', 'package', 'parameters', 'code'] has non-unique elements."
            );
        } else {
    ...
    async fn test_parse_openai_stream_chunk_stagehand_runner() {
        ...
        // Verify the arguments
        // Note: The test input chunks concatenate to a URL from captured API response data
        let expected_args = serde_json::json!({
            "commands": [
                {
                    "action": "goto",
            ...
            "choices": [
                {
                    ...
                    "message": {
                        ...
                        "function_call": {
                            "name": "stagehand_runner",
                            "arguments": "{\"commands\":[{\"action\":\"goto\",\"payload\":\"https://zoo.ngo\"},{\"action\":\"extract\",\"payload\":\"titles and subtitles\"}]}"
                        },
                        "refusal": null
                    },
                    "logprobs": null,
