...
use zoo_message_primitives::schemas::ws_types::{WSMessageType, WSMetadata, WSUpdateHandler};
...
pub async fn save_image_file(
    ...
    session_id: &str,
    db: &SqliteManager,
) -> Result<ZooPath, LLMProviderError> {
    ...
    let zoo_path = ZooFileManager::save_and_process_file_with_jobid(
        &job_id,
        filename.clone(),
        image_data,
        db,
...
pub async fn send_tool_ws_update_with_status(
    ...
    status_type: Option<zoo_message_primitives::schemas::ws_types::ToolStatusType>,
) -> Result<(), LLMProviderError> {
    use zoo_message_primitives::schemas::ws_types::{ToolMetadata, ToolStatus, ToolStatusType, WSMessageType, WidgetMetadata};
