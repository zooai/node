...
use zoo_message_primitives::schemas::llm_providers::serialized_llm_provider::{
    LLMProviderInterface, ZooBackend
};
...
#[async_trait]
impl LLMService for ZooBackend {
    async fn call_api(
        &self,
        client: &Client,
        url: Option<&String>,
        ...
        if let Some(base_url) = url {
            ...
            let base_url = env::var("ZOO_INFERENCE_BASE_URL")
                .unwrap_or_else(|_| "https://api.zoo.ngo/inference".to_string());
            api_url = format!("{}/ai/chat/completions", base_url);
        }
        ...
        let headers = json!({
            "x-zoo-version": env!("CARGO_PKG_VERSION"),
            "x-zoo-identity": node_name,
            "x-zoo-signature": signature,
            "x-zoo-metadata": metadata,
            "x-zoo-session-id": session_id,
            "x-zoo-job-id": job_id,
        });
...
pub async fn check_quota(db: Arc<SqliteManager>, model_type: String) -> Result<QuotaResponse, LLMProviderError> {
    ...
    let session_id = Uuid::new_v4().to_string();
    let base_url =
        env::var("ZOO_INFERENCE_BASE_URL").unwrap_or_else(|_| "https://api.zoo.ngo/inference".to_string());
    ...
    match client
        ...
        .header("x-zoo-identity", node_name)
        .header("x-zoo-signature", signature)
        .header("x-zoo-metadata", metadata)
        .header("x-zoo-session-id", session_id)
        .send()
        .await
    {
        Ok(response) => {
