...
use zoo_message_primitives::schemas::subprompts::SubPromptType;
...
impl JobPromptGenerator {
    ...
    pub async fn generic_inference_prompt(
        ...
        if !tools.is_empty() {
                ...
                if let Ok(tool_content) = tool.json_function_call_format() {
                    match tool_content.get("function") {
                        Some(function) => {
                            ...
                            if tool_router_key == "local:::__official_zoo:::zoo_sqlite_query_executor" {
                            ...
                            if tool_router_key == "local:::__official_zoo:::zoo_typescript_unsafe_processor" {
                                continue;
                            }
                        }
                        None => {}
            ...
            // Add job scope files
            let job_scope = ZooFileManager::get_absolute_path_for_job_scope(&db, &job_id);
