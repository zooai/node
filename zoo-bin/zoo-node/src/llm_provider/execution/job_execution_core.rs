...
use zoo_message_primitives::zoo_message::zoo_message_schemas::{CallbackAction, MessageMetadata};
use zoo_message_primitives::zoo_utils::zoo_logging::{zoo_log, ZooLogLevel, ZooLogOption};
use zoo_message_primitives::{
    schemas::zoo_name::ZooName, zoo_message::zoo_message_schemas::JobMessage, zoo_utils::{zoo_message_builder::ZooMessageBuilder, signatures::clone_signature_secret_key}
};
...
impl JobManager {
    ...
    pub async fn process_job_message_queued(
        ...
        let db = db.upgrade().ok_or("Failed to upgrade zoo_db").unwrap();
        ...
        let user_profile = ZooName::from_node_and_profile_names(
    ...
    async fn handle_error(
        ...
        let node_name = user_profile
            .unwrap_or_else(|| ZooName::new("@@localhost.sep-zoo".to_string()).unwrap())
            .node_name;
        ...
        db.add_message_to_job_inbox(job_id, &zoo_message, None, ws_manager)
            .await
    ...
    pub async fn process_inference_chain(
        ...
        .map_err(|e| LLMProviderError::ZooMessageBuilderError(e.to_string()))?;

        zoo_log(
            ZooLogOption::JobExecution,
            ZooLogLevel::Debug,
            format!("process_inference_chain> zoo_message: {:?}", zoo_message).as_str(),
        );

        db.add_message_to_job_inbox(&job_message.job_id.clone(), &zoo_message, None, ws_manager)
            .await?;
    ...
    pub async fn get_image_files_from_message(
        ...
        for filename in &job_message.job_filenames {
                ...
                || filename_lower.ends_with(".gif")
            {
                ...
                match ZooFileManager::construct_job_file_path(&job_message.job_id, filename, &db) {
                    Ok(file_path) => {
                        // Retrieve the file content
                        match ZooFileManager::get_file_content(file_path.clone()) {
                            Ok(content) => {
