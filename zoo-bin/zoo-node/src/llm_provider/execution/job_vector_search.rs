...
impl JobManager {
    ...
    async fn process_folder_contents(
        ...
        paths_map: &mut HashMap<i64, ZooPath>,
        ...
        for file_info in files {
            if !file_info.is_directory && file_info.has_embeddings {
                ...
                if let Some(parsed_file) = sqlite_manager.get_parsed_file_by_zoo_path(&file_path).unwrap() {
                ...
                let sub_folder_path = ZooPath::from_string(file_info.path);
    ...
    pub async fn search_for_chunks_in_resources(
        ...
        for path in &fs_files_paths {
            if let Some(parsed_file) = sqlite_manager.get_parsed_file_by_zoo_path(path).unwrap() {
                let file_id = parsed_file.id.unwrap();
                parsed_file_ids.push(file_id);
        ...
        for filename in &job_filenames {
            let file_path = match ZooFileManager::construct_job_file_path(&job_id, filename, &sqlite_manager) {
                Ok(path) => path,
                Err(_) => continue,
            };
        ...
        if all_files_have_token_count && total_tokens <= max_tokens_in_prompt as i64 {
            ...
            return Ok(ZooFileChunkCollection {
                chunks: all_chunks,
        ...
        let expanded_results: Vec<ZooFileChunk> = expanded_results_set.into_iter().collect();

        Ok(ZooFileChunkCollection {
