...
impl JobCallbackManager {
    ...
    pub async fn handle_implementation_check_callback(
        ...
        // Create a ZooMessage that looks it came from the user
        let zoo_message = ZooMessageBuilder::job_message_unencrypted(
            job_id.to_string(),
            error_message,
            vec![],
            "".to_string(),
        ...
        if let Some(job_manager) = job_manager {
            let _result = Node::internal_job_message(job_manager, zoo_message.clone(), true).await;
        } else {
