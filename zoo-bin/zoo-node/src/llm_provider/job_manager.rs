...
use zoo_message_primitives::schemas::job::JobLike;
...
use zoo_message_primitives::{
    schemas::zoo_name::ZooName, zoo_message::{
        zoo_message::{MessageBody, MessageData, ZooMessage}, zoo_message_schemas::{JobCreationInfo, JobMessage, MessageSchemaType}
    }, zoo_utils::signatures::clone_signature_secret_key
};
...
impl JobManager {
    ...
    pub async fn new(
        ...
        {
            let db_arc = db.upgrade().ok_or("Failed to upgrade zoo_db").unwrap();
    ...
    pub async fn process_job_message(
        ...
        let profile = ZooName::from_zoo_message_using_recipient_subidentity(&message)?;
                        ...
                        MessageData::Unencrypted(data) => {
                            ...
                            match message_type {
                                MessageSchemaType::JobCreationSchema => {
                                    let agent_name =
                                        ZooName::from_zoo_message_using_recipient_subidentity(&message)?;
    ...
    // From JobManager
    /// Checks that the provided ZooMessage is an unencrypted job message
    pub fn is_job_message(&mut self, message: ZooMessage) -> bool {
        matches!(
            &message.body,
    ...
    pub async fn process_job_creation(
        ...
        {
            let db_arc = self.db.upgrade().ok_or("Failed to upgrade zoo_db").unwrap();
            ...
            match db_arc.create_new_job(
                ...
                Ok(_) => (),
                Err(err) => return Err(LLMProviderError::ZooDB(err)),
            };
            ...
            match db_arc.get_job(&job_id) {
                ...
                Err(err) => Err(LLMProviderError::ZooDB(err)),
            }
        }
    }
    ...
    async fn update_job_folder_name(
        ...
        let old_folder = db_arc
            ...
            .map_err(|e| LLMProviderError::ZooDB(e))?;
        ...
        if old_folder.exists() {
            ZooFileManager::move_folder(old_folder, new_folder, db_arc)
    ...
    pub async fn add_to_job_processing_queue(
        ...
        let sender_subidentity_result = ZooName::from_zoo_message_using_sender_subidentity(&message.clone());
