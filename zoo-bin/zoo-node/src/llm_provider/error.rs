...
use zoo_message_primitives::{
    schemas::{inbox_name::InboxNameError, prompts::PromptError, zoo_name::ZooNameError},
    zoo_message::zoo_message_error::ZooMessageError,
};
use zoo_sqlite::errors::SqliteManagerError;
use zoo_tools_primitives::tools::{error::ToolError, rust_tools::RustToolError};
use std::fmt;
...
pub enum LLMProviderError {
    ...
    IO(String),
    ZooDB(SqliteManagerError),
    ZooNameError(ZooNameError),
    LLMProviderNotFound,
    ContentParseFailed,
    InferenceJSONResponseMissingField(String),
    JSONSerializationError(String),
    InvalidSubidentity(ZooNameError),
    ...
    ZooBackendInvalidAuthentication(String),
    ZooBackendInvalidConfiguration(String),
    ZooBackendInferenceLimitReached(String),
    ZooBackendAIProviderError(String),
    ZooBackendUnexpectedStatusCode(u64),
    ZooBackendUnexpectedError(String),
...
impl fmt::Display for LLMProviderError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            LLMProviderError::ZooDB(err) => write!(f, "Zoo DB error: {}", err),
            ...
            LLMProviderError::ZooNameError(err) => write!(f, "ZooName error: {}", err),
            ...
            LLMProviderError::ZooMessage(err) => write!(f, "ZooMessage error: {}", err),
            ...
            LLMProviderError::ZooBackendInvalidAuthentication(s) => write!(f, "Zoo Backend Invalid Authentication: {}", s),
            LLMProviderError::ZooBackendInvalidConfiguration(s) => write!(f, "Zoo Backend Invalid configuration: {}", s),
            LLMProviderError::ZooBackendInferenceLimitReached(s) => write!(f, "Zoo Backend Inference Limit Reached: {}", s),
            LLMProviderError::ZooBackendAIProviderError(s) => write!(f, "Zoo Backend AI Provider Error: {}", s),
            LLMProviderError::ZooBackendUnexpectedStatusCode(code) => write!(f, "Zoo Backend Unexpected Status Code: {}", code),
            LLMProviderError::ZooBackendUnexpectedError(e) => write!(f, "Zoo Backend Unexpected Error: {}", e),
            ...
            LLMProviderError::ZooMessageBuilderError(s) => write!(f, "{}", s),
...
impl LLMProviderError {
    ...
    pub fn to_error_message(&self) -> String {
        let error_name = match self {
            ...
            LLMProviderError::ZooDB(_) => "ZooDB",
            LLMProviderError::ZooNameError(_) => "ZooNameError",
            ...
            LLMProviderError::ZooMessage(_) => "ZooMessage",
            ...
            LLMProviderError::ZooBackendInvalidAuthentication(_) => "ZooBackendInvalidAuthentication",
            LLMProviderError::ZooBackendInvalidConfiguration(_) => "ZooBackendInvalidConfiguration",
            LLMProviderError::ZooBackendInferenceLimitReached(_) => "ZooBackendInferenceLimitReached",
            LLMProviderError::ZooBackendAIProviderError(_) => "ZooBackendAIProviderError",
            LLMProviderError::ZooBackendUnexpectedStatusCode(_) => "ZooBackendUnexpectedStatusCode",
            LLMProviderError::ZooBackendUnexpectedError(_) => "ZooBackendUnexpectedError",
            ...
            LLMProviderError::ZooMessageBuilderError(_) => "ZooMessageBuilderError",
...
impl std::error::Error for LLMProviderError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            LLMProviderError::ZooDB(err) => Some(err),
            LLMProviderError::ZooNameError(err) => Some(err),
            _ => None,
        }
    }
}
...
impl From<SqliteManagerError> for LLMProviderError {
    fn from(err: SqliteManagerError) -> LLMProviderError {
        LLMProviderError::ZooDB(err)
    }
}

impl From<ZooNameError> for LLMProviderError {
    fn from(err: ZooNameError) -> LLMProviderError {
        LLMProviderError::ZooNameError(err)
    }
}
...
impl From<ZooMessageError> for LLMProviderError {
    fn from(error: ZooMessageError) -> Self {
        LLMProviderError::ZooMessage(error)
    }
}
...
impl From<ZooFsError> for LLMProviderError {
    fn from(err: ZooFsError) -> LLMProviderError {
        LLMProviderError::IO(err.to_string())
    }
}
