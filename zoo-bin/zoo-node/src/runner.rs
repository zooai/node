...
> {
    ...
    let global_identity_name = secrets
        ...
        .cloned()
        .unwrap_or_else(|| env::var("GLOBAL_IDENTITY_NAME").unwrap_or("@@localhost.sep-zoo".to_string()));

    let global_identity_name = if global_identity_name.is_empty() {
        "@@localhost.sep-zoo".to_string()
    } else {
...
pub async fn run_node_tasks(
    ...
    match tokio::try_join!(api_server, node_task) {
        Ok(_) => {
            zoo_log(ZooLogOption::Node, ZooLogLevel::Info, "All tasks completed");
            Ok(())
        }
        Err(e) => {
...
/// Machine filesystem path to the main ZooDB database, pub key based.
    ...
    for line in contents.lines() {
        if let Some((key, value)) = line.split_once('=') {
            ...
            if key == "GLOBAL_IDENTITY_NAME" {
                let updated_value = if value.contains(".arb-sep-zoo") {
                    value.replace(".arb-sep-zoo", ".sep-zoo")
                } else {
                    value.to_string()
                };
...
pub fn print_node_info(node_env: &NodeEnvironment, encryption_pk: &str, signature_pk: &str, main_db_path: &str) {
    ...
    println!("Node Zoo identity: {}", node_env.global_identity_name);
