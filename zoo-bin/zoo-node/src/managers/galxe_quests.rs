...
pub async fn compute_create_identity_quest(
    ...
    if node_name.to_string() == "@@localhost.sep-zoo" {
...
pub async fn compute_download_store_quest(db: Arc<SqliteManager>) -> Result<bool, String> {
    ...
    let url = std::env::var("ZOO_TOOLS_DIRECTORY_URL")
        .unwrap_or_else(|_| "https://store-api.zoo.ngo/store/defaults".to_string());
...
pub async fn compute_submit_approval_quest(_db: Arc<SqliteManager>, node_name: ZooName) -> Result<bool, String> {
    let url = format!("https://store-api.zoo.ngo/user/{}/apps", node_name.to_string());
...
pub async fn compute_top_ranking_quest(_db: Arc<SqliteManager>, node_name: ZooName) -> Result<bool, String> {
    let url = format!("https://store-api.zoo.ngo/user/{}/apps", node_name.to_string());
...
pub async fn compute_write_app_reviews_quest(_db: Arc<SqliteManager>, node_name: ZooName) -> Result<bool, String> {
    let url = format!("https://store-api.zoo.ngo/user/{}/reviews", node_name.to_string());
