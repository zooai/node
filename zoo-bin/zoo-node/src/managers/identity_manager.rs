...
use zoo_crypto_identities::ZooRegistryError;
use zoo_message_primitives::schemas::identity::{DeviceIdentity, Identity, StandardIdentity, StandardIdentityType};
...
pub trait IdentityManagerTrait {
    fn find_by_identity_name(&self, full_profile_name: ZooName) -> Option<&Identity>;
...
impl IdentityManager {
    ...
    pub async fn search_local_identity(&self, full_identity_name: &str) -> Option<Identity> {
        let node_in_question = ZooName::new(full_identity_name.to_string()).ok()?.extract_node();
...
impl IdentityManagerTrait for IdentityManager {
    ...
    async fn search_identity(&self, full_identity_name: &str) -> Option<Identity> {
        let identity_name = ZooName::new(full_identity_name.to_string()).ok()?;
    ...
    async fn external_profile_to_global_identity(
        ...
        let full_identity_name = match ZooName::new(full_profile_name.to_string().clone()) {
            Ok(name) => name,
            Err(_) => {
                zoo_log(
                    ZooLogOption::Identity,
                ...
                return Err(format!(
                    "Failed to convert profile name to ZooName: {}",
                    full_profile_name
                ));
            }
        };
...
impl IdentityManager {
    ...
    pub fn verify_message_signature(
        ...
        original_message: &ZooMessage,
        decrypted_message: &ZooMessage,
    ) -> Result<(), NodeError> {
