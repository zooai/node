...
use zoo_message_primitives::schemas::wallet_mixed::AddressBalanceList;
use zoo_message_primitives::schemas::x402_types::Network;
use zoo_message_primitives::schemas::{
    indexable_version::IndexableVersion, invoices::{Invoice, InvoiceStatusEnum}, job::JobLike, llm_providers::common_agent_llm_provider::ProviderOrAgent, zoo_name::ZooName, zoo_preferences::ZooInternalComms, zoo_tool_offering::{ToolPrice, UsageType, UsageTypeInquiry}, tool_router_key::ToolRouterKey, ws_types::{PaymentMetadata, WSMessageType, WidgetMetadata}, x402_types::PaymentRequirements
};
use zoo_message_primitives::zoo_message::zoo_message_schemas::{AssociatedUI, WSTopic};
...
use zoo_sqlite::files::prompts_data;
...
use zoo_tools_primitives::tools::{
    error::ToolError, network_tool::NetworkTool, parameters::Parameters, rust_tools::RustTool, zoo_tool::{ZooTool, ZooToolHeader}, tool_config::ToolConfig, tool_output_arg::ToolOutputArg
};
use std::env;
...
impl ToolRouter {
    ...
    pub async fn initialization(&self, embedding_generator: Arc<dyn EmbeddingGenerator>) -> Result<(), ToolError> {
        ...
        let full_identity =
            ZooName::new(format!("{}/main", node_name)).map_err(|e| ToolError::ExecutionError(e.to_string()))?;
    ...
    pub async fn force_reinstall_all(&self, embedding_generator: Arc<dyn EmbeddingGenerator>) -> Result<(), ToolError> {
        ...
        let node_env = fetch_node_environment();
        let full_identity = ZooName::new(format!("{}/main", node_env.global_identity_name))
    ...
    async fn import_tools_from_directory(
            ...
            .eq("true")
        {
            ...
            let internal_comms = ZooInternalComms {
        ...
        let url = env::var("ZOO_TOOLS_DIRECTORY_URL")
            .unwrap_or_else(|_| format!("https://store-api.zoo.ngo/store/defaults"));

        println!("Fetching tools from remote directory: {}", url);
        let client = reqwest::Client::new();
        let response = client
            .get(url)
            .header("X-Zoo-Version", env!("CARGO_PKG_VERSION"))
            .send()
            .await
    ...
    pub async fn add_network_tool(&self, network_tool: NetworkTool) -> Result<(), ToolError> {
        self.sqlite_manager
            .add_tool(ZooTool::Network(network_tool, true))
            .await
            .map(|_| ())
            .map_err(|e| ToolError::DatabaseError(e.to_string()))
    }
        ...
        for tool in rust_tools {
            ...
            let _ = match self.sqlite_manager.get_tool_by_key(&rust_tool.tool_router_key) {
                Err(SqliteManagerError::ToolNotFound(_)) => {
                    added_count += 1;
                    self.sqlite_manager
                        .add_tool(ZooTool::Rust(rust_tool, true))
                        .await
                        .map_err(|e| ToolError::DatabaseError(e.to_string()))
                }
                Err(e) => Err(ToolError::DatabaseError(e.to_string())),
    ...
    async fn add_testing_network_tools(&self) -> Result<(), ToolError> {
        ...
        if std::env::var("ADD_TESTING_EXTERNAL_NETWORK_ECHO").unwrap_or_else(|_| "false".to_string()) == "true" {
            ...
            let provider = ZooName::new("@@node1_test.sep-zoo".to_string()).unwrap();
    ...
    pub async fn get_tool_by_name(&self, name: &str) -> Result<Option<ZooTool>, ToolError> {
    ...
    ) -> Result<Option<ZooTool>, ToolError> {
    ...
    pub async fn vector_search_enabled_tools(
        ...
        query: &str,
        num_of_results: u64,
    ) -> Result<Vec<ZooToolHeader>, ToolError> {
    ...
    pub async fn call_function(
        ...
        // Add job scope files
        let job_scope =
            ZooFileManager::get_absolute_path_for_job_scope(&context.db(), &context.full_job().job_id());
        ...
        let function_config = zoo_tool.get_config_from_env();
            ...
            ProviderOrAgent::Agent(agent) => {
                if agent_id.is_some() {
                    function_config_vec = override_tool_config(
                        zoo_tool.tool_router_key().to_string_without_version().clone(),
                        agent,
        ...
        match zoo_tool {
            ZooTool::MCPServer(mcp_server_tool, _is_enabled) => {
            ...
            ZooTool::Python(python_tool, _is_enabled) => {
                let node_env = fetch_node_environment();
                ...
                let tool_id = zoo_tool.tool_router_key().to_string_without_version().clone();
            ...
            ZooTool::Rust(rust_tool, _is_enabled) => {
                // Get app_id from Cron UI if present, otherwise use job_id
                let app_id = match context.full_job().associated_ui().as_ref() {
                    Some(AssociatedUI::Cron(cron_id)) => cron_id.clone(),
                    _ => context.full_job().job_id().to_string(),
            ...
            ZooTool::Agent(agent_tool, _is_enabled) => {
                let job_callback_manager = context.job_callback_manager();
                let mut job_manager: Option<Arc<Mutex<JobManager>>> = None;
                if let Some(job_callback_manager) = &job_callback_manager {
                    let job_callback_manager = job_callback_manager.lock().await;
            ...
            ZooTool::Deno(deno_tool, _is_enabled) => {
                let node_env = fetch_node_environment();
                let node_storage_path = node_env
                    .node_storage_path
                    .clone()
            ...
            ZooTool::Network(network_tool, _is_enabled) => {
    ...
    pub async fn call_js_function(
        ...
        requester_node_name: ZooName,
        js_tool_name: &str,
    ) -> Result<String, LLMProviderError> {
        let zoo_tool = self.get_tool_by_name(js_tool_name).await?;

        if zoo_tool.is_none() {
            return Err(LLMProviderError::FunctionNotFound(js_tool_name.to_string()));
        }

        let zoo_tool = zoo_tool.unwrap();
        let function_config = zoo_tool.get_config_from_env();
        let function_config_vec: Vec<ToolConfig> = function_config.into_iter().collect();

        let js_tool = match zoo_tool.clone() {
            ZooTool::Deno(js_tool, _) => js_tool,
        ...
        let tool_id = zoo_tool.tool_router_key().clone().to_string_without_version();
        let support_files =
            generate_tool_definitions(tools, CodeLanguage::Typescript, self.sqlite_manager.clone(), false)
                .await
                .map_err(|e| ToolError::ExecutionError(format!("Failed to generate tool definitions: {:?}", e)))?;

        let oauth = match zoo_tool.clone() {
            ZooTool::Deno(deno_tool, _) => deno_tool.oauth.clone(),
            ZooTool::Python(python_tool, _) => python_tool.oauth.clone(),
            _ => return Err(LLMProviderError::FunctionNotFound(js_tool_name.to_string())),
        };
        ...
        let env = generate_execution_environment(
            ...
            format!("xid-{}", tool_id),
            None,
            zoo_tool.tool_router_key().clone().to_string_without_version(),
