...
func TestParseBootstrapFlags_Defaults(t *testing.T) {
	...
	if cfg.Chains[0].Name != "LiquidEVM" {
		t.Errorf("Chain[0].Name: got %q, want %q", cfg.Chains[0].Name, "LiquidEVM")
	}
	if cfg.Chains[0].Alias != "zooevm" {
		t.Errorf("Chain[0].Alias: got %q, want %q", cfg.Chains[0].Alias, "zooevm")
	}
	if cfg.Chains[1].Name != "LiquidDEX" {
		t.Errorf("Chain[1].Name: got %q, want %q", cfg.Chains[1].Name, "LiquidDEX")
	}
...
func TestParseBootstrapFlags_ChainsDexOnly(t *testing.T) {
	...
	if cfg.Chains[0].Name != "LiquidDEX" {
