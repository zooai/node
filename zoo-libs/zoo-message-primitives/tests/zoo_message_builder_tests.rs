...
mod tests {
    use zoo_message_primitives::{
        ...
        zoo_message::{
            zoo_message::{MessageBody, MessageData},
            zoo_message_schemas::{IdentityPermissions, JobMessage, MessageSchemaType},
        },
        zoo_utils::{
            encryption::{
    ...
    fn test_job_message() {
        ...
        let node_sender = "@@localhost.zoo".to_string();
        let node_receiver = "@@localhost.zoo".to_string();
        ...
        let message_result = ZooMessageBuilder::job_message(
            inbox.clone(),
        ...
        if let MessageBody::Unencrypted(zoo_body) = &message.body {
            if let MessageData::Unencrypted(zoo_data) = &zoo_body.message_data {
                let job_message: JobMessage = serde_json::from_str(&zoo_data.message_raw_content).unwrap();
    ...
    fn test_builder_with_all_fields_no_encryption() {
        ...
        let recipient = "@@other_node.zoo".to_string();
        let sender = "@@my_node.zoo".to_string();
        let scheduled_time = "2023-07-02T20:53:34Z".to_string();

        let message_result = ZooMessageBuilder::new(my_encryption_sk, my_identity_sk, node2_encryption_pk)
        ...
        if let MessageBody::Unencrypted(zoo_body) = message.body {
            if let MessageData::Unencrypted(zoo_data) = zoo_body.message_data {
                assert_eq!(zoo_data.message_raw_content, "body content");
            }
            assert_eq!(zoo_body.internal_metadata.sender_subidentity, "");
            assert_eq!(zoo_body.internal_metadata.recipient_subidentity, "");
            assert_eq!(
                zoo_body.internal_metadata.inbox,
                "inbox::@@my_node.zoo::@@other_node.zoo::false"
            );
        }
    ...
    fn test_builder_with_all_fields_body_encryption() {
        ...
        let message_result = ZooMessageBuilder::new(my_encryption_sk.clone(), my_identity_sk, node2_encryption_pk)
            .message_raw_content("body content".to_string())
        ...
        if let MessageBody::Unencrypted(zoo_body) = binding {
            let message_data = zoo_body.message_data;
            if let MessageData::Unencrypted(zoo_data) = message_data {
                let decrypted_content = zoo_data.message_raw_content;
        ...
        let binding = decrypted_message.body.clone();
        if let MessageBody::Unencrypted(zoo_body) = binding {
            let internal_metadata = &zoo_body.internal_metadata;
    ...
    fn test_builder_with_all_fields_content_encryption() {
        ...
        if let MessageBody::Unencrypted(zoo_body) = message.clone().body {
            assert_eq!(
                zoo_body.internal_metadata.encryption,
                EncryptionMethod::DiffieHellmanChaChaPoly1305
            );
        }
        ...
        if let MessageBody::Unencrypted(zoo_body) = decrypted_message.body {
            if let MessageData::Unencrypted(zoo_data) = zoo_body.message_data {
                assert_eq!(zoo_data.message_raw_content, "body content");
            }
        }
    ...
    fn test_builder_use_code_registration() {
        ...
        let message_result = ZooMessageBuilder::use_code_registration_for_device(
            my_encryption_sk.clone(),
            my_identity_sk,
        ...
        if let MessageBody::Unencrypted(zoo_body) = decrypted_message.body {
            if let MessageData::Unencrypted(zoo_data) = zoo_body.message_data {
                ...
                let parsed_content: Result<RegistrationCodeSimple, _> =
                    serde_json::from_str(&zoo_data.message_raw_content);
            ...
            assert_eq!(zoo_body.internal_metadata.sender_subidentity, sender_subidentity);
        }
    ...
    fn test_initial_registration_with_no_code_for_device() {
        ...
        let message_result = ZooMessageBuilder::initial_registration_with_no_code_for_device(
    ...
    fn test_builder_missing_fields() {
        ...
        let message_result = ZooMessageBuilder::new(my_encryption_sk, my_identity_sk, node2_encryption_pk).build();
