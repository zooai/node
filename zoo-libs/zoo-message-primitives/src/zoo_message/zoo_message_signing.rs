use super::zoo_message::{MessageBody, ZooMessage};
...
impl ZooMessage {
    pub fn sign_outer_layer(&self, secret_key: &SigningKey) -> Result<ZooMessage, ZooMessageError> {
        ...
        let message_hash_bytes = hex::decode(message_hash)
            .map_err(|e| ZooMessageError::SigningError(format!("Failed to decode message hash: {}", e)))?;
    ...
    pub fn sign_inner_layer(&mut self, secret_key: &SigningKey) -> Result<(), ZooMessageError> {
        // Calculate the hash of the ZooBody with an empty inner signature
        let zoo_body_hash = self.calculate_message_hash_with_empty_inner_signature()?;

        // Convert the hexadecimal hash back to bytes
        let zoo_body_hash_bytes = hex::decode(zoo_body_hash)
            .map_err(|e| ZooMessageError::SigningError(format!("Failed to decode message hash: {}", e)))?;

        // Sign the hash of the ZooBody
        let signature = secret_key.sign(zoo_body_hash_bytes.as_slice());
        ...
        match &mut self.body {
            ...
            _ => {
                return Err(ZooMessageError::SigningError(
    ...
    pub fn verify_outer_layer_signature(
        ...
        let signature_bytes = hex::decode(hex_signature)
            .map_err(|e| ZooMessageError::SigningError(format!("Failed to decode signature: {}", e)))?;
        ...
        let signature_bytes_array: &[u8; 64] =
            signature_bytes_slice
                ...
                .map_err(|e: std::array::TryFromSliceError| {
                    ZooMessageError::SigningError(format!("Failed to convert signature bytes to array: {}", e))
                })?;
    ...
    pub fn verify_inner_layer_signature(
        ...
        // Convert the MessageBody to a ZooBody
        let zoo_body = match &self.body {
        ...
        // Get the signature from zoo_body.internal_metadata.signature
        let signature = &zoo_body.internal_metadata.signature;
        ...
        // Verify the signature against the hash of the ZooBody
        match public_key.verify(&zoo_body_hash_bytes, &signature) {
            Ok(_) => Ok(true),
            Err(_) => Ok(false),
        }
    }
    ...
    pub fn calculate_message_hash_with_empty_inner_signature(&self) -> Result<String, ZooMessageError> {
        // Get the ZooBody ready for hashing
        let zoo_body_string = self.inner_content_ready_for_hashing()?;

        // Hash the ZooBody
        let mut hasher = Hasher::new();
        hasher.update(zoo_body_string.as_bytes());
        let zoo_body_hash = hasher.finalize();

        Ok(hex::encode(zoo_body_hash.as_bytes()))
    }

    pub fn inner_content_ready_for_hashing(&self) -> Result<String, ZooMessageError> {
        ...
        // Prepare ZooBody for hashing - set signature to empty
        let mut zoo_body_for_hashing = zoo_body.clone();
        zoo_body_for_hashing.internal_metadata.signature = String::from("");
        zoo_body_for_hashing.internal_metadata.node_api_data = None;

        // Sort the JSON Value using the helper function
        let sorted_json = Self::to_sorted_json(&zoo_body_for_hashing);

        // Convert the sorted JSON Value back to a string
        let zoo_body_string = sorted_json.to_string();
    ...
    ) -> Result<(String, String), ZooMessageError> {
        // Read the secret desktop key from the environment
        let secret_desktop_key = env::var("SECRET_DESKTOP_KEY").map_err(|e| {
            ZooMessageError::SigningError(format!("Failed to read SECRET_DESKTOP_KEY from environment: {}", e))
        })?;
...
mod tests {
    use crate::{
        zoo_message::zoo_message::{EncryptedZooBody, ExternalMetadata, ZooVersion}, zoo_utils::{encryption::EncryptionMethod, signatures::unsafe_deterministic_signature_keypair}
    };
    ...
    fn test_generate_desktop_installation_proof() {
        ...
        let result = ZooMessage::generate_desktop_installation_proof(&public_key, &secret_key);
    ...
    fn test_calculate_message_hash_with_empty_outer_signature() {
        // Create a ZooMessage instance with the provided data
        let message = ZooMessage {
            body: MessageBody::Encrypted(EncryptedZooBody {
            ...
            external_metadata: ExternalMetadata {
                sender: "@@my_local_ai.sep-zoo".to_string(),
                recipient: "@@my_local_ai.sep-zoo".to_string(),
        ...
        // Expected hash
        let expected_hash = "58e1e5f420afc137e788d89f095e3404666d9e6eee9fa94296f78790494cdc11";
