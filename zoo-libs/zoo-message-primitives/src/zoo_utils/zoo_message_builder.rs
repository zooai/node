use crate::zoo_message::zoo_message::NodeApiData;
...
use crate::{
    ...
    zoo_message::{
        zoo_message::{
            ExternalMetadata, InternalMetadata, MessageBody, MessageData, ZooBody, ZooData, ZooMessage,
            ZooVersion,
        },
...
use super::{
    encryption::{clone_static_secret_key, encryption_secret_key_to_string}, zoo_time::ZooStringTime, signatures::{clone_signature_secret_key, signature_secret_key_to_string}
};

pub type ZooNameString = String;
...
impl ZooMessageBuilder {
    ...
    pub fn external_metadata(mut self, recipient: ZooNameString, sender: ZooNameString) -> Self {
        let signature = "".to_string();
        let other = "".to_string();
        let intra_sender = "".to_string();
        let scheduled_time = ZooStringTime::generate_time_now();
    ...
    pub fn build(&self) -> Result<ZooMessage, &'static str> {
        ...
        if let Some(internal_metadata) = &mut new_self.internal_metadata {
            ...
            let mut unsigned_msg = ZooMessage {
            ...
            let signed_body = match unsigned_msg.body {
                MessageBody::Unencrypted(ref body) => ZooBody {
                    message_data: new_message_data.clone(),
...
impl std::fmt::Debug for ZooMessageBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("ZooMessageBuilder")
