...
impl ZooPath {
    /// Private helper method to create a ZooPath from a &str.
    ...
    /// Creates a new ZooPath from a string slice, ensuring it's absolute relative to the base path.
    /// If `path` is not absolute, it is joined to the base path.
    pub fn from_str(path: &str) -> Self {
        Self::new(path)
    }

    /// Creates a new ZooPath from a String, ensuring it's absolute relative to the base path.
    /// If `path` is not absolute, it is joined to the base path.
    ...
    /// Converts the ZooPath to a Path reference.
    pub fn as_path(&self) -> &Path {
        &self.path
    }

    /// Returns the relative path of this ZooPath with respect to the base path.
    ...
    /// Creates a new ZooPath representing the base path.
    ...
    /// Returns the parent directory as a new ZooPath, if it exists.
    pub fn parent(&self) -> Option<ZooPath> {
        self.path.parent().map(|p| ZooPath::new(p.to_str().unwrap()))
    }
}

// Implement Display for ZooPath to easily print it
impl fmt::Display for ZooPath {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

impl<'de> Deserialize<'de> for ZooPath {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        struct ZooPathVisitor;

        impl<'de> Visitor<'de> for ZooPathVisitor {
            type Value = ZooPath;
                ...
                M: MapAccess<'de>,
            {
                ...
                Ok(ZooPath::from_str(&actual_path))
            }
        }

        // deserialize_any will check the JSON token and call visit_str or visit_map accordingly
        deserializer.deserialize_any(ZooPathVisitor)
    }
}

impl Serialize for ZooPath {
...
#[cfg(test)]
mod tests {
    use crate::zoo_utils::test_utils::testing_create_tempdir_and_set_env_var;
    ...
    fn test_from_string_with_base_path() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let path = ZooPath::from_string("word_files/christmas.docx".to_string());
        assert_eq!(
            path.as_path(),
            Path::new(&format!(
    ...
    fn test_relative_path_outside_base() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let absolute_outside = ZooPath::from_string("/some/other/path".to_string());
        assert_eq!(
    ...
    fn test_extension() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let path_with_extension = ZooPath::from_string("word_files/christmas.docx".to_string());
        assert_eq!(path_with_extension.extension(), Some("docx"));

        let path_without_extension = ZooPath::from_string("word_files/christmas".to_string());
    ...
    fn test_new_with_base_path() {
        ...
        let zoo_path = ZooPath::new(test_path.to_str().unwrap());
        eprintln!("zoo_path: {:?}", zoo_path.full_path());
        assert_eq!(zoo_path.path, test_path);
    }
    ...
    fn test_new_without_base_path() {
        ...
        let zoo_path = ZooPath::new(relative_path);
        eprintln!("zoo_path: {:?}", zoo_path.full_path());
        eprintln!("expected_path: {:?}", expected_path);

        assert_eq!(zoo_path.path, expected_path);
    }
    ...
    fn test_new_with_root_path() {
        let _dir = testing_create_tempdir_and_set_env_var();
        let root_path = "/";
        let zoo_path = ZooPath::new(root_path);

        let expected_path = ZooPath::base_path().join(root_path.trim_start_matches('/'));
        assert_eq!(zoo_path.path, expected_path);
    }
    ...
    fn test_is_file() {
        ...
        let zoo_path = ZooPath::from_string(file_path.to_string());

        fs::create_dir_all(zoo_path.as_path().parent().unwrap()).unwrap();
        fs::write(zoo_path.as_path(), "test".as_bytes()).unwrap();

        assert!(zoo_path.is_file());
    }
    ...
    fn test_filename() {
        ...
        let zoo_path_with_extension = ZooPath::from_string(path_with_extension.to_string());
        fs::create_dir_all(zoo_path_with_extension.as_path().parent().unwrap()).unwrap();
        fs::write(zoo_path_with_extension.as_path(), "test".as_bytes()).unwrap();
        assert_eq!(zoo_path_with_extension.filename(), Some("christmas.docx"));

        // Create a file without an extension
        let path_without_extension = "word_files/christmas";
        let zoo_path_without_extension = ZooPath::from_string(path_without_extension.to_string());
        fs::write(zoo_path_without_extension.as_path(), "test".as_bytes()).unwrap();
        assert_eq!(zoo_path_without_extension.filename(), Some("christmas"));

        // Test a directory path
        let path_with_no_filename = "word_files/";
        let zoo_path_with_no_filename = ZooPath::from_string(path_with_no_filename.to_string());
        assert_eq!(zoo_path_with_no_filename.filename(), None);
    }
    ...
    fn test_serialize_relative_path() {
        ...
        // Create a ZooPath instance
        let path = ZooPath::from_string("word_files/christmas.docx".to_string());

        // Serialize the ZooPath
