use crate::schemas::zoo_name::ZooName;
...
pub struct SerializedLLMProvider {
    ...
    pub full_identity_name: ZooName,
...
impl SerializedLLMProvider {
    pub fn get_provider_string(&self) -> String {
        match &self.model {
            ...
            LLMProviderInterface::ZooBackend(_) => "zoo-backend",
    ...
    pub fn baml_provider_string(&self) -> String {
        match &self.model {
            ...
            LLMProviderInterface::ZooBackend(_) => "zoo-backend".to_string(),
    ...
    pub fn get_model_string(&self) -> String {
        match &self.model {
            ...
            LLMProviderInterface::ZooBackend(zoobackend) => zoobackend.model_type.clone(),
    ...
    pub fn mock_provider() -> Self {
        SerializedLLMProvider {
            ...
            full_identity_name: ZooName::new("@@test.zoo/main/agent/mock_agent".to_string()).unwrap(),
    ...
    pub fn mock_provider_with_reasoning() -> Self {
        SerializedLLMProvider {
            ...
            full_identity_name: ZooName::new("@@test.zoo/main/agent/mock_agent_reasoning".to_string()).unwrap(),
            external_url: Some("https://api.example.com".to_string()),
            api_key: Some("mockapikey".to_string()),
            model: LLMProviderInterface::OpenAI(OpenAI {
...
impl FromStr for LLMProviderInterface {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        } else if s.starts_with("zoo-backend:") {
            let model_type = s.strip_prefix("zoo-backend:").unwrap_or("").to_string();
            Ok(LLMProviderInterface::ZooBackend(ZooBackend { model_type }))
...
impl Serialize for LLMProviderInterface {
        ...
        S: Serializer,
    {
        match self {
            ...
            LLMProviderInterface::ZooBackend(zoobackend) => {
                let model_type = format!("zoo-backend:{}", zoobackend.model_type);
                serializer.serialize_str(&model_type)
            }
...
impl<'de> Visitor<'de> for LLMProviderInterfaceVisitor {
        ...
        E: de::Error,
    {
        ...
        match parts[0] {
            ...
            "zoo-backend" => Ok(LLMProviderInterface::ZooBackend(ZooBackend {
                model_type: parts.get(1).unwrap_or(&"").to_string(),
            })),
