...
impl ToolRouterKey {
    ...
    /// Example: "local:::guillevalin:::echo_function" with node_name "@@guillevalin.sep-zoo"
    /// becomes "__guillevalin_sep_zoo:::guillevalin:::echo_function"
    pub fn to_network_router_key(key_str: &str, node_name: &str) -> Result<String, String> {
        let key = Self::from_string(key_str)?;

        // Create a ZooName to properly validate the node name
        let zoo_name =
            ZooName::new(node_name.to_string()).map_err(|e| format!("Invalid node name '{}': {}", node_name, e))?;

        // Sanitize the node name to create the network source
        let network_source = Self::sanitize(&zoo_name.node_name);
...
mod tests {
    ...
    fn test_tool_router_key_to_string_without_version() {
        ...
        assert_eq!(
            ...
            "local:::__official_zoo:::concat_strings"
        );
    }
    ...
    fn test_tool_router_key_to_string_with_version() {
        ...
        assert_eq!(
            ...
            "local:::__official_zoo:::concat_strings:::1.0"
        );
    }

    #[test]
    fn test_tool_router_key_from_string_without_version() {
        let key_str = "local:::__official_zoo:::concat_strings";
        let key = ToolRouterKey::from_string(key_str).unwrap();
        assert_eq!(
            key,
            ToolRouterKey::new(
                "local".to_string(),
                "__official_zoo".to_string(),
                "concat_strings".to_string(),
                None
            )
        );
    ...
    fn test_tool_router_key_from_string_with_version() {
        let key_str = "local:::__official_zoo:::concat_strings:::1.0";
        let key = ToolRouterKey::from_string(key_str).unwrap();
        assert_eq!(
            key,
            ToolRouterKey::new(
    ...
    fn test_tool_router_key_generation() {
        ...
        // Expected key format
        let expected_key = "local:::__system_zoo:::zoo__download_pages";
    ...
    fn test_tool_router_key_no_spaces_in_to_string() {
        ...
        assert_eq!(key_string, "local:::__system_zoo:::versioned_tool");
    }
    ...
    fn test_to_network_router_key() {
        ...
        let node_name = "@@guillevalin.sep-zoo";
        let network_key = ToolRouterKey::to_network_router_key(original_key, node_name).unwrap();
        assert_eq!(network_key, "__guillevalin_sep_zoo:::guillevalin:::echo_function");
    }
    ...
    fn test_to_network_router_key_with_version() {
        ...
        assert_eq!(
            network_key,
            "__guillevalin_sep_zoo:::guillevalin:::echo_function:::1.0"
        );
    }
    ...
    fn test_to_network_router_key_different_domains() {
        ...
        // Test with .zoo domain
        let node_name1 = "@@alice.sep-zoo";
        let network_key1 = ToolRouterKey::to_network_router_key(original_key, node_name1).unwrap();
        assert_eq!(network_key1, "__alice_sep_zoo:::alice:::test_tool");

        // Test with .arb-sep-zoo domain
        let node_name2 = "@@alice.sep-zoo";
        let network_key2 = ToolRouterKey::to_network_router_key(original_key, node_name2).unwrap();
        assert_eq!(network_key2, "__alice_sep_zoo:::alice:::test_tool");
    }
}
