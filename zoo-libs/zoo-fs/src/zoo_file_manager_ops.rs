...
use crate::zoo_file_manager::ZooFileManager;
use crate::zoo_fs_error::ZooFsError;

impl ZooFileManager {
    /// Add a file: writes a file from `data` to a relative path under `base_dir`.
    pub fn write_file_to_fs(dest_path: ZooPath, data: Vec<u8>) -> Result<(), ZooFsError> {
    ...
    pub fn remove_file(path: ZooPath, sqlite_manager: &SqliteManager) -> Result<(), ZooFsError> {
        // Check if file exists on filesystem
        if !path.exists() {
            return Err(ZooFsError::FileNotFoundOnFilesystem);
        }
    ...
    pub fn create_folder(path: ZooPath) -> Result<(), ZooFsError> {
    ...
    /// might be necessary.
    pub fn remove_folder(path: ZooPath, sqlite_manager: &SqliteManager) -> Result<(), ZooFsError> {
        if !path.exists() {
            return Err(ZooFsError::FolderNotFoundOnFilesystem);
        }

        // Iterate over each file or directory in the directory
        for entry in fs::read_dir(path.as_path())? {
            let entry = entry?;
            let file_path = ZooPath::from_str(entry.path().to_str().unwrap());
    ...
    pub fn copy_file(input_path: ZooPath, destination_path: ZooPath) -> Result<(), ZooFsError> {
...
#[cfg(test)]
mod tests {
    use crate::zoo_file_manager::FileProcessingMode;
    ...
    fn test_remove_empty_folder() {
        ...
        let path = ZooPath::from_string(dir.path().to_string_lossy().to_string());
        ...
        assert!(ZooFileManager::remove_folder(path.clone(), &sqlite_manager).is_ok());
    ...
    fn test_remove_non_empty_folder() {
        ...
        let folder_path = ZooPath::new("test_folder");
        fs::create_dir_all(folder_path.as_path()).unwrap();

        let file_path = ZooPath::new("test_folder/test_file.txt");
        ...
        assert!(ZooFileManager::remove_folder(folder_path.clone(), &sqlite_manager).is_ok());
    ...
    fn test_add_file() {
        let dir = tempdir().unwrap();
        let path = ZooPath::from_string(dir.path().join("test_file.txt").to_string_lossy().to_string());
        let data = b"Hello, Zoo!".to_vec();

        // Add the file
        assert!(ZooFileManager::write_file_to_fs(path.clone(), data.clone()).is_ok());
    ...
    fn test_rename_file_without_embeddings() {
        ...
        let old_path = ZooPath::from_string("old_file.txt".to_string());
        let new_path = ZooPath::from_string("new_file.txt".to_string());

        let data = b"Hello, Zoo!".to_vec();

        // Create the original file
        ZooFileManager::write_file_to_fs(old_path.clone(), data.clone()).unwrap();
        ...
        let contents =
            ZooFileManager::list_directory_contents(ZooPath::from_base_path(), &sqlite_manager).unwrap();
        ...
        // Rename the file
        let rename_result = ZooFileManager::rename_file(old_path.clone(), new_path.clone(), &sqlite_manager);
        assert!(
            rename_result.is_ok(),
    ...
    async fn test_rename_file_with_embeddings() {
        ...
        let _ = ZooFileManager::process_embeddings_for_file(
            old_path.clone(),
            &sqlite_manager,
    ...
    fn test_copy_file() {
        ...
        let input_path = ZooPath::from_string("input_file.txt".to_string());
        let destination_dir = ZooPath::from_string("destination_dir".to_string());
        let data = b"Hello, Zoo!".to_vec();

        // Add the input file
        assert!(ZooFileManager::write_file_to_fs(input_path.clone(), data.clone()).is_ok());

        // Create the destination directory
        assert!(ZooFileManager::create_folder(destination_dir.clone()).is_ok());

        // Copy the file
        assert!(ZooFileManager::copy_file(input_path.clone(), destination_dir.clone()).is_ok());
    ...
    fn test_remove_file_and_folder() {
        ...
        let folder_path = ZooPath::from_string("test_folder".to_string());
        let data = b"Hello, Zoo!".to_vec();
        ...
        // Add the file
        assert!(ZooFileManager::write_file_to_fs(file_path.clone(), data.clone()).is_ok());
        ...
        // Remove the file
        assert!(ZooFileManager::remove_file(file_path.clone(), &sqlite_manager).is_ok());
        ...
        // Create a folder
        assert!(ZooFileManager::create_folder(folder_path.clone()).is_ok());
    ...
    fn test_move_folder() {
        ...
        let folder_path = ZooPath::from_string("test_folder".to_string());
        let new_folder_path = ZooPath::from_string("new_test_folder".to_string());

        let file1_path = ZooPath::from_string("test_folder/file1.txt".to_string());
        let file2_path = ZooPath::from_string("test_folder/file2.txt".to_string());
        ...
        assert!(ZooFileManager::create_folder(folder_path.clone()).is_ok());
        assert!(ZooFileManager::write_file_to_fs(file1_path.clone(), data1.clone()).is_ok());
        assert!(ZooFileManager::write_file_to_fs(file2_path.clone(), data2.clone()).is_ok());
        ...
        // Move the folder
        assert!(ZooFileManager::move_folder(folder_path.clone(), new_folder_path.clone(), &sqlite_manager).is_ok());

        // Verify the files have been moved in the filesystem
        let new_file1_path = ZooPath::from_string("new_test_folder/file1.txt".to_string());
        let new_file2_path = ZooPath::from_string("new_test_folder/file2.txt".to_string());
    ...
    fn test_remove_folder_with_subfolder_and_embeddings() {
        ...
        let main_folder_path = ZooPath::from_string("main_folder".to_string());
        let subfolder_path = ZooPath::from_string("main_folder/subfolder".to_string());

        let file1_path = ZooPath::from_string("main_folder/file1.txt".to_string());
        let file2_path = ZooPath::from_string("main_folder/subfolder/file2.txt".to_string());
        ...
        assert!(ZooFileManager::create_folder(main_folder_path.clone()).is_ok());
        assert!(ZooFileManager::create_folder(subfolder_path.clone()).is_ok());
        assert!(ZooFileManager::write_file_to_fs(file1_path.clone(), data1.clone()).is_ok());
        assert!(ZooFileManager::write_file_to_fs(file2_path.clone(), data2.clone()).is_ok());
        ...
        assert!(ZooFileManager::remove_folder(main_folder_path.clone(), &sqlite_manager).is_ok());
