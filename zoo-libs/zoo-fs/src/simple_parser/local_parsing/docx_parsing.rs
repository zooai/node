...
use zoo_non_rust_code::functions::parse_docx::parse_docx;
...
impl LocalFileParser {
    pub async fn process_docx_file(
        ...
        let parsed_docx = parse_docx(file_path)
            .await
            .map_err(|_| ZooFsError::FailedDOCXParsing)?;

        let mut text_groups = Vec::new();
        ZooFileParser::push_text_group_by_depth(&mut text_groups, 0, parsed_docx.text, max_node_text_size, None);
        Ok(text_groups)
    }
}
