...
impl ZooFileParser {
    ...
    fn extract_metadata_from_capture(
        ...
        *parsed_any_metadata = true;
        ...
        match key {
            ...
            _ if key == ZooFileParser::datetime_metadata_key()
                || key == ZooFileParser::timestamp_metadata_key() =>
            {
                let datetime = chrono::DateTime::parse_from_rfc3339(value);
                match datetime {
                    Ok(_) => {
                        metadata.insert(ZooFileParser::datetime_metadata_key(), value.to_string());
                        if is_pure {
                            "".to_string()
                        } else {
                            value.to_string()
            ...
            _ if key == ZooFileParser::page_numbers_metadata_key() => {
    ...
    pub fn parse_and_split_into_text_groups(
        ...
        let (parsed_text, metadata, parsed_any_metadata) = ZooFileParser::parse_and_extract_metadata(&text);
        let (parsed_md_text, md_metadata) = ZooFileParser::parse_and_extract_md_metadata(&parsed_text);
        ...
        if parsed_md_text.len() as u64 > max_node_text_size {
            ...
            let chunks = if parsed_any_metadata {
                ZooFileParser::split_into_chunks_with_metadata(&text, max_node_text_size as usize)
            } else {
