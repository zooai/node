...
use crate::simple_parser::file_parser_helper::ZooFileParser;
...
impl TextGroup {
    ...
    pub fn format_text_for_embedding(&self, max_node_text_size: u64) -> String {
        ...
        let keywords: Vec<String> = ZooFileParser::extract_keywords(&vec![self.clone()], 1);
    ...
    pub fn push_page_number(&mut self, page_number: u32) {
        ...
        if let Some(page_numbers_metadata) = self.metadata.get(&ZooFileParser::page_numbers_metadata_key()) {
        ...
        self.metadata.insert(
            ZooFileParser::page_numbers_metadata_key(),
            format!(
                "[{}]",
                unique_page_numbers
                    .iter()
