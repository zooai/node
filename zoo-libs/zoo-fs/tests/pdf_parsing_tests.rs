use zoo_vector_resources::{
    ...
    file_parser::file_parser::ZooFileParser,
    source::DistributionInfo,
};

#[tokio::test]
async fn local_pdf_parsing_test() {
    ZooFileParser::initialize_local_file_parser().await.unwrap();

    let generator = RemoteEmbeddingGenerator::new_default();
    let source_file_name = "zoo_intro.pdf";
    let buffer = std::fs::read(format!("../../files/{}", source_file_name)).unwrap();
    let resource = ZooFileParser::process_file_into_resource(
        buffer,
        &generator,
    ...
    let query_string = "What is Zoo?".to_string();
