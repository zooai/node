...
use zoo_message_primitives::{
    schemas::zoo_fs::{ParsedFile, ZooFileChunk}, zoo_utils::zoo_path::ZooPath
};
...
impl SqliteManager {
    ...
    pub fn get_chunk_with_embedding(
        &self,
        chunk_id: i64,
    ) -> Result<Option<(ZooFileChunk, Option<Vec<f32>>)>, SqliteManagerError> {
    ...
    pub fn get_chunks_for_parsed_file(&self, parsed_file_id: i64) -> Result<Vec<ZooFileChunk>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
    ...
    ) -> Result<Vec<ZooFileChunk>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
    ...
    pub fn search_chunks(
        ...
        limit: usize,
    ) -> Result<Vec<(ZooFileChunk, f64)>, SqliteManagerError> {
        let conn = self.get_connection()?;
        ...
        for (chunk_id, distance) in chunk_ids_and_distances {
            if let Some(chunk) = self.get_chunk_with_embedding(chunk_id)? {
                results.push((chunk.0, distance)); // Assuming get_chunk_with_embedding returns (ZooFileChunk,
                                                   // Option<Vec<f32>>)
            }
        }
    ...
    pub fn get_parsed_file_by_zoo_path(
        &self,
        path: &ZooPath,
