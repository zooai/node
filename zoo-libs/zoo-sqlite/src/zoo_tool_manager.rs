...
use zoo_message_primitives::schemas::indexable_version::IndexableVersion;
use zoo_tools_primitives::tools::mcp_server_tool::MCPServerTool;
use zoo_tools_primitives::tools::zoo_tool::{ZooTool, ZooToolHeader};
use zoo_tools_primitives::tools::tool_config::{BasicConfig, ToolConfig};
use std::collections::{HashMap, HashSet};

impl SqliteManager {
    // Adds a ZooTool entry to the zoo_tools table
    pub async fn add_tool(&self, tool: ZooTool) -> Result<ZooTool, SqliteManagerError> {
    ...
    pub fn add_tool_with_vector(
        ...
        tool: ZooTool,
        embedding: Vec<f32>,
    ) -> Result<ZooTool, SqliteManagerError> {
        ...
        let exists: bool = tx.query_row(
            "SELECT EXISTS(SELECT 1 FROM zoo_tools WHERE tool_key = ?1 AND version = ?2)",
        ...
        let (on_demand_price, is_network) = match tool_clone {
            ZooTool::Network(ref network_tool, _) => {
        ...
        let (_, is_network) = match tool_clone {
            ZooTool::Network(_, _) => (Some(0.0), true),
            _ => (None, false),
        };

        // Insert the embedding into the zoo_tools_vec_items table with metadata
        tx.execute(
            "INSERT INTO zoo_tools_vec_items (
                embedding, 
                is_enabled, 
                is_network, 
                tool_key
    ...
    pub async fn upgrade_tool(&self, new_tool: ZooTool) -> Result<ZooTool, SqliteManagerError> {
        // Generate or retrieve the embedding
    ...
    pub fn upgrade_tool_with_vector(
        ...
        let (_old_config, upgraded): (Vec<ToolConfig>, ZooTool) = match (old_tool, new_tool) {
            (ZooTool::Deno(old_deno, _), ZooTool::Deno(mut new_deno, is_enabled)) => {
                ...
                (old_config, ZooTool::Deno(new_deno, is_enabled))
            }
            (ZooTool::Network(old_network, _), ZooTool::Network(mut new_network, is_enabled)) => {
                ...
                (old_config, ZooTool::Network(new_network, is_enabled))
            }
            (ZooTool::Python(old_python, _), ZooTool::Python(mut new_python, is_enabled)) => {
                ...
                (old_config, ZooTool::Python(new_python, is_enabled))
            }
    ...
    pub fn tool_vector_search_with_vector(
    ...
    ) -> Result<Vec<(ZooToolHeader, f64)>, SqliteManagerError> {
        ...
        // Retrieve the corresponding ZooToolHeaders and pair with distances
    ...
    /// Retrieves a ZooToolHeader based on its tool_key
    pub fn get_tool_header_by_key(&self, tool_key: &str) -> Result<ZooToolHeader, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT tool_header FROM zoo_tools WHERE tool_key = ?1 ORDER BY version DESC LIMIT 1")?;
        ...
        let tool_header: ZooToolHeader = serde_json::from_slice(&tool_header_data).map_err(|e| {
    ...
    pub fn get_tool_by_agent_id(&self, agent_id: &str) -> Result<ZooTool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT tool_key FROM zoo_tools WHERE tool_type = 'Agent' AND tool_key LIKE ?1")?;
        ...
        let tools = row.collect::<Result<Vec<Option<ZooTool>>, _>>()?;
    ...
    /// Retrieves a ZooTool based on its tool_key, sorted by descending version
    pub fn get_tool_by_key(&self, tool_key: &str) -> Result<ZooTool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
            conn.prepare("SELECT tool_data FROM zoo_tools WHERE tool_key = ?1 ORDER BY version DESC LIMIT 1")?;
        ...
        // Deserialize the tool_data to get the ZooTool
        let tool: ZooTool = serde_json::from_slice(&tool_data).map_err(|e| {
            eprintln!("Deserialization error: {}", e);
            SqliteManagerError::SerializationError(e.to_string())
        })?;
    ...
    // Updates a ZooTool entry in the zoo_tools table with a new embedding
    pub fn update_tool_with_vector(
        ...
        let rowid: i64 = tx
            .query_row(
                "SELECT rowid FROM zoo_tools WHERE tool_key = ?1 AND version = ?2",
    ...
    /// Updates a ZooTool entry by generating a new embedding
    pub async fn update_tool(&self, tool: ZooTool) -> Result<ZooTool, SqliteManagerError> {
        // Generate or retrieve the embedding
        let embedding = match tool.get_embedding() {
            Some(embedding) => embedding,
            None => self.generate_embeddings(&tool.format_embedding_string()).await?,
    ...
    /// Retrieves all ZooToolHeader entries from the zoo_tools table
    pub fn get_all_tool_headers(&self) -> Result<Vec<ZooToolHeader>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT tool_header FROM zoo_tools")?;
    ...
    /// Removes one or all versions of a ZooTool entry from the zoo_tools table.
    ...
    pub fn remove_tool(&self, tool_key: &str, version: Option<String>) -> Result<(), SqliteManagerError> {
        ...
        let rowids: Vec<i64> = if let Some(ver_str) = version {
            ...
            let mut stmt = tx.prepare("SELECT rowid FROM zoo_tools WHERE tool_key = ?1")?;
        ...
        tx.execute(
            "DELETE FROM invoice_network_errors WHERE invoice_id IN (
                SELECT invoice_id FROM invoices WHERE zoo_offering_key = ?1
            )",
            params![tool_key_lower],
        )?;

        // Update any existing invoices to set zoo_offering_key to NULL
        ...
        tx.execute(
            "UPDATE invoices SET zoo_offering_key = NULL WHERE zoo_offering_key = ?1",
            params![tool_key_lower],
        )?;
        ...
        // Remove tool playground entries (these reference zoo_tools with cascade delete)
        // No explicit delete needed as ON DELETE CASCADE should handle it

        // Delete each row from zoo_tools and zoo_tools_vec_items
        for rowid in &rowids {
            tx.execute("DELETE FROM zoo_tools WHERE rowid = ?1", params![rowid])?;
            tx.execute("DELETE FROM zoo_tools_vec_items WHERE rowid = ?1", params![rowid])?;
        }
        ...
        for rowid in rowids {
            fts_conn.execute("DELETE FROM zoo_tools_fts WHERE rowid = ?1", params![rowid])?;
        }

        Ok(())
    }

    /// Checks if the zoo_tools table is empty
    pub fn is_empty(&self) -> Result<bool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM zoo_tools", [], |row| row.get(0))
            .map_err(|e| {
                eprintln!("Database error: {}", e);
                SqliteManagerError::DatabaseError(e)
            })?;
    ...
    /// Checks if a tool exists in the zoo_tools table by its tool_key
    pub fn tool_exists(&self, tool_key: &str, version: Option<IndexableVersion>) -> Result<bool, SqliteManagerError> {
        ...
        let exists = match version {
            ...
            None => conn.query_row(
                "SELECT EXISTS(SELECT 1 FROM zoo_tools WHERE tool_key = ?1)",
    ...
    /// Checks if there are any JS tools in the zoo_tools table
    pub fn has_any_js_tools(&self) -> Result<bool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let exists: bool = conn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM zoo_tools WHERE tool_type = 'Deno')",
                [],
                |row| row.get(0),
            )
            .map_err(|e| {
    ...
    /// Checks if there are any Rust tools in the zoo_tools table
    pub fn has_rust_tools(&self) -> Result<bool, SqliteManagerError> {
        let conn = self.get_connection()?;
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM zoo_tools WHERE tool_type = 'Rust'",
                [],
                |row| row.get(0),
            )
            .map_err(|e| {
    ...
    pub fn update_tools_fts(&self, tool: &ZooTool) -> Result<(), SqliteManagerError> {
        ...
        tx.execute("DELETE FROM zoo_tools_fts WHERE name = ?1", params![tool.name()])?;

        // Insert the updated tool name
        tx.execute("INSERT INTO zoo_tools_fts(name) VALUES (?1)", params![tool.name()])?;
    ...
    pub fn search_tools_fts(&self, query: &str) -> Result<Vec<ZooToolHeader>, SqliteManagerError> {
        // Get a connection from the in-memory pool for FTS operations
        let fts_conn = self
            .fts_pool
            .get()
        ...
        for phrase in phrases_to_search {
            let mut stmt = fts_conn.prepare("SELECT name FROM zoo_tools_fts WHERE zoo_tools_fts MATCH ?1")?;
            ...
            for name_res in name_iter {
                ...
                if seen.insert(name.clone()) {
                    let mut stmt =
                        conn.prepare("SELECT tool_header FROM zoo_tools WHERE name = ?1 ORDER BY version DESC")?;
    ...
    pub fn sync_tools_fts_table(&self) -> Result<(), SqliteManagerError> {
        // Use the pooled connection to access the zoo_tools table
        let conn = self.get_connection()?;

        let mut stmt = conn.prepare("SELECT rowid, name FROM zoo_tools")?;
        ...
        while let Some(row) = rows.next()? {
            ...
            fts_conn.execute(
                "INSERT INTO zoo_tools_fts(rowid, name) VALUES (?1, ?2)",
                params![rowid, name],
            )?;
        }
        Ok(())
    ...
    pub fn update_tools_vector(
        ...
        let (is_enabled, is_network): (i32, i32) = tx.query_row(
            "SELECT is_enabled, is_network FROM zoo_tools WHERE tool_key = ?1",
            params![tool_key],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;

        tx.execute(
            "UPDATE zoo_tools_vec_items SET 
                embedding = ?1,
                is_enabled = ?2,
                is_network = ?3
             WHERE tool_key = ?4",
    ...
    pub fn get_tool_by_key_and_version(
        ...
        let tool: ZooTool = if let Some(version) = version {
            let version_number = version.get_version_number();
            conn.query_row(
                "SELECT tool_data FROM zoo_tools WHERE tool_key = ?1 AND version = ?2",
            ...
            conn.query_row(
                "SELECT tool_data FROM zoo_tools WHERE tool_key = ?1 ORDER BY version DESC LIMIT 1",
                params![tool_key_lower],
                |row| {
                    let tool_data: Vec<u8> = row.get(0)?;
                    serde_json::from_slice(&tool_data).map_err(|e| {
    ...
    /// Retrieves all ZooTool entries that belong to a specific tool_set.
    pub fn get_tools_by_tool_set(&self, tool_set_name: &str) -> Result<Vec<ZooTool>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT tool_data FROM zoo_tools")?;
        ...
        for tool_result in tool_iter {
            let tool: ZooTool = tool_result.map_err(|e| {
                eprintln!("Database error: {}", e);
                SqliteManagerError::DatabaseError(e)
            })?;
    ...
    pub fn get_all_tools_from_mcp_server(
        ...
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT tool_data FROM zoo_tools WHERE tool_type = 'MCPServer' AND (json(tool_data) -> '$.content[0].mcp_server_ref') = '\"' || ? || '\"'")?;
        ...
        while let Some(row) = rows.next()? {
            ...
            if let ZooTool::MCPServer(mcp_tool, _) = tool {
                tools.push(mcp_tool);
            }
        }
        Ok(tools)
    ...
    pub fn delete_all_tools_from_mcp_server(&self, mcp_server_id: String) -> Result<usize, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("DELETE FROM zoo_tools WHERE tool_type = 'MCPServer' AND (json(tool_data) -> '$.content[0].mcp_server_ref') = '\"' || ? || '\"'")?;
...
mod tests {
    ...
    use zoo_message_primitives::schemas::x402_types::PaymentRequirements;
    use zoo_tools_primitives::tools::deno_tools::DenoTool;
    use zoo_tools_primitives::tools::network_tool::NetworkTool;
    use zoo_tools_primitives::tools::parameters::Parameters;
    use zoo_tools_primitives::tools::python_tools::PythonTool;
    use zoo_tools_primitives::tools::tool_config::{BasicConfig, ToolConfig};
    use zoo_tools_primitives::tools::tool_output_arg::ToolOutputArg;
    use zoo_tools_primitives::tools::tool_types::OperatingSystem;
    use zoo_tools_primitives::tools::tool_types::RunnerType;
    use zoo_tools_primitives::tools::tool_types::ToolResult;
    use std::collections::{HashMap, HashSet};
    ...
    async fn test_add_deno_tool() {
        ...
        // Wrap the DenoTool in a ZooTool::Deno variant
        let zoo_tool = ZooTool::Deno(deno_tool, true);

        // Debug: Print the tool before adding
        println!("Testing add_tool with: {:?}", zoo_tool);

        let vector = SqliteManager::generate_vector_for_testing(0.1);

        // Add the tool to the database
        let result = manager.add_tool_with_vector(zoo_tool.clone(), vector);
        assert!(result.is_ok());

        // Retrieve the tool using the new method
        let retrieved_tool = manager
            .get_tool_by_key(&zoo_tool.tool_router_key().to_string_without_version())
            .unwrap();

        // Assert that the retrieved tool matches the added tool
        assert_eq!(retrieved_tool.name(), zoo_tool.name());
        assert_eq!(retrieved_tool.description(), zoo_tool.description());
        assert_eq!(retrieved_tool.author(), zoo_tool.author());

        // Remove the tool from the database
        manager
            .remove_tool(&zoo_tool.tool_router_key().to_string_without_version(), None)
            .unwrap();

        // Verify that the tool is removed from the zoo_tools table
        let tool_removal_result = manager.get_tool_by_key(&zoo_tool.tool_router_key().to_string_without_version());
        assert!(tool_removal_result.is_err());

        // Verify that the embedding is removed from the zoo_tools_vec_items table
        let conn = manager.get_connection().unwrap();
        let embedding_removal_result: Result<i64, _> = conn.query_row(
            "SELECT rowid FROM zoo_tools_vec_items WHERE rowid = ?1",
            params![zoo_tool
                .tool_router_key()
    ...
    async fn test_tool_vector_search() {
        ...
        let zoo_tool_1 = ZooTool::Deno(deno_tool_1, true);
        let zoo_tool_2 = ZooTool::Deno(deno_tool_2, true);
        let zoo_tool_3 = ZooTool::Deno(deno_tool_3, true);

        // Add the tools to the database with different vectors
        manager
            .add_tool_with_vector(zoo_tool_1.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool_2.clone(), SqliteManager::generate_vector_for_testing(0.5))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool_3.clone(), SqliteManager::generate_vector_for_testing(0.9))
            .unwrap();
        ...
        let num_results = 1;
        let search_results: Vec<ZooToolHeader> = manager
    ...
    async fn test_update_middle_tool() {
        ...
        // Wrap the DenoTools in ZooTool::Deno variants
        let zoo_tool_1 = ZooTool::Deno(deno_tool_1, true);
        let zoo_tool_2 = ZooTool::Deno(deno_tool_2, true);
        let zoo_tool_3 = ZooTool::Deno(deno_tool_3, true);
        ...
        manager
            ...
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool_2.clone(), SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool_3.clone(), SqliteManager::generate_vector_for_testing(0.3))
            .unwrap();
        ...
        let mut updated_tool_2 = zoo_tool_2.clone();
        if let ZooTool::Deno(ref mut deno_tool, _) = updated_tool_2 {
        ...
        // Manually query the zoo_tools_vec_items table to verify the vector
        let conn = manager.get_connection().unwrap();
        let rowid: i64 = conn
            .query_row(
                "SELECT rowid FROM zoo_tools WHERE tool_key = ?1",
    ...
    async fn test_add_duplicate_tool() {
        ...
        // Add the tool to the database
        let vector = SqliteManager::generate_vector_for_testing(0.1);
        let result = manager.add_tool_with_vector(zoo_tool.clone(), vector.clone());
        assert!(result.is_ok());

        // Attempt to add the same tool again
        let duplicate_result = manager.add_tool_with_vector(zoo_tool.clone(), vector);
    ...
    async fn test_fts_search() {
        ...
        for (i, tool) in tools.into_iter().enumerate() {
            let zoo_tool = ZooTool::Deno(tool, true);
            let vector = SqliteManager::generate_vector_for_testing(0.1 * (i + 1) as f32);
            if let Err(e) = manager.add_tool_with_vector(zoo_tool, vector) {
    ...
    async fn test_tool_vector_search_with_disabled() {
        ...
        let zoo_enabled = ZooTool::Deno(enabled_tool, true);
        let zoo_disabled = ZooTool::Deno(disabled_tool, false);

        manager
            .add_tool_with_vector(zoo_enabled.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
        ...
        if let ZooTool::Deno(mut deno_tool, _is_enabled) = zoo_enabled {
            deno_tool.activated = false;
            let updated_tool = ZooTool::Deno(deno_tool, false);
    ...
    async fn test_tool_vector_search_with_network_filter() {
        ...
        let usage_type = UsageType::PerUse(ToolPrice::Payment(vec![PaymentRequirements {
            ...
            resource: "https://zoo.ngo".to_string(),
        ...
        let provider = ZooName::new("@@agent_provider.sep-zoo".to_string()).unwrap();
        let tool_router_key = ToolRouterKey::new(
            provider.to_string(),
            "Author 3".to_string(),
        ...
        // Wrap the tools in ZooTool variants
        let zoo_enabled_non_network = ZooTool::Deno(enabled_non_network_tool, true);
        let zoo_disabled_non_network = ZooTool::Deno(disabled_non_network_tool, false);
        let zoo_enabled_network = ZooTool::Network(enabled_network_tool, true);

        // Add the tools to the database
        manager
            .add_tool_with_vector(
                zoo_enabled_non_network.clone(),
                SqliteManager::generate_vector_for_testing(0.1),
            )
            .unwrap();
        manager
            .add_tool_with_vector(
                zoo_disabled_non_network.clone(),
                SqliteManager::generate_vector_for_testing(0.2),
            )
            .unwrap();
        manager
            .add_tool_with_vector(
                zoo_enabled_network.clone(),
    ...
    async fn test_tool_vector_search_with_vector_limited() {
        ...
        let zoo_tool1 = ZooTool::Deno(tool1, true);
        let zoo_tool2 = ZooTool::Deno(tool2, true);
        let zoo_tool3 = ZooTool::Deno(tool3, true);

        // Tool 2 will have the closest vector to our search query
        manager
            .add_tool_with_vector(zoo_tool1.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool2.clone(), SqliteManager::generate_vector_for_testing(0.5))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool3.clone(), SqliteManager::generate_vector_for_testing(0.9))
            .unwrap();
        ...
        let limited_tool_keys = vec![
            zoo_tool1.tool_router_key().to_string_without_version(),
            zoo_tool3.tool_router_key().to_string_without_version(),
        ];
    ...
    async fn test_add_tools_with_different_versions() {
        ...
        // Wrap the DenoTools in ZooTool::Deno variants
        let zoo_tool_v1 = ZooTool::Deno(deno_tool_v1, true);
        let zoo_tool_v2 = ZooTool::Deno(deno_tool_v2, true);

        // Add both tools to the database
        manager
            .add_tool_with_vector(zoo_tool_v1.clone(), SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool_v2.clone(), SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();

        // Retrieve and verify both tools are added
        let retrieved_tool_v1 = manager
            .get_tool_by_key(&zoo_tool_v1.tool_router_key().to_string_without_version())
            .unwrap();
        let retrieved_tool_v2 = manager
            .get_tool_by_key(&zoo_tool_v2.tool_router_key().to_string_without_version())
            .unwrap();
        ...
        let retrieved_tool_v1_0 = manager
            .get_tool_by_key_and_version(
                &zoo_tool_v1.tool_router_key().to_string_without_version(),
                Some(version_1_0),
            )
            .unwrap();
        ...
        let retrieved_tool_latest = manager
            .get_tool_by_key_and_version(&zoo_tool_v1.tool_router_key().to_string_without_version(), None)
            .unwrap();
    ...
    async fn test_upgrade_tool_preserves_config() {
        ...
        let zoo_tool_v1 = ZooTool::Deno(deno_tool_v1.clone(), true);
        let vector_v1 = SqliteManager::generate_vector_for_testing(0.1);
        manager
            .add_tool_with_vector(zoo_tool_v1.clone(), vector_v1)
            .unwrap();
        ...
        let zoo_tool_v2 = ZooTool::Deno(deno_tool_v2.clone(), true);

        // Upgrade to version 2.0.0
        let vector_v2 = SqliteManager::generate_vector_for_testing(0.2);
        let upgraded = manager
            .upgrade_tool_with_vector(zoo_tool_v2.clone(), vector_v2)
            .unwrap();
        ...
        if let ZooTool::Deno(new_tool, _) = retrieved {
    ...
    async fn test_upgrade_tool_preserves_config_python() {
        ...
        let zoo_tool_v1 = ZooTool::Python(python_tool_v1, true);
        manager
            .add_tool_with_vector(zoo_tool_v1, SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        ...
        let zoo_tool_v2 = ZooTool::Python(python_tool_v2, true);
        let upgraded = manager
            .upgrade_tool_with_vector(zoo_tool_v2, SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();
        ...
        if let ZooTool::Python(new_tool, _) = retrieved {
    ...
    async fn test_get_tools_by_tool_set() {
        ...
        let zoo_tool1 = ZooTool::Deno(tool1, true);
        let zoo_tool2 = ZooTool::Deno(tool2, true);
        let zoo_tool3 = ZooTool::Python(tool3, true);
        let zoo_tool4 = ZooTool::Deno(tool4, true);

        manager
            .add_tool_with_vector(zoo_tool1, SqliteManager::generate_vector_for_testing(0.1))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool2, SqliteManager::generate_vector_for_testing(0.2))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool3, SqliteManager::generate_vector_for_testing(0.3))
            .unwrap();
        manager
            .add_tool_with_vector(zoo_tool4, SqliteManager::generate_vector_for_testing(0.4))
            .unwrap();
    ...
    async fn test_set_common_toolset_config() {
        ...
        // Add tools
        let zoo_tool1 = ZooTool::Deno(tool1, true);
        let zoo_tool2 = ZooTool::Python(tool2, true);
        let zoo_tool3 = ZooTool::Deno(tool3, true);

        manager.add_tool(zoo_tool1.clone()).await.unwrap();
        manager.add_tool(zoo_tool2.clone()).await.unwrap();
        manager.add_tool(zoo_tool3.clone()).await.unwrap();
        ...
        assert!(updated_keys_set.contains(&zoo_tool1.tool_router_key().to_string_without_version()));
        assert!(updated_keys_set.contains(&zoo_tool2.tool_router_key().to_string_without_version()));

        // Verify Tool 1 config
        let updated_tool1 = manager
            .get_tool_by_key(&zoo_tool1.tool_router_key().to_string_without_version())
            .unwrap();
        if let ZooTool::Deno(t, _) = updated_tool1 {
        ...
        let updated_tool2 = manager
            .get_tool_by_key(&zoo_tool2.tool_router_key().to_string_without_version())
            .unwrap();
        if let ZooTool::Python(t, _) = updated_tool2 {
            let api_key_config = t.config.iter().find(|c| c.name() == "api_key").unwrap();
            assert_eq!(api_key_config.header(), json!("new_common_key"));
        ...
        let updated_tool3 = manager
            .get_tool_by_key(&zoo_tool3.tool_router_key().to_string_without_version())
            .unwrap();
        if let ZooTool::Deno(t, _) = updated_tool3 {
            let api_key_config = t.config.iter().find(|c| c.name() == "api_key").unwrap();
    ...
    async fn test_add_duplicate_python_tool() {
        ...
        // Wrap the PythonTool in a ZooTool::Python variant
        let zoo_tool = ZooTool::Python(python_tool_data, true);
    ...
    // Helper function to create an MCPServerTool wrapped in ZooTool
    fn create_mcp_zoo_tool(name: &str, mcp_server_id: String, profile_name: &str, version: &str) -> ZooTool {
        ...
        ZooTool::MCPServer(mcp_tool_data, true)
    }

    // Helper for Deno tool
    fn create_deno_zoo_tool(name: &str, author: &str, version: &str) -> ZooTool {
        let tool_router_key = ToolRouterKey::new(
        ...
        ZooTool::Deno(deno_tool_data, true)
    }
    ...
    async fn test_get_all_tools_from_mcp_server() {
        ...
        // Tools are still created with Uuid for mcp_server_ref via create_mcp_zoo_tool
        let tool1_s1 = create_mcp_zoo_tool(
            "Tool1S1",
            mcp_server1.id.unwrap_or_default().to_string(),
            "profile1",
            "1.0",
        );
        let tool2_s1 = create_mcp_zoo_tool(
            "Tool2S1",
            mcp_server1.id.unwrap_or_default().to_string(),
            "profile1",
            "1.0",
        );
        let tool1_s2 = create_mcp_zoo_tool(
            "Tool1S2",
            mcp_server2.id.unwrap_or_default().to_string(),
            "profile2",
            "1.0",
        );
        let deno_tool = create_deno_zoo_tool("MyDenoTool", "DenoAuthor", "1.0");
    ...
    async fn test_delete_all_tools_from_mcp_server() {
        ...
        let tool1_del = create_mcp_zoo_tool(
            "MCPTool1ToDel",
        ...
        let tool2_del = create_mcp_zoo_tool(
            "MCPTool2ToDel",
            mcp_server_del.id.unwrap_or_default().to_string(),
            "profile_del",
            "1.0",
        ...
        let tool_keep_mcp = create_mcp_zoo_tool(
            "MCPToolToKeep",
        ...
        let deno_tool_keep = create_deno_zoo_tool("DenoToolToKeep", "DenoAuthor", "1.0");
