use rusqlite::params;
use zoo_message_primitives::schemas::{invoices::InternalInvoiceRequest, zoo_name::ZooName};
...
impl SqliteManager {
    ...
    pub fn get_internal_invoice_request(&self, unique_id: &str) -> Result<InternalInvoiceRequest, SqliteManagerError> {
        ...
        Ok(InternalInvoiceRequest {
            ...
            provider_name: ZooName::new(row.get(0)?).map_err(|e| {
                rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(e.to_string())))
            })?,
            requester_name: ZooName::new(row.get(1)?).map_err(|e| {
                rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(e.to_string())))
            })?,
...
mod tests {
    ...
    use zoo_message_primitives::schemas::{zoo_name::ZooName, zoo_tool_offering::UsageTypeInquiry};
