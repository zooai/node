...
use zoo_message_primitives::schemas::{
    invoices::{Invoice, InvoiceRequestNetworkError}, zoo_name::ZooName
};
...
impl SqliteManager {
    pub fn set_invoice(&self, invoice: &Invoice) -> Result<(), SqliteManagerError> {
        ...
        match self.get_tool_offering(&invoice.zoo_offering.tool_key) {
            Err(SqliteManagerError::ToolOfferingNotFound(_)) => {
                self.set_tool_offering(invoice.zoo_offering.clone())?
            }
            Err(e) => return Err(e),
            Ok(_) => {}
        }
    ...
    pub fn get_invoice(&self, invoice_id: &str) -> Result<Invoice, SqliteManagerError> {
        ...
        let invoice = stmt
            .query_row(params![invoice_id], |row| {
                ...
                let zoo_offering_key: String = row.get(4)?;
                ...
                Ok(Invoice {
                    ...
                    parent_message_id,
                    provider_name: ZooName::new(provider_name).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
                    requester_name: ZooName::new(requester_name).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
                    ...
                    zoo_offering: self
                        .get_tool_offering(&zoo_offering_key)
        ...
        let error = stmt
            .query_row(params![invoice_id], |row| {
                Ok(InvoiceRequestNetworkError {
                    invoice_id: row.get(0)?,
                    provider_name: ZooName::new(row.get(1)?).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
                    requester_name: ZooName::new(row.get(2)?).map_err(|e| {
                        rusqlite::Error::ToSqlConversionFailure(Box::new(SqliteManagerError::SerializationError(
                            e.to_string(),
                        )))
                    })?,
...
mod tests {
    ...
    use zoo_message_primitives::schemas::{
        invoices::InvoiceStatusEnum, zoo_name::ZooName, zoo_tool_offering::{ZooToolOffering, ToolPrice, UsageType, UsageTypeInquiry}, wallet_mixed::{NetworkIdentifier, PublicAddress}, x402_types::Network
    };
    ...
    fn test_set_invoice() {
        ...
        let invoice = Invoice {
            ...
            provider_name: ZooName::new("@@node1.zoo/main_profile_node1".to_string()).unwrap(),
            requester_name: ZooName::new("@@node2.zoo/main_profile_node2".to_string()).unwrap(),
