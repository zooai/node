...
use zoo_message_primitives::{
    schemas::{
        identity::StandardIdentity, inbox_name::InboxName, inbox_permission::InboxPermission, job_config::JobConfig, zoo_name::ZooName, smart_inbox::{LLMProviderSubset, ProviderType, SmartInbox}, ws_types::{WSMessageType, WSUpdateHandler}
    }, zoo_message::{
        zoo_message::{NodeApiData, ZooMessage}, zoo_message_schemas::WSTopic
    }, zoo_utils::zoo_time::ZooStringTime
};
...
impl SqliteManager {
    pub fn create_empty_inbox(&self, inbox_name: String, is_hidden: Option<bool>) -> Result<(), SqliteManagerError> {
        ...
        conn.execute(
            ...
            params![
                inbox_name,
                smart_inbox_name,
                ZooStringTime::generate_time_now(),
                is_hidden
            ],
        )?;
        Ok(())
    ...
    pub async fn unsafe_insert_inbox_message(
        ...
        let mut time_key = match ext_metadata.scheduled_time.is_empty() {
            true => ZooStringTime::generate_time_now(),
        ...
        if let InboxName::JobInbox { .. } = inbox_name_manager {
            if let Some(parent_key) = &parent_key.clone() {
                ...
                if parsed_time_key < parsed_parent_time {
                    time_key = ZooStringTime::generate_time_now();
                }
            }
        }
            ...
            .map_err(|e| SqliteManagerError::SomeError(e.to_string()))?;
        {
            ...
            tx.execute(
            "INSERT OR REPLACE INTO inbox_messages (message_hash, inbox_name, zoo_message, parent_message_hash, time_key) VALUES (?1, ?2, ?3, ?4, ?5)",
    ...
    pub fn fetch_message_and_hash(&self, hash_key: &str) -> Result<(ZooMessage, String), SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare("SELECT zoo_message FROM inbox_messages WHERE message_hash = ?1")?;
        let mut rows = stmt.query(params![hash_key])?;

        let encoded_message: Vec<u8> = rows.next()?.ok_or(SqliteManagerError::DataNotFound)?.get(0)?;
        let message = ZooMessage::decode_message_result(encoded_message)
            .map_err(|e| SqliteManagerError::SomeError(e.to_string()))?;
    ...
    pub fn get_last_messages_from_inbox(
    ...
    ) -> Result<Vec<Vec<ZooMessage>>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT message_hash, parent_message_hash, zoo_message FROM inbox_messages
                WHERE inbox_name = ?1
        ...
        let message_rows = stmt.query_map(params![inbox_name], |row| {
            ...
            let message = ZooMessage::decode_message_result(encoded_message).map_err(|e| {
    ...
    pub fn get_last_unread_messages_from_inbox(
    ...
    ) -> Result<Vec<ZooMessage>, SqliteManagerError> {
        ...
        // Flatten the Vec<Vec<ZooMessage>> to Vec<ZooMessage>
        let messages: Vec<ZooMessage> = messages.into_iter().flatten().collect();
    ...
    fn get_agent_provider(&self, p: ZooName, parent_id: String) -> (Option<LLMProviderSubset>, ProviderType) {
    ...
    pub fn get_last_messages_from_all(&self, n: usize) -> Result<Vec<ZooMessage>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT zoo_message FROM inbox_messages
                ORDER BY time_key DESC
                LIMIT ?1",
        )?;
    ...
    pub fn get_first_message_from_inbox(
        &self,
        inbox_name: String,
    ) -> Result<Option<ZooMessage>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn.prepare(
            "SELECT zoo_message FROM inbox_messages 
            WHERE inbox_name = ?1 
...
mod tests {
    ...
    use zoo_message_primitives::{
        schemas::identity::StandardIdentityType, zoo_message::{
            ...
            encryption::{unsafe_deterministic_encryption_keypair, EncryptionMethod}, job_scope::MinimalJobScope, search_mode::VectorSearchMode, zoo_message_builder::ZooMessageBuilder, signatures::{clone_signature_secret_key, unsafe_deterministic_signature_keypair}
        }
    };
    ...
    async fn test_insert_single_inbox_message() {
        ...
        let node_identity_name = "@@node.zoo";
    ...
    async fn test_insert_two_messages_and_check_order_and_parent() {
        ...
        let expected_parent_hash = if let MessageBody::Unencrypted(zoo_body) = &messages[0][0].body {
            zoo_body
                .internal_metadata
                .node_api_data
                .as_ref()
        ...
        let actual_parent_hash = if let MessageBody::Unencrypted(zoo_body) = &messages[1][0].body {
            zoo_body
                .internal_metadata
                .node_api_data
                .as_ref()
    ...
    async fn db_inbox() {
        ...
        let result = ZooMessage::decode_message_result(message.encode_message().unwrap());
        ...
        assert_eq!(
            inbox_name_value,
            "inbox::@@node1.zoo::@@node1.zoo/main_profile_node1::false".to_string()
        );
    ...
    async fn benchmark_get_all_smart_inboxes_for_profile() {
        ...
        let profile_identity = StandardIdentity::new(
            ZooName::from_node_and_profile_names(node_identity_name.to_string(), subidentity_name.to_string())
                .unwrap(),
            None,
