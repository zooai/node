...
use zoo_message_primitives::schemas::{
    llm_providers::serialized_llm_provider::SerializedLLMProvider, zoo_name::ZooName
};
...
impl SqliteManager {
    ...
    pub fn remove_llm_provider(&self, llm_provider_id: &str, profile: &ZooName) -> Result<(), SqliteManagerError> {
        let conn = self.get_connection()?;
    ...
    pub fn get_llm_provider_profiles_with_access(
        ...
        let rows = stmt.query_map(params![&llm_provider_id], |row| {
            ...
            ZooName::new(full_identity_name).map_err(|e| {
    ...
    fn db_llm_provider_id(llm_provider_id: &str, profile: &ZooName) -> Result<String, SqliteManagerError> {
...
mod tests {
    ...
    fn test_add_llm_provider() {
        ...
        let identity = ZooName::new("@@alice.zoo/profileName/agent/myChatGPTAgent".to_string()).unwrap();
    ...
    fn test_get_llm_providers_for_profile() {
        ...
        let identity1 = ZooName::new("@@alice.zoo/profileName1/agent/myChatGPTAgent".to_string()).unwrap();
        let identity2 = ZooName::new("@@bob.zoo/profileName2/agent/myChatGPTAgent2".to_string()).unwrap();
