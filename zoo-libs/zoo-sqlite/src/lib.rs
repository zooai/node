...
impl SqliteManager {
    ...
    fn migrate_agents_full_identity_name(manager: &SqliteManager) -> Result<(), SqliteManagerError> {
        ...
        for mut agent in agents {
            if !agent.full_identity_name.has_profile() {
                ...
                agent.full_identity_name = ZooName::new(format!(
                    "{}/main/agent/{}",
    ...
    fn migrate_agents_table(conn: &rusqlite::Connection) -> Result<()> {
        ...
        let mut stmt = conn
            .prepare("SELECT COUNT(*) FROM pragma_table_info('zoo_agents') WHERE name = 'tools_config_override'")?;
        let column_exists: i64 = stmt.query_row([], |row| row.get(0))?;

        // Add the column if it doesn't exist
        if column_exists == 0 {
            conn.execute("ALTER TABLE zoo_agents ADD COLUMN tools_config_override TEXT", [])?;
        }
        // Check if edited column exists
        let mut stmt =
            conn.prepare("SELECT COUNT(*) FROM pragma_table_info('zoo_agents') WHERE name = 'edited'")?;
        let column_exists: i64 = stmt.query_row([], |row| row.get(0))?;

        // Add the column if it doesn't exist
        if column_exists == 0 {
            conn.execute(
                "ALTER TABLE zoo_agents ADD COLUMN edited INTEGER NOT NULL DEFAULT 0",
                [],
            )?;
        }
    ...
    fn migrate_invoices_table(conn: &rusqlite::Connection) -> Result<()> {
        // Check if we need to make zoo_offering_key nullable
        ...
        // If the table definition still has "zoo_offering_key TEXT NOT NULL", we need to migrate
        if table_sql.contains("zoo_offering_key TEXT NOT NULL") {
            ...
            conn.execute(
                "CREATE TABLE invoices_new (
                    ...
                    zoo_offering_key TEXT, -- Made nullable
                    ...
                    FOREIGN KEY(zoo_offering_key) REFERENCES tool_micropayments_requirements(tool_key)
                );",
                [],
            )?;
    ...
    fn initialize_agents_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS zoo_agents (
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_zoo_agents_agent_id ON zoo_agents (agent_id);",
            [],
        )?;
    ...
    fn initialize_settings_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS zoo_settings (
    ...
    fn initialize_sheets_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS zoo_sheets (
    ...
    // Initializes the zoo_prompts table and its indexes
    fn initialize_prompt_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS zoo_prompts (
        ...
        // Create indexes for the zoo_prompts table if needed
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_zoo_prompts_name ON zoo_prompts (name);",
            [],
        )?;
    ...
    fn initialize_tools_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS zoo_tools (
                name TEXT NOT NULL,
                description TEXT,
    ...
    fn migrate_tools_table(conn: &rusqlite::Connection) -> Result<()> {
        ...
        let columns = conn
            .prepare("PRAGMA table_info(zoo_tools)")?
            .query_map([], |row| {
        ...
        if !columns.contains(&"mcp_enabled".to_string()) {
            conn.execute("ALTER TABLE zoo_tools ADD COLUMN mcp_enabled INTEGER", [])?;
        }
    ...
    fn initialize_tools_vector_table(conn: &rusqlite::Connection) -> Result<()> {
        ...
        conn.execute(
            "CREATE VIRTUAL TABLE IF NOT EXISTS zoo_tools_vec_items USING vec0(
                embedding float[384],
                is_enabled integer,
                is_network integer,
                +tool_key text
    ...
    fn initialize_tools_fts_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE VIRTUAL TABLE IF NOT EXISTS zoo_tools_fts USING fts5(name)",
            [],
        )?;
        Ok(())
    }
    ...
    fn initialize_prompts_fts_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE VIRTUAL TABLE IF NOT EXISTS zoo_prompts_fts USING fts5(name)",
            [],
        )?;
        Ok(())
    }
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS tool_playground (
                ...
                FOREIGN KEY (tool_router_key, tool_version)
                    REFERENCES zoo_tools (tool_key, version)
                    ON DELETE CASCADE
            );",
            [],
        )?;
    ...
    fn initialize_wallets_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS zoo_wallet (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    fn initialize_invoice_table(conn: &rusqlite::Connection) -> Result<()> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS invoices (
                ...
                requester_name TEXT NOT NULL,
                usage_type_inquiry TEXT NOT NULL,
                zoo_offering_key TEXT, -- Made nullable to allow tool deletion
                request_date_time TEXT NOT NULL,
                invoice_date_time TEXT NOT NULL,
                expiration_time TEXT NOT NULL,
                status TEXT NOT NULL,
