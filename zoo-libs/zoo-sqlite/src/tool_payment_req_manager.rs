use rusqlite::params;
use zoo_message_primitives::schemas::zoo_tool_offering::{ZooToolOffering, UsageType};

use crate::{SqliteManager, SqliteManagerError};

impl SqliteManager {
    pub fn set_tool_offering(&self, tool_offering: ZooToolOffering) -> Result<(), SqliteManagerError> {
    ...
    pub fn get_tool_offering(&self, tool_key: &str) -> Result<ZooToolOffering, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt = conn
    ...
    pub fn remove_tool_offering(&self, tool_key: &str) -> Result<(), SqliteManagerError> {
        ...
        transaction.execute("UPDATE invoices SET zoo_offering_key = NULL WHERE zoo_offering_key = ?1", params![tool_key])?;
    ...
    pub fn get_all_tool_offerings(&self) -> Result<Vec<ZooToolOffering>, SqliteManagerError> {
        let conn = self.get_connection()?;
        let mut stmt =
...
mod tests {
    ...
    use zoo_message_primitives::schemas::{
        zoo_tool_offering::ToolPrice, x402_types::{Network, PaymentRequirements}
    };
    ...
    async fn test_set_and_get_tool_offerings() {
        ...
        let tool_offering = ZooToolOffering {
            tool_key: "tool_key".to_string(),
    ...
    async fn test_upsert_tool_offering() {
        ...
        let updated_tool_offering = ZooToolOffering {
            tool_key: "tool_key".to_string(),
            usage_type: UsageType::PerUse(ToolPrice::Payment(vec![PaymentRequirements {
                scheme: "exact".to_string(),
    ...
    async fn test_get_all_tool_offerings() {
        let manager = setup_test_db();
        let tool_offering1 = ZooToolOffering {
            tool_key: "local:::__localhost_sep_zoo:::tool_key1".to_string(),
            usage_type: UsageType::PerUse(ToolPrice::Payment(vec![PaymentRequirements {
                scheme: "exact".to_string(),
        ...
        let tool_offering2 = ZooToolOffering {
            tool_key: "local:::__localhost_sep_zoo:::tool_key2".to_string(),
            usage_type: UsageType::PerUse(ToolPrice::Payment(vec![PaymentRequirements {
                scheme: "exact".to_string(),
    ...
    async fn test_remove_tool_offering_with_invoices() {
        ...
        use zoo_message_primitives::schemas::zoo_tool_offering::UsageTypeInquiry;
        use zoo_message_primitives::schemas::wallet_mixed::PublicAddress;
        use rusqlite::params;
        ...
        let invoice = Invoice {
            ...
            provider_name: ZooName::new("@@provider.zoo".to_string()).unwrap(),
            requester_name: ZooName::new("@@requester.zoo".to_string()).unwrap(),
            zoo_offering: tool_offering.clone(),
        ...
        // Verify that the invoice still exists but with zoo_offering_key set to NULL
        ...
        let mut stmt = conn.prepare("SELECT invoice_id, zoo_offering_key FROM invoices WHERE invoice_id = ?1").unwrap();
        ...
        match result {
            Ok((invoice_id, offering_key)) => {
                ...
                assert!(offering_key.is_none(), "zoo_offering_key should be NULL after tool offering removal");
                println!("Verified: zoo_offering_key is NULL as expected");
            },
            Err(err) => {
