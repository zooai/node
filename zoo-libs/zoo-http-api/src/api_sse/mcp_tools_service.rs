...
impl McpToolsService {
    ...
    pub async fn update_tools_cache(&self) -> anyhow::Result<()> {
        ...
        self.node_commands_sender
            .send(NodeCommand::V2ApiListAllMcpZooTools {
                category: None,
                res: tx,
            })
            .await
    ...
    async fn execute_zoo_tool(&self, tool_router_key: String, params: Value) -> Result<String, String> {
        // Create a response channel
        let (tx, rx) = async_channel::bounded(1);
        ...
        async move {
            ...
            match tool_router_key {
                Some(key) => {
                    ...
                    // Convert arguments JsonObject into the Value expected by execute_zoo_tool
                    let params_value = Value::Object(arguments);

                    match self.execute_zoo_tool(key, params_value).await {
                        Ok(output_str) => {
                            tracing::debug!(
