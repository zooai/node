...
use zoo_message_primitives::{
    schemas::{
        zoo_tools::{CodeLanguage, DynamicToolType}, tool_router_key::ToolRouterKey
    }, zoo_message::zoo_message_schemas::JobMessage
};
use zoo_tools_primitives::tools::{
    zoo_tool::ZooToolWithAssets, tool_config::OAuth, tool_playground::ToolPlayground, tool_types::{OperatingSystem, RunnerType}
};
...
pub fn tool_routes(
    ...
    let list_all_zoo_tools_route = warp::path("list_all_zoo_tools")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
        .and_then(list_all_zoo_tools_handler);

    let list_all_network_zoo_tools_route = warp::path("list_all_network_zoo_tools")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and_then(list_all_network_zoo_tools_handler);

    let set_zoo_tool_route = warp::path("set_zoo_tool")
        .and(warp::post())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
        .and(warp::body::json())
        .and_then(set_zoo_tool_handler);
    ...
    let get_zoo_tool_route = warp::path("get_zoo_tool")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
        .and_then(get_zoo_tool_handler);

    let search_zoo_tool_route = warp::path("search_zoo_tool")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
        .and_then(search_zoo_tool_handler);

    let get_zoo_tools_by_tool_set_route = warp::path("tools_from_toolset")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
    ...
    let add_zoo_tool_route = warp::path("add_zoo_tool")
        .and(warp::post())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::body::json())
        .and_then(add_zoo_tool_handler);
    ...
    let tool_execution_route = warp::path("tool_execution")
        ...
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::header::<String>("x-zoo-tool-id"))
        .and(warp::header::<String>("x-zoo-app-id"))
        .and(warp::header::optional::<String>("x-zoo-agent-id"))
        .and(warp::body::json())
    ...
    let set_playground_tool_route = warp::path("set_playground_tool")
        ...
        .and(warp::header::<String>("x-zoo-tool-id"))
        .and(warp::header::<String>("x-zoo-app-id"))
        .and(warp::header::optional::<String>("x-zoo-original-tool-router-key"))
        .and(warp::body::json())
    ...
    // Resolves zoo://file URLs to actual file bytes, providing secure access to files in the node's storage
    let resolve_zoo_file_protocol_route = warp::path("resolve_zoo_file_protocol")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
        .and_then(resolve_zoo_file_protocol_handler);
    ...
    let standalone_playground_route = warp::path("tools_standalone_playground")
        ...
        .and(warp::header::<String>("x-zoo-tool-id"))
        .and(warp::header::<String>("x-zoo-app-id"))
        .and(warp::header::<String>("x-zoo-llm-provider"))
        .and(warp::body::json())
        .and_then(standalone_playground_handler);

    let list_all_zoo_tools_versions_route = warp::path("list_all_zoo_tools_versions")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and_then(list_all_zoo_tools_versions_handler);
    ...
    let get_zoo_tool_metadata_route = warp::path("get_zoo_tool_metadata")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        .and(warp::query::<HashMap<String, String>>())
        .and_then(get_zoo_tool_metadata_handler);
    ...
    tool_execution_route
        ...
        .or(list_all_zoo_tools_route)
        .or(set_zoo_tool_route)
        .or(get_zoo_tool_route)
        .or(search_zoo_tool_route)
        .or(get_zoo_tools_by_tool_set_route)
        .or(set_common_toolset_config_route)
        .or(add_zoo_tool_route)
        ...
        .or(resolve_zoo_file_protocol_route)
...
#[utoipa::path(
    post,
    path = "/v1/node/tool_implementation",
...
#[utoipa::path(
    post,
    path = "/v1/node/tool_metadata_implementation",
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Successfully searched Zoo tools", body = Value),
        (status = 400, description = "Bad request", body = APIError),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn search_zoo_tool_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    query_params: HashMap<String, String>,
) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiSearchZooTool {
            bearer,
            query,
            agent_or_llm,
            res: res_sender,
...
#[utoipa::path(
    get,
    path = "/v1/node/list_all_zoo_tools",
    params(
    ...
    responses(
        (status = 200, description = "Successfully listed all Zoo tools", body = Value),
        (status = 400, description = "Bad request", body = APIError),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn list_all_zoo_tools_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    query_params: HashMap<String, String>,
) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiListAllZooTools {
            bearer,
            category,
            res: res_sender,
        })
...
#[utoipa::path(
    get,
    path = "/v1/node/list_all_network_zoo_tools",
    responses(
...
pub async fn list_all_network_zoo_tools_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
) -> Result<impl warp::Reply, warp::Rejection> {
    let bearer = authorization.strip_prefix("Bearer ").unwrap_or("").to_string();
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiListAllNetworkZooTools {
            bearer,
            res: res_sender,
        })
        .await
...
#[utoipa::path(
    post,
    path = "/v1/node/set_zoo_tool",
    request_body = Value,
    params(
        ("tool_name" = String, Query, description = "Key name of the Zoo tool")
    ),
    responses(
        (status = 200, description = "Successfully set Zoo tool", body = bool),
        (status = 400, description = "Bad request", body = APIError),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn set_zoo_tool_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    query_params: HashMap<String, String>,
    payload: Value,
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiSetZooTool {
            bearer,
            tool_key,
            payload,
            res: res_sender,
...
#[utoipa::path(
    get,
    path = "/v1/node/get_zoo_tool",
    params(
        ("tool_name" = String, Query, description = "Name of the Zoo tool"),
        ("serialize_config" = bool, Query, description = "Serialize the config")
    ),
    responses(
        (status = 200, description = "Successfully retrieved Zoo tool", body = Value),
        (status = 400, description = "Bad request", body = APIError),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn get_zoo_tool_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    query_params: HashMap<String, String>,
) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiGetZooTool {
            bearer,
            payload: tool_name,
            serialize_config,
            res: res_sender,
...
pub async fn add_zoo_tool_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    payload: ZooToolWithAssets,
) -> Result<impl warp::Reply, warp::Rejection> {
    let bearer = authorization.strip_prefix("Bearer ").unwrap_or("").to_string();
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiAddZooTool {
            bearer,
            zoo_tool: payload,
            res: res_sender,
        })
...
#[utoipa::path(
    post,
    path = "/v1/node/add_network_agent",
    request_body = ZooTool,
    responses(
...
#[utoipa::path(
    post,
    path = "/v1/node/set_playground_tool",
...
#[utoipa::path(
    get,
    path = "/v1/node/list_playground_tools",
    responses(
...
#[utoipa::path(
    delete,
    path = "/v1/node/remove_playground_tool",
    params(
...
#[utoipa::path(
    get,
    path = "/v1/node/get_playground_tool",
    params(
...
#[utoipa::path(
    get,
    path = "/v1/node/get_tool_implementation_prompt",
    responses(
...
#[utoipa::path(
    post,
    path = "/v1/node/tool_implementation_undo_to",
...
#[utoipa::path(
    post,
    path = "/v1/node/tool_implementation_code_update",
...
#[utoipa::path(
    get,
    path = "/v1/node/resolve_zoo_file_protocol",
    params(
        ("file" = String, Query, description = "Zoo file protocol")
    ),
    responses(
        (status = 200, description = "Resolved zoo file protocol", body = Vec<u8>),
        (status = 400, description = "Invalid zoo file protocol", body = APIError),
    )
)]
pub async fn resolve_zoo_file_protocol_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    query_params: HashMap<String, String>,
) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let zoo_file_protocol = query_params
        .get("file")
        .ok_or_else(|| {
            warp::reject::custom(APIError {
                code: 400,
                error: "Invalid zoo file protocol".to_string(),
                message: "Zoo file protocol is required".to_string(),
            })
        })?
        .to_string();
    ...
    sender
        .send(NodeCommand::V2ApiResolveZooFileProtocol {
            bearer,
            zoo_file_protocol,
            res: res_sender,
        })
...
#[utoipa::path(
    post,
    path = "/v1/node/disable_all_tools",
    responses(
...
#[utoipa::path(
    get,
    path = "/v1/node/tool_store_proxy/{tool_router_key}",
    params(
...
#[utoipa::path(
    post,
    path = "/v1/node/tools_standalone_playground",
...
#[utoipa::path(
    get,
    path = "/v1/node/list_all_zoo_tools_versions",
    responses(
...
pub async fn list_all_zoo_tools_versions_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
) -> Result<impl warp::Reply, warp::Rejection> {
    let bearer = authorization.strip_prefix("Bearer ").unwrap_or("").to_string();
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiListAllZooToolsVersions {
            bearer,
            res: res_sender,
        })
        .await
...
#[utoipa::path(
    post,
    path = "/v1/node/set_tool_mcp_enabled",
...
#[utoipa::path(
    get,
    path = "/v1/node/tools_from_toolset",
    params(
        ("tool_set_key" = String, Query, description = "The key of the toolset")
    ),
    responses(
        (status = 200, description = "Successfully retrieved tools from toolset", body = Vec<ZooTool>), // Updated body type
        (status = 400, description = "Bad request", body = APIError),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
...
#[utoipa::path(
    post,
    path = "/v1/node/set_common_toolset_config",
...
#[openapi(
    paths(
        ...
        list_all_zoo_tools_handler,
        list_all_network_zoo_tools_handler,
        set_zoo_tool_handler,
        get_zoo_tool_handler,
        search_zoo_tool_handler,
        add_zoo_tool_handler,
        ...
        resolve_zoo_file_protocol_handler,
        tool_asset_handler,
...
#[derive(serde::Serialize, ToSchema)]
pub struct GetZooToolMetadataResponse {
    pub tool_router_key: String,
}

#[utoipa::path(
    get,
    path = "/v1/node/get_zoo_tool_metadata",
    params(
        ("tool_router_key" = String, Query, description = "Tool router key of the Zoo tool metadata to retrieve")
    ),
    responses(
        (status = 200, description = "Successfully retrieved Zoo tool metadata", body = GetZooToolMetadataResponse),
        (status = 400, description = "Bad request", body = APIError),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn get_zoo_tool_metadata_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    query_params: HashMap<String, String>,
) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiGetZooToolMetadata {
            bearer,
            tool_router_key,
            res: res_sender,
        })
