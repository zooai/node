...
use zoo_message_primitives::schemas::llm_providers::serialized_llm_provider::{
    Exo, Gemini, Groq, LLMProviderInterface, Ollama, OpenAI, ZooBackend
};
...
use zoo_message_primitives::zoo_message::zoo_message::{
    EncryptedZooBody, EncryptedZooData, ExternalMetadata, InternalMetadata, MessageBody, MessageData, NodeApiData, ZooBody, ZooData, ZooMessage, ZooVersion
};
...
use zoo_message_primitives::{
    schemas::llm_providers::serialized_llm_provider::SerializedLLMProvider, zoo_message::zoo_message_schemas::APIAddOllamaModels
};
...
pub fn general_routes(
    ...
    let zoo_backend_quota_route = warp::path("zoo_backend_quota")
        .and(warp::get())
        .and(with_sender(node_commands_sender.clone()))
        .and(warp::header::<String>("authorization"))
        ...
        .and_then(|sender, auth, params: HashMap<String, String>| {
            ...
            zoo_backend_quota_handler(sender, auth, model_type)
        });
...
#[utoipa::path(
    get,
    path = "/v1/node/zoo_backend_quota",
    responses(
        (status = 200, description = "Successfully checked Zoo backend quota", body = QuotaResponse),
        (status = 500, description = "Internal server error", body = APIError)
    )
)]
pub async fn zoo_backend_quota_handler(
    sender: Sender<NodeCommand>,
    authorization: String,
    model_type: String,
) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let (res_sender, res_receiver) = async_channel::bounded(1);
    sender
        .send(NodeCommand::V2ApiZooBackendGetQuota {
            bearer,
            model_type,
            res: res_sender,
        })
...
#[openapi(
    ...
    components(
        schemas(APIAddOllamaModels, SerializedLLMProvider, ZooName, LLMProviderInterface,
            ZooMessage, MessageBody, EncryptionMethod, ExternalMetadata, ZooVersion,
            OpenAI, Ollama, Groq, Gemini, Exo, EncryptedZooBody, ZooBody,
            ZooSubidentityType, ZooBackend, InternalMetadata, MessageData, StopLLMRequest,
            NodeApiData, EncryptedZooData, ZooData, MessageSchemaType,
