...
use zoo_message_primitives::zoo_message::zoo_message::{MessageBody, MessageData, ZooMessage};
use zoo_message_primitives::zoo_message::zoo_message_schemas::MessageSchemaType;
use zoo_message_primitives::schemas::agent_network_offering::AgentNetworkOfferingRequest;
use zoo_message_primitives::zoo_utils::encryption::encryption_public_key_to_string;
use x25519_dalek::{StaticSecret as EncryptionStaticKey};
use std::time::{Duration, Instant, SystemTime};
use dashmap::DashMap;
use zoo_crypto_identities::ZooRegistry;
...
pub struct RelayBehaviour {
    ...
    pub request_response: request_response::json::Behaviour<ZooMessage, ZooMessage>,
}
...
pub struct RelayManager {
    ...
    request_response_channels: DashMap<request_response::OutboundRequestId, request_response::ResponseChannel<ZooMessage>>, // request_id -> channel
    external_ip: Option<std::net::IpAddr>, // Store detected external IP
    registry: ZooRegistry, // Blockchain registry for identity verification
    ...
    identity_cache: DashMap<String, (zoo_crypto_identities::OnchainIdentity, Instant)>,
    cache_ttl: Duration,
...
impl RelayManager {
    ...
    pub async fn new(
        ...
        // Configure identify protocol - use same protocol version as Zoo nodes
        ...
        let request_response = request_response::json::Behaviour::new(
            std::iter::once((libp2p::StreamProtocol::new("/zoo/message/1.0.0"), request_response::ProtocolSupport::Full)),
    ...
    async fn request_tool_offerings(&mut self, peer_id: PeerId) {
        if let Some(identity) = self.find_identity_by_peer(&peer_id) {
            ...
            let node_name = if let Ok(parsed_name) = zoo_message_primitives::schemas::zoo_name::ZooName::new(identity.clone()) {
            ...
            let node_encryption_public_key = match self.registry.get_identity_record(node_name.clone(), None).await {
                Ok(identity_record) => {
                    match zoo_message_primitives::zoo_utils::encryption::string_to_encryption_public_key(&identity_record.encryption_key) {
                        Ok(key) => key,
                        Err(e) => {
            ...
            let request_message = match ZooMessageBuilder::new(
            ...
            .internal_metadata(
                "main".to_string(),
                "main".to_string(),
                zoo_message_primitives::zoo_utils::encryption::EncryptionMethod::None,
                None,
            )
            .external_metadata(
                identity.clone(),
    ...
    async fn verify_peer_identity_internal(
        ...
        let identity = if agent_version.contains("zoo") || agent_version.contains("node") {
    ...
    async fn handle_swarm_event(
        ...
        match event {
                ...
                if public_key_bytes.len() >= 32 {
                    ...
                    if let Ok(verifying_key) = ed25519_dalek::VerifyingKey::from_bytes(&key_bytes.try_into().unwrap_or([0u8; 32])) {
                        ...
                        if let Some(verified_identity) = self.verify_peer_identity_internal(verifying_key, info.agent_version.clone()).await {
                            ...
                            let possible_identity = if info.agent_version.ends_with("zoo") {
            ...
            SwarmEvent::Behaviour(RelayBehaviourEvent::RequestResponse(req_resp_event)) => {
                // Handle request-response events for relaying direct messages between Zoo nodes
                    ...
                    request_response::Event::Message { peer, message, .. } => {
                        ...
                        match message {
                            request_response::Message::Request { mut request, channel, .. } => {
                                ...
                                let target_node = if let Ok(parsed_name) = zoo_message_primitives::schemas::zoo_name::ZooName::new(target_identity.clone()) {
                                    parsed_name.get_node_name_string()
                                } else {
                                ...
                                if request.external_metadata.recipient.contains(self.config.relay_node_name.as_str()) && 
                                   !request.external_metadata.intra_sender.is_empty() {
                                    ...
                                    request.external_metadata.recipient = "@@localhost.sep-zoo".to_string();

                                    // Re-encrypt message for localhost recipient
                                    self.relay_message_encryption(&mut request, &"@@localhost.sep-zoo".to_string()).await;
    ...
    async fn process_relay_message(&mut self, request: ZooMessage, sender_peer: PeerId) -> Option<ZooMessage> {
        ...
        let sender_enc_key = if sender_identity.contains("localhost") {
            ...
            match zoo_message_primitives::zoo_utils::encryption::string_to_encryption_public_key(&request.external_metadata.other) {
                Ok(key) => key,
                Err(e) => {
    ...
    fn create_ack_response(
        &self,
        original_request: &ZooMessage,
        sender_encryption_key: &x25519_dalek::PublicKey,
        _processed_message: &ZooMessage,
    ) -> Option<ZooMessage> {
        // Create a simple ACK message using the ZooMessageBuilder
        match ZooMessageBuilder::ack_message(
            self.config.encryption_secret_key.clone(),
            self.config.identity_secret_key.clone(),
    ...
    async fn process_relay_response(&mut self, response: ZooMessage, sender_peer: PeerId) {
        ...
        let decrypted_response = if response.encryption != zoo_message_primitives::zoo_utils::encryption::EncryptionMethod::None {
            // Get sender's encryption key for decryption
            let sender_enc_key = if sender_identity.contains("localhost") {
                println!("🔑 Relay: Using other field for localhost sender");
                match zoo_message_primitives::zoo_utils::encryption::string_to_encryption_public_key(&response.external_metadata.other) {
                    Ok(key) => key,
                    Err(e) => {
                        println!("❌ Relay: Failed to parse sender's encryption key from other field: {}", e);
                        return;
...
async fn relay_message_encryption(&mut self, request: &mut ZooMessage, target_node: &String) {
        // Parse recipient name
        let recipient_name = match zoo_message_primitives::schemas::zoo_name::ZooName::new(target_node.clone()) {
            Ok(name) => name,
            Err(_) => {
        ...
        let recipient_enc_key = if target_node.contains("localhost") {
            ...
            // Parse recipient's encryption key
            match zoo_message_primitives::zoo_utils::encryption::string_to_encryption_public_key(&recipient_identity.encryption_key) {
                Ok(key) => key,
                Err(_) => {
                    println!("❌ Relay: Failed to parse recipient's encryption key");
                    return;
        ...
        let original_sender_enc_key = match zoo_message_primitives::zoo_utils::encryption::string_to_encryption_public_key(&original_sender_identity.encryption_key) {
            Ok(key) => key,
            Err(_) => {
