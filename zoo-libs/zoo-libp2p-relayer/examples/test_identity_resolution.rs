...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let registry = ZooRegistry::new(&rpc_url, &contract_address, None).await?;

    let relay_identity = "@@libp2p_relayer.sep-zoo";
    ...
    match registry.get_identity_record(relay_identity.to_string(), Some(true)).await {
        Ok(identity) => {
            ...
            println!("   Identity: {}", identity.zoo_identity);
        ...
        Err(e) => {
            ...
            println!("💡 This might be why Zoo nodes can't connect to the relay");
        }
    }

    // Also test the other identities
    let test_identities = [
        "@@node1_with_libp2p_relayer.sep-zoo",
        "@@node2_with_libp2p_relayer.sep-zoo",
    ];
