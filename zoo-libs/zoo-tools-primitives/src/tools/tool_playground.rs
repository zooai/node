...
use zoo_message_primitives::schemas::{zoo_tools::CodeLanguage, tool_router_key::ToolRouterKey};
...
mod tests {
    ...
    fn test_deserialize_playground_tool_with_coinbase_data() {
        let json_data = r#"
        {
            ...
            "metadata": {
                "id": "zoo-tool-coinbase-create-wallet",
                "version": "1.0.0",
                "name": "Zoo: Coinbase Wallet Creator",
            ...
            "job_id": "123",
            "job_id_history": [],
            "code": "import { zooDownloadPages } from '@zoo/local-tools'; type CONFIG = {}; type INPUTS = { urls: string[] }; type OUTPUT = { markdowns: string[] }; export async function run(config: CONFIG, inputs: INPUTS): Promise<OUTPUT> { const { urls } = inputs; if (!urls || urls.length === 0) { throw new Error('URL list is required'); } return zooDownloadPages(urls); }",
            "language": "Typescript"
        }
        "#;
        ...
        assert_eq!(deserialized.metadata.name, "Zoo: Coinbase Wallet Creator");
        assert_eq!(deserialized.metadata.description, "Tool for creating a Coinbase wallet");
        assert_eq!(deserialized.metadata.author, "Zoo");
        assert_eq!(
            deserialized.metadata.keywords,
            vec!["coinbase", "wallet", "creator", "zoo"]
        );
        assert_eq!(deserialized.tool_router_key, None);
        assert_eq!(deserialized.job_id, "123");
        assert_eq!(deserialized.job_id_history, Vec::<String>::new());
        assert_eq!(deserialized.code, "import { zooDownloadPages } from '@zoo/local-tools'; type CONFIG = {}; type INPUTS = { urls: string[] }; type OUTPUT = { markdowns: string[] }; export async function run(config: CONFIG, inputs: INPUTS): Promise<OUTPUT> { const { urls } = inputs; if (!urls || urls.length === 0) { throw new Error('URL list is required'); } return zooDownloadPages(urls); }");
    }
