...
pub fn convert_to_zoo_file_protocol(node_name: &ZooName, path: &str, app_id: &str) -> String {
    ...
    if let Some(position) = path_buf.components().enumerate().find(|(_, component)| {
        ...
        let relative_path = path_buf
            ...
            .to_string();
        {
            // Construct the zoo URL with forward slashes regardless of platform
            return format!(
                "zoo://file/{}/{}/{}",
                node_name,
                app_id,
    ...
    println!("[Running DenoTool] Failed to convert to zoo file protocol {}", path);
    "".to_string()
}
...
pub fn get_files_after_with_protocol(
    ...
    .into_iter()
    .map(|(name, _)| serde_json::Value::String(convert_to_zoo_file_protocol(&node_name, &name, &app_id)))
    .collect()
}
